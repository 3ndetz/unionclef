#!/usr/bin/env python3
"""Moving-target follow autotest for the ALTOCLEF @follow command path.

Same arena/chase as follow_test.py, but drives the follower via the altoclef
command `@follow <victim>` (ExecuteCommand) instead of the tungsten `;followPlayer`.
This verifies the routing fix: altoclef FollowPlayerTask now drives the tungsten
follow engine (FollowEntityTask) instead of baritone. The failure mode we guard
against is exactly "forever rebuilds the route and stands still" on a moving
target — so the follower MUST keep moving and stay reasonably close.

Requires the pvp profile (two clients). Exit code 0 = pass.
"""
...
MIN_TOTAL_MOVE = 20.0   # follower must have travelled at least this far overall
...
if op == "state":
    ...
    mc.ExecuteCommand(req["cmd"]); out = {"ok": True}
elif op == "connect":
...
def main():
    ...
    # clear any prior task on both nav stacks
    py4j(FOLLOWER_CONTAINER, "exec", cmd="@stop")
    py4j(FOLLOWER_CONTAINER, "chat", msg=";stop")
    rcon(f"tp {FOLLOWER} -20.5 -60 0.5 0 0")
    rcon(f"tp {VICTIM} -20.5 -60 -7.5 0 0")
    time.sleep(3)

    print("[3/3] chase via ALTOCLEF @follow ...")
    py4j(FOLLOWER_CONTAINER, "exec", cmd=f"@follow {VICTIM}")
    route = loop_route()
    t0 = time.time()
    dists = []
    freeze_windows = 0
    start_follower = entity_pos(FOLLOWER)
    last_follower = start_follower
    total_move = 0.0
    last_move_t = t0
    i = 0
    while time.time() - t0 < CHASE_S:
        ...
        dists.append(d)
        step = sum(abs(a - b) for a, b in zip(fp, last_follower))
        total_move += step
        if step > 0.1:
    ...
    print(f"  total follower travel: {total_move:.1f} (min {MIN_TOTAL_MOVE})")
    ok = (avg <= AVG_DIST_LIMIT and freeze_windows <= FREEZE_LIMIT
          and total_move >= MIN_TOTAL_MOVE)
    if not ok:
        for line in py4j(FOLLOWER_CONTAINER, "recent", n=12).get("chat", []):
            print(f"  chat| {line}")
    print("  FOLLOW_ALTOCLEF:", "PASS" if ok else "FAIL")
