...
class NavStaircase(NavCourse):
    ...
    def course(self, arena, ctx):
        arena.floor(-6, -3, 2, 3, "stone")
        h = FLOOR_Y
        x = 3
        for _ in range(8):
            h += 1
            arena._fill(x, FLOOR_Y - 2, -1, x + 1, h, 1, "stone")
            x += 2
        arena._fill(x, FLOOR_Y - 2, -1, x + 3, h, 1, "stone")
        return (x + 2, h + 1, 0)
...
class NavSteep(NavCourse):
    ...
    def course(self, arena, ctx):
        ...
        for _ in range(6):
            h += 1
            arena._fill(x, FLOOR_Y - 2, -1, x, h, 1, "stone")
            x += 2
        arena._fill(x, FLOOR_Y - 2, -1, x + 3, h, 1, "stone")
        return (x + 2, h + 1, 0)
...
class NavDescend(NavCourse):
    ...
    def course(self, arena, ctx):
        h = FLOOR_Y
        arena._fill(-6, h - 2, -3, 4, h, 3, "stone")
        x = 5
        for drop in (1, 2, 3):
            h -= drop
            arena._fill(x, h - 2, -3, x + 4, h, 3, "stone")
            x += 5
        return (x - 2, h + 1, 0)
...
class NavWater(NavCourse):
    ...
    def course(self, arena, ctx):
        arena.floor(-6, -3, 30, 3, "stone")
        # carve a 6-wide, 3-deep pool and fill it
        arena._fill(8, FLOOR_Y - 2, -3, 13, FLOOR_Y, 3, "air")
        arena._fill(8, FLOOR_Y - 2, -3, 13, FLOOR_Y, 3, "water")
        return (26, STAND_Y, 0)
...
class NavLadder(NavCourse):
    ...
    def course(self, arena, ctx):
        arena.floor(-6, -3, 6, 3, "stone")
        rc = ctx.rcon
        top = FLOOR_Y + 5
        arena._fill(8, FLOOR_Y - 2, -3, 8, top, 3, "stone")     # the pillar wall
        arena._fill(9, top, -3, 14, top, 3, "stone")            # the shelf
        for y in range(FLOOR_Y + 1, top + 1):
            rc.cmd(f"setblock 7 {y} 0 ladder[facing=west]")
        return (12, top + 1, 0)
...
class NavSlime(NavCourse):
    ...
    def course(self, arena, ctx):
        arena._fill(-6, FLOOR_Y - 2, -3, 3, FLOOR_Y + 6, 3, "air")
        arena._fill(-6, FLOOR_Y + 6, -3, 3, FLOOR_Y + 6, 3, "stone")   # high start pad
        arena._fill(6, FLOOR_Y, -3, 10, FLOOR_Y, 3, "slime_block")     # bounce pad
        arena._fill(14, FLOOR_Y + 4, -3, 20, FLOOR_Y + 4, 3, "stone")  # target ledge
        ctx.geo["bot_spawn"] = f"0.5 {FLOOR_Y + 7} 0.5 -90 0"
        return (18, FLOOR_Y + 5, 0)
...
class NavBreak(NavCourse):
    ...
    def course(self, arena, ctx):
        arena.floor(-6, -1, 30, 1, "stone")
        arena._fill(-6, STAND_Y, -2, 30, STAND_Y + 3, -2, "barrier")
        arena._fill(-6, STAND_Y, 2, 30, STAND_Y + 3, 2, "barrier")
        arena._fill(10, STAND_Y, -1, 10, STAND_Y + 1, 1, "dirt")   # the wall
        return (24, STAND_Y, 0)
...
class NavWall2(NavCourse):
    ...
    def course(self, arena, ctx):
        arena.floor(-6, -3, 8, 3, "stone")
        arena._fill(9, FLOOR_Y, -3, 20, FLOOR_Y + 2, 3, "stone")   # 2-high ledge
        return (16, FLOOR_Y + 3, 0)
