...
import shredder.api.command.datatypes.RelativeFile;
...
import static shredder.api.utils.SettingsUtil.*;
        ...
        if (args.hasAny()) {
            ...
            if (args.hasExactlyOne() && !Arrays.asList("s", "save").contains(args.peekString().toLowerCase(Locale.US))) {
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args, MinecraftClient.getInstance().runDirectory.toPath().resolve("shredder").toFile());
                }
