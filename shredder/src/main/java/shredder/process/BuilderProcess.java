...
package shredder.process;

import shredder.Baritone;
import shredder.altoclef.AltoClefSettings;
import shredder.api.pathing.goals.Goal;
import shredder.api.pathing.goals.GoalBlock;
import shredder.api.pathing.goals.GoalComposite;
import shredder.api.pathing.goals.GoalGetToBlock;
import shredder.api.process.IBuilderProcess;
import shredder.api.process.PathingCommand;
import shredder.api.process.PathingCommandType;
import shredder.api.schematic.FillSchematic;
import shredder.api.schematic.ISchematic;
import shredder.api.schematic.IStaticSchematic;
import shredder.api.schematic.MaskSchematic;
import shredder.api.schematic.SubstituteSchematic;
import shredder.api.schematic.format.ISchematicFormat;
import shredder.api.utils.*;
import shredder.api.utils.input.Input;
import shredder.pathing.movement.CalculationContext;
import shredder.pathing.movement.Movement;
import shredder.pathing.movement.MovementHelper;
import shredder.pathing.path.PathExecutor;
import shredder.utils.BaritoneProcessHelper;
import shredder.utils.BlockStateInterface;
import shredder.utils.PathingCommandContext;
import shredder.utils.schematic.MapArtSchematic;
import shredder.utils.schematic.SelectionSchematic;
import shredder.utils.schematic.SchematicSystem;
import shredder.utils.schematic.format.defaults.LitematicaSchematic;
import shredder.utils.schematic.litematica.LitematicaHelper;
import shredder.utils.schematic.schematica.SchematicaHelper;
...
import static shredder.api.pathing.movement.ActionCosts.COST_INF;
...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    public void buildOpenLitematic(int i) {
        if (LitematicaHelper.isLitematicaPresent()) {
            ...
            if (LitematicaHelper.hasLoadedSchematic()) {
                ...
                try {
                    LitematicaSchematic schematic1 = new LitematicaSchematic(NbtIo.readCompressed(Files.newInputStream(LitematicaHelper.getSchematicFile(i).toPath()), NbtSizeTracker.ofUnlimitedBytes()), false);
    ...
    private boolean isDefaultState(final BlockState state) {
        final List<PropertyContainer> propertyContainers = state.getEntries().entrySet().stream().map(PROPERTY_ENTRY_TO_STRING_FUNCTION).toList();
