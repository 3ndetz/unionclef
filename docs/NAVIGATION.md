...
**СЛЕДУЮЩИЙ ШАГ — только диагностика, никаких правок вслепую:**
Доказать логом, попадают ли клетки лестницы в блочную подсказку, которую получает
физический движок. Если НЕТ — проблема в `FastPlanner.special()` (ход не эмитится);
если ДА — проблема в гейте `Node.java:133` или в самом `ClimbALadderMove`.
Разделить эти два случая ОДНИМ экспериментом, прежде чем трогать код.
