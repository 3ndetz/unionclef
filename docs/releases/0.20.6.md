...
- Rewrote jump bridging as sprint-speed telly bridge —
  continuous FJ_SPRINT → FJ_AIRBORNE cycle instead of old state machine
- Block placement bypasses crosshair (processRightClickBlock) —
  fixes objectMouseOver returning MISS at steep pitch (86°+)
- Sprint forced at entity level (setSprinting) — no more losing sprint mid-bridge
- TestBridgingCommand: GoalBlock at player Y instead of GoalXZ
  (pathfinder was descending instead of bridging forward)
- Debug flag, drift correction, cooldown, path-end graceful exit
- Enable: `;settings bridgingMode jump` (or `back_jump` for backward-facing variant)
