...
## 2026-09-15 — Natural death: hostile selection investigation

- Natural3744e9c6 no longer latched the grass shimmy. Pillar on azalea194,56,37 could not place(readyNull),then route changed. Later travelled around y60,16.5HP. At139.7s had11.5HP,146.2s6.5HP;defence alternated zombie approach and flee. At15:31:34UTC server death message named Enderman. Lost kit,respawned and resumed wood collection. Paused after239.2s,lastobserved108.7,142,-31.7,20HP,7blocks. TwoMP4decoded;combat frame inspected;client-death.log saved in outputs/survival-grass-fixed.
- MobDefenseChain ignoredMobs contains Enderman;even an already-angry,visible tracked Enderman is excluded from committed threat scoring/selection while HP>10. EntityTracker already filters calm Endermen. Separately,hostile tracking requires line-of-sight;natural fleeLive reported0dangers between3/4-danger routes,which may explain early flee completion. Neither issue yet fixed or experimentally attributed to this death. First bounded pass: reproduce angry Enderman exclusion with a controlled mob fixture.

- Isolated enderman_priority_test reproduced the selection defect at20HP:visible angry Enderman6.686blocks away,NoAI zombie8blocks away;first committed target was zombie. Anger induced by server player-attack damage,then movement frozen only to isolate selection. IdleTask keeps real defence chains active. This proves selection,not survival against a live mixed group.
- Removed Enderman from ignoredMobs (calm Endermen remain excluded by EntityTracker). Build successful,deploy running. Angry/calm selection repeats and combat audit pending.
- Fixed selection matrix passed5/5 angry and3/3 calm controls:angry closer Enderman selected at20HP,calm Enderman left out and zombie selected. Eight videos decoded. Battle mode now releases both mobs after setup,records health/aliveness and requires surviving both;first live battle running.

- Live mixed battles FAILED2/2:death from Enderman at9.75s/8.53s. Second trace:Enderman39->24HP from three5HP hits;player20->13->6->0;controller stopped ticking when flight began. Zombie was burning in daylight (1HP drops),so this is not a controlled mixed-group comparison. First battle's entity-alive flags became false on respawn;fixture now additionally checks server entity existence before accepting a battle pass. Both videos decoded.
- combatShieldEnabled is shippedfalse for measured offence regressions (TungstenConfig5081ff);simply enabling it is not a fix. Old shield policy holds only below55% recharge,and release clears the key without explicitly stopping use. TriggerBot attacks directly without coordinating shield release. This is a next investigation,not yet a proven cause.
- Existing hungry zombie ambush passed after the selection change:both targets killed,6cobblestone,no continuous mining tool interruption;video decoded. Selection correction can be committed independently;live Enderman survival remains failed.

## 2026-09-15 — Shield release and activation window investigation

- Selection correction committed1639845d;hungry ambush minHP20,hunger5->20,one mining/combat handover. Natural world remains disconnected after the Enderman death.
- Night-normalized shield baseline matrix alternated off/on/off/on,all4failed. Old ON yielded only2/0 actually-blocking samples despite14/7 primitive-held samples;max item-use duration6/2ticks. OFF also failed. MeanFPS25.0/25.5/25.1/19.6,brief lows5–8,so no timing/rate claim from these four runs. Damage and server survival gates remain explicit.
- Local Minecraft bytecode confirms attackEntity sends ATTACK without stopping item use;stopUsingItem sends RELEASE_USE_ITEM then clears client use. Old ShieldBlocker.release only clears useKey;old controller guards only below55% recharge,leaving little time beyond shield activation.
- Pending implementation:hold shield while threat is close;TriggerBot explicitly releases its owned shield after all swing gates pass and before ATTACK;ShieldBlocker.release sends stopUsingItem only for an actively used shield. shieldSwingReleases measures the swing handover. Existing combatShieldEnabled staysFALSE until validation. Clean build passed,deploy running. No fixed result yet.

- First shield-timing implementation FAILED4/4 live battles;actual blocking occurred(1–10 samples),so activation improved but survival did not. Both explicit release and normal5HP attacks observed. No enablement justified. Videos decoded.
- Navigation cleanup did clear useKey after ShieldBlocker.tick. Guard added to retain an active shield;counter navigationReleasesPrevented. Post-guard interleaved ON/OFF/ON/OFF stillFAILED4/4,with2 prevented clears on each ON run. Startup FPS readiness added before this series;no cross-series timing claim.
- Important correction: melee shielding is NOT globally off. KillAura.tickEnd has its own shield implementation independent of combatShieldEnabled. doForceField skipped only the controller's selected target;other mobs still entered legacy aura and could write the same camera/hand/use keys. Thus per-target ownership did not imply one owner of the body.
- Pending ownership fix:deferral of the existing force-field call until after the current priority decision (only on evaluations that previously reached it);controller success marks this evaluation owned;all mob aura writers then yield,while fireballs remain allowed. mdAuraOwnerYields records these candidates. Unlike the old selected-target500ms suppression,this uses current evaluation rather than a stale timer. Build running;no fixed result yet.
- Foreground/aura ownership matrix FAILED4/4 (off/on/off/on). ON swing handovers0/4; both controller and aura-yield counters executed. Runs2/3 fell to2–3FPS; these are failure observations,not an effect-size claim. Handoff and ownership videos all decoded. Default shield flag remainsfalse; experimental combat changes are not committed.
- Direct shield primitive probe (no running bot chain,one angry Enderman,fixed camera) exposed an independent limit: unshielded controls died2/2; continuously-held shield outcomes were0/20/6 minimumHP across3 runs. Further server-stat probes died3/3 despite continuous client blocking; server damage_blocked_by_shield increased70/0/70 (stat units). Shield activation is real but client blocking alone does not imply protection from every hit. Investigating attack direction/position with fixed-facing versus tracking before changing combat policy again. Temporary probe artifacts:outputs/enderman-priority/shield-primitive-*,shield-server-*.
- Fixed-facing versus target-tracking primitive probes:fixed-facing died2/2;tracking minHP6/20,with server shield-stat deltas490/0. The zero-stat survivor did not validate repeated attack blocking. Position samples show Enderman moving behind the stationary guard;client/server update timing prevents attributing every hit from sampled angles. No robust survival improvement established. Reverted all five experimental Java files to1639845d;complete patch retained at workspace work/shield-combat-experiment.patch. Fixture instrumentation retained for future diagnosis. Next bounded pass:hostile visibility and premature flee completion.
- Occlusion baseline confirmed with a real NoAI zombie4blocks away across a bedrock wall:RunAwayFromHostilesTask(12,true).isFinished=true,visible_hostiles0,20HP. Probe executes tracker/goal reads on the client thread and explicitly dirties the tracker while the task runner is stopped;the first stale-cache probe was invalid and discarded.
- Implementing a separate nearby-hostile tracker view (same26-block bound,alive and hostility gates,without LOS). Existing getHostiles remains visible-only;only RunAwayFromHostilesTask uses the occlusion-independent view. This changes completion of an existing retreat,not the decision to initiate combat. Build and full fixture validation pending.
- First visibility fix passed6/6 corrected live retreats plus42predicate controls. First fixture run reached2292.6/cell2292 with20HP,but its exact2292.5 cutoff disagreed with FleeLive's documented feet-cell-center contract;fixture corrected,first run remains recordedfailed. Videos decoded/escape frame inspected.
- Pre-commit range audit reproduced a second premature completion:30-block retreat reportsfinished with an occluded zombie28blocks away,because the detection list clips at26. Tracker now exposes all loaded hostile candidates;the flee task filters at its requested distance+the goal's2-block destination margin. Visible getHostiles retains26 and LOS. Added28/30 and34/30 controls plus12/30-block live matrix. Rebuild pending.
- Final requested-radius matrix passed6/6 (three12-block and three30-block live retreats),54predicate controls. Short endpoints2292.6–2292.7;long2274.4;all20HP. Long retreats no longer stop at the previous26-block cutoff. Six videos decoded;30-block movement frame inspected. No newer upstream afterfetch.
- Natural world checkpoint saved with zero connected players:save-off,save-all flush,archive,save-on in finally. Archive verified and SHA256 recorded at workspace outputs/natural-checkpoints/20260915-163211-pre-resume.tar.gz (56055067bytes). This is the post-Enderman-death world;no restore or inventory change performed. Adjacent clearance/ambush/selection audit running.
- Adjacent one-high ascent passed2/2:three head cells planned/removed,groundedarrival,20HP;video decoded and mining frame inspected. Hungry mining ambush passed:6cobblestone,both targets gone,0pick-to-sword transitions during a continuous mining queue,final17HP. No claim of zero damage for this run.
- Selection audit first3passed;fourthcalm failed after initial actualEnderman distance23.5 despite metadata claiming5 (it only comparedX). Position later drifted44.8 away;handempty withswordinslot0. No flee task ran: residual movement began during setup beforeIdleTask. Original attribution to a combat decision was incorrect. Fixture now dispatches/resetAllState onclientthread afterasynchronousstop,selectsslot0,assertsfullstartposition+heldsword,records3Ddistances. Missing optional shield counters are JSONnull,notzero. Normalized angry/calm repetitions running. General stop/input leakage remains a separate open investigation;no core stop change here.
- Normalized selection passed6/6 (3angry,3calm),all20HP,stone_sword equipped;all videos decoded. First normalized attempt did not start:FPS readinessfailed. Direct limiter query showedLONG_AFK at10FPS;onInput onclientthread changedreasonNONE andFPS29. This restored the stand for the six selection runs. TungstenConfig.botFpsNoIdleThrottle exists but hasno reader;run_suite pins it anyway. Next focused pass implements this known missing bench capability before long natural observation. Flee correction and normalized diagnostics ready for localcommit;push/TG remain subject to previously reported target-specific auto-review blocks.

## 2026-09-15 — Standalone runs must enable the existing idle-FPS override

### Investigate
- Live limiter reportedLONG_AFK and10FPS withbotFpsNoIdleThrottle=false;onInput changedNONE and29FPS. run_suite already pins the flag,butwatch_survival and the standaloneEnderman fixture didnot.
- Initial search incorrectly covered onlyTungsten and trusted its staleJavadoc. A redundant newmixin was added during investigation,then removed after finding altoclef's existing preprocessedInactivityFpsLimiterMixin. javap ofthe built1.21.11 class confirmed the reader. No functionalJava change remains. Existing behavior raises lower idle,minimized andmenu limits toconfiguredFPS when explicitlyenabled;defaultfalse remains.

### Implement
- Survival observer pins the bench flag for the continuing bot. Enderman fixture pins/restores it aroundFPS readiness and testing. TungstenConfig documentation now names the actual live reader.
- idle_fps_test exercises the real client's limiter with controlled timer ages;all mutations occur onclientthread andrestore in finally. Window minimization is injected state,not anOSwindow-manager test. The menu boundary is tested byactualdisconnect.
