...
- [ ] **G-1.48 ТРИАЖ ВСЕХ 29 МЕТОК ПОРТА — «26 заглушек» превращаются в ОДИН пункт по делу (2026-08-07).**
      ...
      ⭐⭐ **НА КРИТИЧЕСКОМ ПУТИ «ПРОЙТИ ИГРУ» ОСТАЁТСЯ РОВНО ОДНА МЕТКА**:
      `ConstructNetherPortalSpeedrunTask` — `Direction.fromVector(int,int,int)` сменил сигнатуру.
      Портал в Нижний мир — следующая ступень после алмазов, и это единственная небоевая,
      неигрушечная заглушка, которая реально преграждает главную цель.
      ⛔ СЛЕДУЮЩИЙ ЗАХОД ПО G-1.38: (1) курс на портал (обсидиан + огниво в руки, гейт — портал
      зажжён), (2) только потом правка. Порядок тот же, что закрыл #37.
