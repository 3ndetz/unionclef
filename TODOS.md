...
- [ ] **C5.14 `bridgeForward` НЕ ПРОВЕРЕН НАД ГЛУБОКОЙ ПУСТОТОЙ, и падение туда УБИВАЕТ СТЕНД**
  ...
  Причина падения в сцене не установлена; ожидание приземления после телепорта добавлено, но
  проверить его не удалось — к тому моменту бот уже был в пустоте от предыдущего прогона.
  ...
  `tp` → `survival`.
  ⛔ ЧЕСТНО: САМУ ЗАЩИТУ ПРОВЕРИТЬ НА ЖИВОМ СЛУЧАЕ НЕ УДАЛОСЬ. Уронить бота в пустоту командой
