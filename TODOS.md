...
- [ ] **G-1.39 ⭐⭐⭐ БЛУЖДАНИЕ БЫЛО ПУСТЫШКОЙ — А В НЕГО УПИРАЮТСЯ ВСЕ ПУТИ ВОССТАНОВЛЕНИЯ (2026-08-07).**
      ...
      ⛔ ЧЕСТНО О ЗАМЕРЕ: craft-сюита на этой сборке ЗЕЛЁНАЯ, но `wander=0` во всех курсах — на
      плоской площадке теряться негде, ветка не срабатывает. Значит доказано «не сломал», а НЕ
      «работает». Для «работает» заведён курс `wander_recovery` (просьба дерева на поле БЕЗ
      деревьев, поле 45 блоков — иначе «блуждание» это шаг в пустоту), и гейт там — ПРОЙДЕННОЕ
      РАССТОЯНИЕ, а не счётчик: счётчик, тикающий при неподвижном теле, — это ровно старый баг
      с зелёной наклейкой.
