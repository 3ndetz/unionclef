...
- [ ] **G-1.32 ЦЕПОЧКА ЗАДАЧ КРУТИТСЯ НА «ДОСКАХ» — СНЯТО С ЖИВОГО ПРОГОНА (2026-08-06).**
      ...
      МЕТОД тот же, что сработал: поймать ПРОВАЛЬНЫЙ прогон и снять с него цепочку задач (стенд
      печатает её каждые 22 с — не фильтровать!), затем счётчик на тот компонент, который она
      назовёт. Известное про провал: `dcTick≈493` из ~6000 (задача добычи активна 8% времени),
