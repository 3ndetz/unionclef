...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      **НИ ОДНОЙ строки `PICKUPDEC` за прогон**. Значит `PickupDroppedItemTask.onTick` до неё не
      ...
      СЛЕДУЮЩИЙ ШАГ — ОДНА СТРОКА: залогировать для ОДНОГО предмета его координату и состояния
      блоков в собственной клетке и на 1-3 вниз. После неё гадать будет не о чем.
