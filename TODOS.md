...
- [ ] **G-0.1 SmartMoves НЕ УМЕЕТ ПЛАВАТЬ — и поэтому не может стать дефолтом (замер 2026-08-06).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД — МЕРИТЬ СЛЕДУЮЩУЮ СТУПЕНЬ КОНВЕЙЕРА, а не генератор: ходы РОЖДАЮТСЯ,
      вопрос в том, доживает ли водяное ребро до исполнения. Счётчик на то, сколько рёбер типа
      `MovementSwim` очередь построила и сколько из них отработало, плюс `qNoClass` — и смотреть,
      где ребро теряется: в поиске (не попало в путь), в очереди (нет класса) или в исполнении.
