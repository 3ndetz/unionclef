...
- [ ] **G-1.35 ВТОРАЯ СТУПЕНЬ: СТОРОЖ СЕТКИ СНОВА ЗАЦИКЛИЛСЯ — ТЕПЕРЬ НА КРОВАТИ (2026-08-07).**
      ...
      **предмет в курсоре не входит в `toMove.getMatches()`** — бот держит бревно, которое рецепт
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД, АДРЕС ТОЧНЫЙ: `StorageHelper.hasRecipeMaterialsOrTarget` —
      считает ли она БРЕВНО достаточным материалом для рецепта, которому нужны ДОСКИ (потому что
      доски из бревна craftable). Если да, `CraftInInventoryTask` решает «материалы есть» и идёт
      крафтить верстак, вместо того чтобы сперва сделать доски. Читать её вместе с
      `CollectRecipeCataloguedResourcesTask` и смотреть, кто из них должен был вставить шаг досок.
