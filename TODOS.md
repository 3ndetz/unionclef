...
- [ ] **G-1.42 ФЛЕЙК `craft_at_distant_table`: бот УХОДИТ ОТ ЦЕЛИ И ЗАМИРАЕТ (2026-08-07).**
      ...
      **КРАФТ ЧЕРЕЗ КНИГУ РЕЦЕПТОВ НА 1.21.11 ОТКЛЮЧЁН ЦЕЛИКОМ.** В `CraftInTableTask` ветка
      ...
      ✅ КАК ЧИНИТЬ, ОСНОВА УЖЕ ЕСТЬ: 1.21.11-API книги рецептов я уже осваивал в
      `CraftingRecipeTracker` (`player.getRecipeBook().getOrderedResults()`, `RecipeDisplayEntry`,
      `SlotDisplayContexts.createParameters`) — оттуда же берётся и выход рецепта, и его ключ.
