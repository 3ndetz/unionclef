...
- [~] 13. TUNGSTEN как DROP-IN замена baritone (проверить что реально работает):
  ...
  - [ ] 13.3b @gamer полный проход на tungsten-primary — nightly-масштаб + капризный
    world-setup (спавн/океан/локейт леса). Ядро (навигация+майнинг) РАБОТАЕТ
    (@get log PASS). Тест @gamer на gamer-server неинформативен: spreadplayers не
    поставил в лес, бот застревал на плохом спавне. Нужен надёжный land+trees мир
    (seed/структура) для полного прогона. Не блокер — ядро доказано
  - [ ] 13.3d РОБАСТНОСТЬ КРИВЫХ GOAL (юзер 2026-07-22, моя задача #25): цель В БЛОКЕ
    или в ВОЗДУХЕ над блоком (клик по траве → z=2/3, не z=1) — сейчас бот не пробует
    подойти. Надо: ближайшая достижимая цель + пока блоки не менялись и настоящая
    недостижима — показывать финальную (в блоке/воздухе), но бот подходит ближе ИЛИ
    ломает блок-цель (если в блоке = goal сломать). Смотреть baritone edge-cases
    (adjustOnPathStart/isInGoal). НЕ сломать текущее (перетест).
  - [ ] 13.3c ВОДНАЯ НАВИГАЦИЯ tungsten (эскалировано юзером 2026-07-22 — НЕ бэклог):
    у tungsten есть swim-физика (Agent.travelPlayer), но block-space выдаёт «Ran out
    of nodes» в воде (не генерит swim-ноды) → бот утонул в океане. Сделать: swim-
    child в воде (плыть/всплывать) + БЕЗОПАСНОСТЬ swim-up при падении ХП под водой
    (не тонуть, выплывать из любой ситуации автономно). Тест: спавн в воде → выплыл.
    (моя задача #24). Очень сложный рельеф block-space Movements — #20/#21 бэклог
...
- [ ] 19. (ПОСЛЕДНЯЯ, после того как ВСЁ сделано и протестировано) разбор PR/issues:
