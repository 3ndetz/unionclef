...
- [ ] 2. PVP: полный аудит и переделка комбата tungsten (smart + fast + effective)
  ...
  - [ ] 2.2 Переделка по результатам аудита: агрессивность, точность, скорость решений
  - [ ] 2.3 Боевой тест на стенде (второй клиент-жертва, метрики: время до первого удара, DPS, отсутствие зависаний, трава на арене)
...
- [ ] 3. Tungsten block break/place: научить ломать (и в идеале ставить) блоки
  - [ ] 3.1 Block-space поиск с учётом ломания (свой BFS/A* с cost'ами ломания; вариант — глубже срастить с baritone: BFS блоков у baritone, физика переходов у tungsten)
  - [ ] 3.2 Физическое исполнение: BreakBlockMove (навестись, держать атаку, дождаться разрушения, продолжить путь) — executor должен уметь паузить replay
  - [ ] 3.3 Физика гравитационных блоков: песок/гравий падает после ломания опоры — учитывать в плане
  - [ ] 3.4 (далёкое будущее) редстоун/поршни в модели мира
  - [ ] 3.5 Автотест-курсы: стенка (сломать 1-2 блока), песок над проходом
