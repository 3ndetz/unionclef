...
- [ ] **C5.15 ПОГОНЯ: СТЕНД МЕРЯЛ НЕ ТО — ТЕПЕРЬ МЕРЯЕТ, И ВИДНО 15 ЗАМИРАНИЙ**
  ...
  время заморозки перепланирование не вызывалось НИ РАЗУ — то есть внутри `FollowEntityTask.tick()`
  не выполняется ничего, то есть ЕГО САМОГО НЕ ТИКАЮТ. Это тот же класс, что и гейт тика
  исполнителя выше (`isRunning()` исключал взведённый путь).
  Следующий заход: кто вызывает `FollowEntityTask.tick` и при каком условии — читать вызывающего,
  а не гадать (обе гадалки уже стоили по заходу).
