...
- [ ] **G-1.44 ⭐⭐⭐ ОБЩИЙ КОРЕНЬ ТРЁХ ЗАВИСАНИЙ: `Nav.isPathing()` ГЛУШИТ ВСЕ ПРОГРЕСС-ЧЕКЕРЫ (2026-08-07).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: НЕ ПРАВИТЬ, А ЗАМЕРИТЬ `failCounter` и срабатывания чекера напрямую
      (счётчик, как везде сегодня). Три правки подряд я сделал по рассуждению — и все три дали
      ровно 2/4. Дальше только по показаниям прибора: сколько раз чекер СОРВАЛСЯ и до какого
      значения дошёл `failCounter` на красном прогоне.
