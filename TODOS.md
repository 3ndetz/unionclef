...
- [ ] **G-0.1 SmartMoves НЕ УМЕЕТ ПЛАВАТЬ — и поэтому не может стать дефолтом (замер 2026-08-06).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД — В ФИЗИЧЕСКИЙ ПОИСК (`PathFinder`), а не в SmartMoves: почему он
      исчерпывает открытый список на воде и почему на этом курсе спрашивают именно его. Заодно
      проверить, есть ли у него мягкая деградация (у блок-спейсного их две — «частичный путь» и
      «ближайшая достигнутая клетка», и обе в логе НЕ появлялись, потому что он и не падал).
