...
- [ ] **G-1.41 ⭐⭐⭐ #37 ВОСПРОИЗВЕДЁН НА ДЕШЁВОМ МИРЕ ЗА 2.5 МИНУТЫ (2026-08-07).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: читать связку `AbstractDoToClosestObjectTask` / `MineAndCollectTask` /
      `DestroyBlockTask` — где именно «прибыл» отделено (или НЕ отделено) от «могу достать», и
      почему нулевой маршрут считается прогрессом. Курс `chop_canopy` — гейт этой работы.
