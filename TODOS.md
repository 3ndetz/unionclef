...
- [~] **C5.12 `//replace` ТЕРЯЛ КЛЕТКИ + СТОРОЖЕВОЙ СЧЁТЧИК СЛОМА ПЕРЕЖИВАЛ ЗАДАНИЕ**
  ...
  Замеры: `diag_replace` 1/3 → 2/4, конвертация впервые доходит до 3 клеток из 3. nav 12/12
  (включая `nav_break`, который этот путь и использует). ОСТАЁТСЯ: прогоны чередуются
  FAIL/PASS/FAIL/PASS — это НЕ случайность, а состояние, переносимое между прогонами; причина
  пока не найдена. Курс красный.
