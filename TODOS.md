...
<!-- LADDER-CEILING -->
## ⭐⭐⭐ ПОТОЛОК ЛЕСТНИЦЫ — НЕ ЧАСЫ И НЕ ЗАСТОЙ. ЭТО ЗАКРЫТАЯ ЛИНИЯ ВЗГЛЯДА (2026-08-22)

Времени хватает. Ступени ложатся так (n=10..28 на ступень): первый крафт 91с, деревянные
инструменты 154с, крафт 166с, каменные инструменты 201с — при окне ~350с. После каменных
остаётся ~150 секунд, то есть БОЛЬШЕ, чем ушло на весь путь до них. И один раз проход дошёл до
ЖЕЛЕЗНЫХ инструментов на 338с, у самого края окна.

⛔ И ЗАСТОЙ ТОЖЕ НЕ ОН. Дважды за сессию убранный застой не конвертировался в ступени:
stallCheckNeedsMovement снял 73.6с неподвижности за прогон (t=-2.16, установлено) при нуле по
ступеням; дедупликация маршрута сняла детерминированное застревание при +0.20 ступени (t=0.13).
Значит ограничитель лежит НИЖЕ по течению, чем стояние на месте.

⭐ ЧТО ЕГО ДЕРЖИТ, ИЗ ПЯТНАДЦАТИМИНУТНОГО ПРОГОНА:

    dbTargetAir=0     целевой блок СУЩЕСТВУЕТ — это не призрак и не устаревшая цель
    rayOther=5012     луч прицела упирается в ПОСТОРОННИЙ блок
    rayMiss=4128      луч не попадает никуда
    rayLeaves=1923    луч упирается в ЛИСТВУ

Бот ломает блок, которого не видит: линия взгляда перекрыта. Задача ломания при этом тикает
4076 раз за 181 секунду одного эпизода. Логика расчистки листвы существует (leafCleared тикает),
но, судя по числам, не успевает или бьёт не туда.

СЛЕДУЮЩИЙ ЗАХОД НАЧИНАТЬ ОТСЮДА, а не с очередной правки движения. И мерить надо тем же
минутным репро (deploy/runner/repro_stall.py), а не шестиминутным прогоном.
