...
- [ ] **C5.15 ПОГОНЯ: СТЕНД МЕРЯЛ НЕ ТО — ТЕПЕРЬ МЕРЯЕТ, И ВИДНО 15 ЗАМИРАНИЙ**
  ...
  Следующий заход: читать тело воспроизведения `PathExecutor.tick()` целиком (участок после
  ветки `armed` до `this.tick++`) и найти ветку, которая возвращается, не двигая `tick`. НЕ
  правки — чтение, как требует чеклист после двух заходов без сдвига числа.
