...
- [~] **C5.12 `//replace` ТЕРЯЛ КЛЕТКИ + СТОРОЖЕВОЙ СЧЁТЧИК СЛОМА ПЕРЕЖИВАЛ ЗАДАНИЕ**
  ...
  Итог по сессии после фикса «подходить»: **21 из 25** (8/10, 4/4, 4/6, 5/5).
...
- [~] **C5.9** `BridgeTask` has no re-equip / no fallback when the stack empties mid-bridge. Build
  ...
  ⛔ ПРОВЕРИТЬ НЕ УДАЛОСЬ, И ПО ДОРОГЕ НАШЛОСЬ ХУДШЕЕ — см. C5.13.
...
- [ ] **C5.13 `bridgeForward` ПАДАЕТ С КРАЯ, поставив НОЛЬ блоков** (найдено 2026-07-31 при
  ...
  ⛔ ОСТАЛОСЬ И НЕ РЕШЕНО: бот всё равно доходит до x=1.3 и `velocity.y < -0.5` → «падение», 0
  клеток, 3 прогона из 3. x=1.3 — это РОВНО предел приседа, описанный в самом файле («центр =
  край + 0.3»), то есть присед держит, и всё же фиксируется падение.
  ⛔ ГИПОТЕЗА «это артефакт телепорта, бот не успел приземлиться» — ОПРОВЕРГНУТА ЗАМЕРОМ: тест
  теперь ждёт `onGround=true` и печатает `settled: {'g': True, 'pos': '0.5,-60.0,0.5'}`, а мост
  всё равно пишет `aborted (falling) after 0`. Следующий заход: снять velocity и позицию ПОТИКОВО
  внутри `BridgeTask` (в файле уже есть verbose-диагностика такого рода у ported-movements) —
  и только потом трогать код.
