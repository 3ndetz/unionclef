...
ВЫВОД: стенд жив и отвечает, `gotoXYZ`/`pathStatus`/`stopPathing` работают через MCP-обход;
бот больше не заморожен. Не разбудил ничего, что не было уже разрешено (`docs/CHECKLIST.md`
