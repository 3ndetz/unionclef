...
- [~] **C5.22 МЁРТВЫЕ ДУБЛИ И `Thread.sleep` В ПОИСКЕ (тот же аудит, 2026-08-02).**
  ...
  * `CornerJump.java:74,89` инкрементит `limit` дважды за итерацию — бюджет 40 тиков вдвое
  ...
  ⛔ ПЕРВЫЙ ПУНКТ УДАЛЁН 2026-09-01, В ЭТОЙ ЖЕ СЕССИИ, СРАЗУ ПОСЛЕ ПОВТОРНОГО ПОДТВЕРЖДЕНИЯ.
  `BlockStateChecker.fullyPassableBlockState` и `MovementHelper.canWalkOnBlockState` (со всеми
  ставшими ненужными импортами — `Ternary`/`YES`/`NO` и восемь неиспользуемых типов блоков на
  двоих) вырезаны целиком; ноль вызывающих было подтверждено grep'ом ПЕРЕД удалением, а не
  предположено. Остальные три пункта (Thread.sleep в DivingMove, потерянная ветка WalkToNode,
  двойной инкремент CornerJump) — НЕ трогала: это или сознательные компромиссы производительности
  ...
  удалению код. Пункт остаётся `[~]`, не `[x]`: один из четырёх фактов исправлен, три — нет.
