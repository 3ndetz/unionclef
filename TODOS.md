...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      То есть до удара дело доходит, но цель оказывается ВОЗДУХОМ. Ни одного булыжника при этом.
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: почему цель добычи — воздух, когда бот стоит НА камне. Гипотеза, которую
      надо проверить ПЕРВОЙ (и не верить ей на слово): арена строится заливкой в начале прогона,
      а `BlockScanner` мог закэшировать состояние ДО заливки — тогда он выдаёт позиции, где камня
      уже/ещё нет. Проверяется тем же способом: залогировать выбранную позицию и то, что в ней
      реально лежит, в момент выбора.
