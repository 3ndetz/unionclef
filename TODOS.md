...
- [ ] **G-1.35 ВТОРАЯ СТУПЕНЬ: СТОРОЖ СЕТКИ СНОВА ЗАЦИКЛИЛСЯ — ТЕПЕРЬ НА КРОВАТИ (2026-08-07).**
      ...
      ⛔ ПРАВКА (следующий заход): «забыто» должно значить «ни одна активная задача крафта этого не
      использует», а не «мой рецепт этого не использует». Смотреть на цепочку задач, а не на один
      `_target`; либо чистить только когда крафт вообще не идёт.
