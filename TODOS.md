...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      **НИ ОДНОЙ строки `PICKUPDEC` за прогон**. Значит `PickupDroppedItemTask.onTick` до неё не
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: почему `itemDropLocations` пуст. Два кандидата, оба проверять счётчиком,
      а не рассуждением: (а) `updateState` трекера вообще не проходит по сущностям (`ensureUpdated`
      / dirty-логика), (б) все дропы отсеивает `entityBlacklist.unreachable`. Счётчик на «сколько
      ItemEntity увидел обход» и «сколько прошло условие grounded» ответит за один прогон.
