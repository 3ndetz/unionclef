...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      **-60.0 во всех 36 сэмплах**, ни разу не изменилась. Никакого падения нет, и 48 «в воздухе» —
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД, ВОПРОС УЗКИЙ: доходит ли дело до РЕАЛЬНОГО удара и ломается ли блок.
      Проверять снаружи, а не по счётчикам: через rcon посмотреть, лежат ли в арене выпавшие
      предметы (`/execute if entity @e[type=item]`) и не появились ли в полу дырки. Если блоки
      ломаются, а булыжника в рюкзаке нет — вопрос в подборе; если не ломаются — в самом ударе.
