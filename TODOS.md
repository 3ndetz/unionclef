...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      **НИ ОДНОЙ строки `PICKUPDEC` за прогон**. Значит `PickupDroppedItemTask.onTick` до неё не
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: читать `EntityTracker.itemDropped` / `getClosestItemDrop` — почему предмет
      не проходит. Кандидаты (проверять, а не верить): сопоставление `ItemTarget` с типом предмета,
      ограничение по дистанции, обновление кэша трекера.
