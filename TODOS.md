...
- [ ] **G-1.35 ВТОРАЯ СТУПЕНЬ: СТОРОЖ СЕТКИ СНОВА ЗАЦИКЛИЛСЯ — ТЕПЕРЬ НА КРОВАТИ (2026-08-07).**
      ...
      **НИ ОДНОЙ строки `MOVEMISMATCH` за прогон**. Значит `wrongItemHeld` НИКОГДА не срабатывает,
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: найти, КТО запускает `EnsureFreePlayerCraftingGridTask` в этот момент
      (`ciGrid` = 0, значит НЕ ветка «забытых ингредиентов» в `CraftInInventoryTask` — искать другие
      места, где эта задача возвращается) и убрать причину, а не следствие. Это ровно тот же
      сюжет «две половины одной задачи отменяют друг друга», третий раз за сутки.
