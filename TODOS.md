...
- [ ] **G-1.32 ЦЕПОЧКА ЗАДАЧ КРУТИТСЯ НА «ДОСКАХ» — СНЯТО С ЖИВОГО ПРОГОНА (2026-08-06).**
      ...
      ⛔ И ВОТ ЧТО ОСТАЛОСЬ, УЖЕ УЗКО: после этого в чате доминирует САМ ДРАЙВЕР —
      `primDrive gridBFS sz1` сотнями и `primDrive robustPath presenttrue sz2`. То есть поиск
      возвращает маршрут ИЗ ОДНОЙ КЛЕТКИ (только старт) снова и снова: BFS не может расшириться из
      клетки, где стоит бот. Следующий заход — читать `CombatPathfinder.getWalkableNeighbors` /
      `isWalkable` / `canPassThrough` против того, на чём бот реально стоит в тёмном лесу (высокая
      трава, листва, ковёр из папоротника) и почему ни один сосед не проходит проверку.
