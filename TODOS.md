...
- [ ] 2.7 PVP: доводка по реальному использованию (фидбек юзера 2026-07-20 — «работает ужасно»):
  ...
  - [ ] 2.7.1 BEDWARS: ПАДАЕТ В VOID + 0 КИЛЛОВ (юзер 2026-07-22, тест релиза 0.26 —
    моя задача #28): бот сам падает с краёв острова в бездну, не берёт даже 1 килл.
    Нужно: void/edge-безопасность (VoidDetector в tungsten/combat не работает —
    переделать: не падать сам, не преследовать в пустоту, учитывать нокбэк к краю)
    + эффективность (киллы). Тест: bedwars-арена (остров+void+жертва) → ≥1 килл БЕЗ
    падений. Хоть полностью переделать combat. КРИТЕРИЙ: не падает + успешен на BW
