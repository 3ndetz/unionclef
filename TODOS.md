...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД, И ЭТО ТЕПЕРЬ ГЛАВНОЕ: почему `@get cobblestone` не даёт ни одного блока,
      стоя НА камне. Метод отработан: счётчики `db*` (DestroyBlockTask) и трасса — сначала выяснить,
      доходит ли дело до удара по блоку, и если да — что с подбором. Курс воспроизводит за 120 с и
      работает при любой чужой нагрузке.
