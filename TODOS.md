...
- [ ] **C5.15 ПОГОНЯ: СТЕНД МЕРЯЛ НЕ ТО — ТЕПЕРЬ МЕРЯЕТ, И ВИДНО 15 ЗАМИРАНИЙ**
  ...
  Следующий заход: читать участок от `tickActive++` до гейта на ранние `return` и снять значения
  этих двух флагов. УРОК: «счётчик не растёт» само по себе НЕ значит «код не вызывается» — надо
  различать «не вызывается» и «вызывается, но не доходит», и это стоило одного захода.
