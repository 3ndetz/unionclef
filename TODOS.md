...
- [ ] **C5.11 ВЕРТИКАЛЬНЫЙ УЧАСТОК НУЖНО СТРОИТЬ ПРЫЖКОМ, А НЕ ХОДЬБОЙ** (найдено 2026-07-31,
  ...
  * цель `GoalPlace` = `target.up()` отдаётся навигатору, только если там реально можно стоять;
  ...
  ОСТАЁТСЯ (следующий заход, это дефект НАВИГАТОРА, а не строителя): бот не доходит до
  `(6,-60,0)` с `(4,-60,0)` — ДВА БЛОКА по прямой, ходка за ходкой заканчивается ничем.
  `diag_build` из-за этого флаки => КРАСНЫЙ.
