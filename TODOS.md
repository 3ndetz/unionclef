...
- [ ] **C5.15 ПОГОНЯ: СТЕНД МЕРЯЛ НЕ ТО — ТЕПЕРЬ МЕРЯЕТ, И ВИДНО 15 ЗАМИРАНИЙ**
  ...
  * `tick=78/65` у исполнителя — маршруты доезжают и проигрываются.
  ...
  СЛЕДУЮЩАЯ ЕДИНИЦА РАБОТЫ (не заплатка): почему BFS-режим `BlockPathWalker` не продвигается по
  своим путевым точкам на сгенерированном рельефе. Читать ветку BFS в `tick()`, а не трогать
  пороги.
