...
- [~] **C5.12 `//replace` ТЕРЯЛ КЛЕТКИ + СТОРОЖЕВОЙ СЧЁТЧИК СЛОМА ПЕРЕЖИВАЛ ЗАДАНИЕ**
  ...
  ⛔ ОСТАЁТСЯ И НЕ РАЗГАДАНО: прогоны чередуются PASS/FAIL/PASS/FAIL/PASS — идеально, а значит
  ДЕТЕРМИНИРОВАННО. Три гипотезы уже ОПРОВЕРГНУТЫ замером (стоит на ломаемом блоке; потеря
  клеток; невзведённый исполнитель) — каждая была настоящим дефектом и каждая исправлена, но
  чередование пережило все три. Следующий замер (НЕ правка): снять КЛИЕНТСКИЙ вид трёх клеток в
  момент `replaceSelection` и через 3 с — подозрение на то, что клиентская копия мира отстаёт от
  rcon-заливки арены, и чередование идёт от неё, а не от мода. Курс красный.
