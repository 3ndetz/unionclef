...
- [ ] **G-1.44 ⭐⭐⭐ ОБЩИЙ КОРЕНЬ ТРЁХ ЗАВИСАНИЙ: `Nav.isPathing()` ГЛУШИТ ВСЕ ПРОГРЕСС-ЧЕКЕРЫ (2026-08-07).**
      ...
      ⛔ ЧТО ПРОВЕРИТЬ ПЕРЕД ПРАВКОЙ (одно чтение, не догадка): на строках ~258-267 флаг
      `_isAvoidingBlockBreak` СБРАСЫВАЕТСЯ по таймеру, но снимается ли при этом сам ПРЕДИКАТ из
      `BotBehaviour`? Если предикат остаётся (или добавляется повторно), запрет вечен — что и
      наблюдаем: 18456 отказов подряд до конца прогона.
