...
<!-- TRIGGERBOT-CENTER-LOS-VS-BESTAIMPOINT-2026-09-02 -->
## ⭐⭐ НАЙДЕНО ЧТЕНИЕМ, ПРОСЛЕЖЕНО ДО КОНЦА: TRIGGERBOT НЕ ЗНАЕТ, КУДА РЕАЛЬНО ЦЕЛИТСЯ БОТ (2026-09-02)

Найдено НЕ живым прогоном, а перепроверкой `docs/ai/audit-2026-07-24-pvp.md` (root cause 2,
«aimer-vs-trigger disagreement») — той же методикой, что весь этот заход применял к
`docs/BARITONE-PORT.md`. Первая находка старого аудита частично почищена (гейт по углу ослаблен
с ±12° до 40°, `TriggerBot.MAX_LOOK_ANGLE_DEG`), но ГЕОМЕТРИЯ несоответствия — жива, прослежена
ДО КОНЦА цепочки вызовов, а не предположена:

1. `SafetySystem.computeAimPrediction` (`:631-645`) зовёт `findBestAimPoint`, который специально
   пробует точки на голове/ногах/боках/углах хитбокса, ПОТОМУ ЧТО ближайшая-к-центру точка не
   имеет чистой видимости (`:716-750`) — то есть цель СПЕЦИАЛЬНО целится НЕ в центр, когда центр
   закрыт препятствием.
2. Этот результат становится `aimYaw`/`aimPitch` (`:657-661`), читается `CombatController` через
   `getAimYaw()`/`getAimPitch()` (`SafetySystem.java:905-906`) и передаётся прямо в
   `WindMouseRotation.INSTANCE.setTarget(...)` (`CombatController.java:394,419`) — ЭТО и есть
   реальный, живой поворот, к которому прицел бота сходится.
3. `TriggerBot.tick(player, target)` вызывается ТОЛЬКО с сырой сущностью-целью
   (`CombatController.java:438`) — в `TriggerBot.java` НЕТ НИ ОДНОЙ ссылки на `SafetySystem`, и
   его собственная проверка видимости (`LOS`-гейт) рейкастит НЕЗАВИСИМО прямо в
   `box.getCenter()` (`TriggerBot.java:205-207`).

ИТОГ: когда препятствие вынуждает целиться в видимую конечность (центр закрыт), прицел
ДЕЙСТВИТЕЛЬНО туда сходится (реальный, проверенный поворот), а собственный LOS-гейт `TriggerBot`
по-прежнему рейкастит в центр и получает отказ — курок не срабатывает по цели, в которую бот
УЖЕ смотрит и МОГ БЫ попасть. Это ровно симптом «stare, doesn't click» из старого аудита
2026-07-24, воспроизведённый ПОЛНОСТЬЮ ПЕРЕДЕЛАННОЙ системой прицеливания на том же самом шве.

НЕ ПОЧИНЕНО ЭТОЙ СЕССИЕЙ: правильный фикс (`TriggerBot` должен принимать ТУ ЖЕ точку прицеливания,
что реально использовал `SafetySystem`, вместо пересчёта своей) — поведенческое изменение
широко используемого боевого гейта, нужен стенд-прогон (nav/pvp регрессия), а не слепой коммит.
⛔ УТОЧНЕНИЕ ПРИЧИНЫ 2026-09-02 — предыдущая формулировка была НЕТОЧНОЙ. `TriggerBot.REACH` и
производные от неё дистанции (`STRIKE_DISTANCE`, `MOB_STRIKE_DISTANCE`, `CombatController.java:
29-69`) считаются от `distSq`/`closest` (ближайшая точка хитбокса) — это ОТДЕЛЬНАЯ, независимая
от LOS-точки величина, проверено чтением (`gateReach` не трогает `box.getCenter()` вообще).
НАСТОЯЩАЯ причина не чинить вслепую: `gateAngle` (`:198-201,212`) и `gateLos` (`:205-207`) ОБА
сейчас считаются от одной и той же точки — центра хитбокса — независимо друг от друга. Починить
ТОЛЬКО LOS (подставить точку `SafetySystem`) и оставить угол по центру — значит развести две
проверки по РАЗНЫМ точкам, что может СОЗДАТЬ новое рассогласование (например: реальная точка
прицеливания — видимая нога, угол к ней в допуске, а угол к ЦЕНТРУ — уже нет, если нога сильно
смещена). Правильный фикс — перевести ОБЕ проверки на одну и ту же, реально используемую точку
прицеливания, а не только LOS-часть. Это переделка геометрии гейта, а не однострочная правка,
и нужен стенд, чтобы отличить исправление от новой формы того же бага.

СЛЕДУЮЩИЙ ШАГ: когда появится стенд — воспроизвести живьём (частичное укрытие, например бой
через щель забора или угол стены, где центр хитбокса закрыт, а конечность видна), подтвердить
`hit.getType() == MISS` в `TriggerBot` при реально сходящемся прицеле, затем передать
`SafetySystem`'ную точку прицеливания в `TriggerBot.tick` явным параметром вместо пересчёта.

<!-- PLACERULES-WORLD-BORDER-FIXED-2026-09-02 -->
## ⭐ НАЙДЕНО ПРИ ПЕРЕПРОВЕРКЕ АУДИТА И ПОЧИНЕНО: РАЗМЕЩЕНИЕ НЕ ЗНАЛО ПРО ГРАНИЦУ МИРА (2026-09-02)

Найдено НЕ живым прогоном, а третьей сверкой `docs/BARITONE-PORT.md` за эту сессию — тем же
методом, что уже дважды в этой сессии находил ложное «MISSING»: проверка ОДНОЙ функции
(`BreakRules.canBreak`, голый `grep` по `FastPlanner.java`) вместо чтения на уровень глубже
(`MovementHelperB`, гораздо более полный порт baritone, чем казалось раньше). На этот раз —
находка НЕ ложная, а РЕАЛЬНАЯ асимметрия: `MovementHelperB.avoidBreaking` (портированный,
вызывается из `getMiningDurationTicks`, вызывается из `FastPlanner.breakThrough`) уже сверяет
`worldBorderCanPlaceAt` (порт `BetterWorldBorder.canPlaceAt`) — ломка у границы мира УЖЕ
запрещена. Размещение (`PlaceRules.canPlace`) — прочитано целиком, никакой проверки границы не
было вообще.

ПОЧИНЕНО (`1cfbe21f`): `worldBorderCanPlaceAt` сделан `public` (был `private`, пакетный), вызван
из `PlaceRules.canPlace` рядом с существующей проверкой `isReplaceable` — переиспользует готовую
реализацию, а не копирует прямоугольную математику границы второй раз (правило «один источник
правды, без дублей»).

СТРОГО КОНСЕРВАТИВНО: правка может только ЗАПРЕТИТЬ размещение за границей мира, никогда не
разрешить новое — ни один текущий курс (`nav_*`, все у спавна) не должен увидеть изменения
поведения от этой правки. ПОЧИНЕНО, НЕ ПОДТВЕРЖДЕНО СТЕНДОМ — та же причина, что у прошлых
находок этой сессии (C8.1: нет RCON/`docker exec`/`gradlew` в этой комнате прямо сейчас).

⛔ ВСЁ ЕЩЁ ОТКРЫТО, БЕЗ ИЗМЕНЕНИЙ: обычный кардинальный шаг ходьбы в `step()` границу мира вообще
не проверяет (ни `avoidBreaking`, ни `PlaceRules` не на этом пути) — ходьба у края границы
по-прежнему без защиты. Границы по высоте мира (`getBottomY`/`getHeight`) отсутствуют полностью,
нигде — проверено напрямую, не по догадке.

<!-- STAND-FOUND-STUCK-LIVE-2026-09-02 -->
## ⚠️ СТЕНД НАЙДЕН ЖИВЬЁМ В ТОЙ ЖЕ ЗАСТРЯВШЕЙ ПОЗИЦИИ ~36 ЧАСОВ (2026-09-02)

Тем же обходным путём (`http://host.docker.internal:25350/mcp`, C8.1), НЕ для нового поиска, а
проверить состояние стенда перед следующим фокус-заходом. `getGameState()` показал бота на
`27.0,-60.0,0.4` — ТОЙ ЖЕ позиции с точностью до сотых, где застрял `gotoXYZ(28,-60,0)` в
записи выше (2026-09-01). `pathStatus()` трижды подряд (~6с) вернул неизменный `pos` и
`busy:true` без полей `distance`/`arrived` вовсе (форма ответа MCP-обёртки отличается от
записанной выше — возможно, обновлялась независимо от джава-кода; не то же самое, что
проверка самого фикса).

СЧЁТЧИК, А НЕ ДОГАДКА: лог (`docker logs uctest-mc-tester1`, тот же обходной путь) печатает
`WALKMODE off=N bfs=91 direct=0` раз в 200 тиков, где `off` — это `BlockPathWalker.tickOff`
(`BlockPathWalker.java:225`, инкремент раз в тик пока `!active`). За окно наблюдения (~60с)
`off` вырос примерно на 1200 — 20 тиков/с, подтверждает счётчик живой. Абсолютное значение
`off≈2,579,000` ⇒ ходок простаивал ПРИМЕРНО 36 часов (2,579,000 / 20 / 3600). Точное совпадение
позиции с местом старой находки — сильное, но косвенное свидетельство, что бот простоял именно
с исходного репро; не утверждаю это как доказанный факт, только как наиболее вероятное
объяснение.

РАСТОРМОШЕНО, НЕ ПОЧИНЕНО: `stopPathing()` + `gotoXYZ(30,-60,0)` — бот сдвинулся и
`pathStatus` отработал нормально (`arrived:true, distance:"0.7"`). Это не тест фикса
`ARRIVE_DIST` (0.7 и 1.3 блока — оба прохода дистанции, которую ловил ещё СТАРЫЙ порог `<1.5`,
не только новый `<=2.0`) — специально попасть в промежуток 1.5–2.0 по требованию не вышло за разумное число попыток, и
дальше пробовать не стал: живой стенд общий, а вопрос уже решён чтением кода (`ARRIVE_DIST`
публичный, `pathStatus` его читает — коммит `099b461e`, запись выше). Вопрос "задеплоен ли этот
коммит на живой стенд" остаётся ОТКРЫТ — не решён этим экспериментом ни в одну сторону.

ДОПОЛНИТЕЛЬНО: `;settings` (через `ChatMessage`, параметр `msg`, НЕ `message` — первая попытка
упала с NPE в MCP-обёртке) снял полный дамп `TungstenConfig` живьём. Ничего не разъехалось с
ожидаемыми дефолтами: `allowBreak=true`, `planPlaceMoves=true`, `slimeCrossing=false`
(совпадает с комментарием "ships OFF" в `FastPlanner.java`), `allowPlace=true`,
`arrivalAgreesWithTheSnap=false` (уже отслеживаемое, отдельное несоответствие «прибытия», см.
запись выше). Дрейф конфига НЕ объясняет находку выше — стенд застревал не из-за
рассинхронизированных настроек.

ВЫВОД: стенд жив и отвечает, `gotoXYZ`/`pathStatus`/`stopPathing`/`ChatMessage` работают через
MCP-обход; бот больше не заморожен. Не разбудил ничего, что не было уже разрешено (`docs/CHECKLIST.md`
позволяет живое чтение узкими проверками; `stopPathing`/`gotoXYZ` — существующие, а не новые
рычаги). СЛЕДУЮЩИЙ ШАГ по этой линии: когда появится `gradlew`/`deploy_jar.sh` доступ —
пересобрать, передеплоить, и ТОГДА замерить `pathStatus` в промежутке 1.5–2.0 прицельно.

<!-- LADDER-BOGUS-CLIMB-INTO-AIR-FIXED-2026-09-02 -->
## ⭐⭐⭐ ПОЧИНЕНО (НЕ ПОДТВЕРЖДЕНО СТЕНДОМ): ЛЕСТНИЦА ПЛАНИРОВАЛА ШАГ В ПУСТОТУ (2026-09-02)

Найдено НЕ живым прогоном, а сверкой `docs/BARITONE-PORT.md` (раздел special-terrain-rules,
раскрытый впервые в этой сессии) против текущего кода — тот же метод, что закрыл предыдущие три
раздела этого аудита. `FastPlanner.special()`, ветка лестниц (`FastPlanner.java:641-654`):
продолжение подъёма/спуска по колонне лестницы предлагалось, если целевая клетка ПРОСТО
вмещает тело (`PlayerFit.bodyFits`), без проверки, что там реально ступенька или пол — то есть
шаг в открытый воздух над верхней ступенькой или под нижней, дословно находка аудита. Такой
узел мог стать `best`-узлом, чья цепочка возвращается как частичный план — точка в воздухе.

ПОЧИНЕНО (`a359fd38`): продолжение по колонне теперь требует, чтобы следующая клетка сама была
лестницей; приземление СНИЗУ (`dy=-1`) дополнительно принимает реальный пол
(`PlayerFit.supportTop` не NaN). Выход СВЕРХУ уже чинился отдельно (боковой cardinal-выход чуть
ниже в том же файле, с собственной проверкой `supportTop`) — не трогал.

⛔ ПОЙМАНА ВТОРАЯ, СВОЯ ЖЕ ОШИБКА ДО КОММИТА: `cachedState` (внутри `isLadder`) трогает общий
`scratch` (`BlockPos.Mutable`) ТОЛЬКО при промахе кэша — при попадании оставляет его там, где
его оставил предыдущий, несвязанный вызов. Первый черновик звал `PlayerFit.supportTop(world,
scratch)` сразу после `isLadder()` без явного `scratch.set(...)`, что при попадании в кэш читало
бы НЕ ту клетку. Исправлено явным `scratch.set` перед вызовом — тем же паттерном, что уже
использует соседний cardinal-выход парой строк ниже.

ПОЧИНЕНО, НЕ РАЗВЁРНУТО И НЕ ПЕРЕИЗМЕРЕНО — та же причина, что у фикса `pathStatus` выше по
файлу (C8.1: `docker exec` под запретом прокси, RCON недостижим, `gradlew`/`deploy_jar.sh` не
запускались в этой комнате). Правка прочитана и логически обоснована, но НЕ ПОДТВЕРЖДЕНА
стендом — не называю сделанным, называю починенным-но-непроверенным (Hard Ban чеклиста).

СЛЕДУЮЩИЙ ШАГ: когда стенд станет доступен — `nav_ladder` (или ближайший курс с лестницей) на
парном замере, смотреть, пропал ли waypoint-в-воздухе/партиальный план на лестничных курсах.

<!-- PATHSTATUS-ARRIVED-THRESHOLD-MISMATCH-FIXED-2026-09-01 -->
## ⭐⭐⭐ НАЙДЕНО ЖИВЫМ ПРОГОНОМ И ПОЧИНЕНО: `pathStatus` ЛГАЛ ПРО `arrived` (2026-09-01)

Найдено не чтением кода, а РЕАЛЬНЫМ вызовом через обходной сетевой путь к стенду (см. C8.1 —
`docker.sock` по-прежнему мёртв, но MCP-порт `uctest-mc-tester1` достижим напрямую по
`http://host.docker.internal:25350/mcp`). Первый живой замер за всю сессию.

ВОСПРОИЗВЕДЕНО: `gotoXYZ(28,-60,0)` с позиции `(23.7,-60.0,0.7)` на плоской арене. Бот дошёл до
`(27.0,-60.0,0.4)` и ВСТАЛ НАВСЕГДА — `pathStatus()` держал `arrived:false, distance:"1.5",
busy:true` НЕИЗМЕННО (та же позиция до сотых) через 4 независимых опроса за ~15с, ни
`stopPathing()`+повторный `gotoXYZ` не сдвинули ни на сантиметр. `getGameState()` (другой,
независимый метод) подтвердил ту же позицию — не артефакт кэша одного метода.

ПРИЧИНА НАЙДЕНА ЧТЕНИЕМ ЛОГА, А НЕ ДОГАДКОЙ: `docker logs` (тот же обходной путь) показал
`FastNavigator: arrived (1.5)` — движок СЧИТАЕТ себя прибывшим на дистанции 1.5 и САМ
останавливает `BlockPathWalker`. Бот не завис — он ЗАКОНЧИЛ навигацию правильно. Баг —
СНАРУЖИ, в отчёте: `FastNavigator.java:36,175` — `arrived = dist <= ARRIVE_DIST(=2.0) &&
goalRise<1.0`, а `Py4jEntryPoint.pathStatus():4373` (СТАРЫЙ код) — `out.put("arrived", dist <
1.5)` — ДРУГОЙ, more строгий и никак не связанный порог. Дистанция 1.5 проходит движок
(`<=2.0`), но не проходит статус-API (`<1.5` строго). Агент, гоняющий `gotoXYZ → pathStatus
until arrived` (ровно тот цикл, что описан в `docs/features/AGENT_PY4J_LEVERS.md`), завис бы
НАВСЕГДА в этом промежутке — не потому что бот застрял, а потому что API соврал о его
состоянии. Это НЕ тот же механизм, что уже задокументированные простои блуждания
(`Nav.isPathing()` true при недвижимом теле, C10.1/верх файла) — там ДЕЙСТВИТЕЛЬНО ничего не
двигает тело; здесь тело УЖЕ пришло, просто отчёт об этом настроен неверно.

ПОЧИНЕНО (код изменён, НЕ развёрнуто и НЕ переизмерено — деплой недостижим тем же путём, что и
раньше: `docker exec` запрещён прокси, RCON недостижим, `gradlew`/`deploy_jar.sh` не
запускались): `FastNavigator.ARRIVE_DIST` сделан `public` (был `private`), и
`Py4jEntryPoint.pathStatus()` теперь читает ЕГО, а не хранит отдельную, независимо
угаданную константу — правило 4c этого же чеклиста («A duplicated constant will drift... Have
one side READ the other») применено, а не проигнорировано. `busy` (`hasActiveTask()` ->
`isTungstenActive()` -> `PATHFINDER.active`/`EXECUTOR.isRunning()`) НЕ тронут — в моём случае
навигация шла через `BlockPathWalker`, не через `PATHFINDER`/`EXECUTOR`, и я не проследила
`busy` до конца; не хочу чинить вслепую то, что не поняла до конца.

⛔ ЗАОДНО, БЕЗ ДОПОЛНИТЕЛЬНОЙ ПРАВКИ: `gotoFar` (`Py4jEntryPoint.java:4153`, javadoc «agent
loops: gotoFar → pathStatus (until arrived) → gotoFar») ходит по ТОМУ ЖЕ `_gotoGoal`/
`pathStatus()` для каждого сегмента — тот же баг МОГ вешать и длинную дистанцию на КАЖДОМ
промежуточном отрезке, не только точечный `gotoXYZ`. Фикс общий, отдельно чинить нечего, но
стоило явно проверить, а не предполагать по аналогии — проверено чтением, `gotoFar` зовёт
`gotoXYZ` изнутри (`:4172`), значит делит с ним и поле, и метод.

Проверено также (не тот же класс бага, отдельная находка, УЖЕ отслеживаемая): `;settings`
живьём показал флаг `arrivalAgreesWithTheSnap = false` — это ДРУГОЕ несоответствие «прибытия»
(снап цели на пригодную клетку vs точное совпадение), с собственным механизм-счётчиком
`arrivedAtSnap` в `CustomBaritoneGoalTask.java:54-58`, никак не пересекающееся с моей правкой.
Не трогаю — уже размечено и ждёт своего парного замера.

СЛЕДУЮЩИЙ ШАГ: пересобрать и передеплоить jar, когда `gradlew`/`deploy_jar.sh` станут
доступны, и живым прогоном подтвердить, что `pathStatus` теперь верно докладывает `arrived`
на той же дистанции 1.5-2.0. До этого момента правка — ПРОЧИТАННАЯ и логически обоснованная,
...
Три правки инструмента (`b69cb74a`, `97bc3123`, `f879f7c0`) снимают структурное препятствие
из записи выше: отдельный запуск `gamer_smoke.py N --pin-alt X=true` (без `--repeat`) теперь
ЧЕРЕДУЕТ ветку через `gamer_arm_index.txt` и печатает тот же маркер `RUN i/i`, что и `sweep()`
— значит вывод нескольких таких запусков, склеенный в один файл (`>>`), читается
`paired_ab.py` в точности как один непрерывный свод. Проверено СКВОЗЬ настоящий
`paired_ab.py` на синтетическом логе (см. коммит) — не только в изоляции.

⛔ СТАРЫЙ `.gradle/ablogs/fire_ab.log` НЕ ГОДИТСЯ ДЛЯ ПРОДОЛЖЕНИЯ. В нём РОВНО один
завершённый прогон (`RUN 1/4`, `ARM B`) — свод был убит после первого же прогона, не после
четырёх, как можно предположить по имени. Он записан ДО починки чередования, и
`gamer_arm_index.txt` сейчас стоит на `0` (только что заведён). Если продолжить дописывать
в ЭТОТ файл прямо с нуля индекса, следующий отдельный запуск посчитает себя «прогоном 1» и
снова выберет ветку B — вместо A, чем должен был бы продолжиться свод (B,A,B,A,...).

НЕ ЧИНИТЬ СТАРЫЙ ФАЙЛ ПОДГОНКОЙ СЧЁТЧИКА. Один прогон — не результат (правило ПЯТЬ), этих
данных всё равно недостаточно ни для чего. Когда стенд вернётся: начать НОВЫЙ лог-файл под
новым именем, с `gamer_arm_index.txt` на `0` (текущее состояние подходит для НОВОГО свода,
не для продолжения старого), и собирать по одному прогону за сессию, пока не наберётся
хотя бы 3 пары (6 прогонов) — тогда исход `fireReleaseNeedsFire` можно будет наконец
оценить по простою (правило 4a2).

Собранный на этот момент код: диагноз кражи клавиши атаки установлен и починен, jar на диске
(`build/libs/unionclef-1.21.11-0.93.0.jar`, mtime 2026-08-31 18:24) актуален для текущего
HEAD — после него в java-код правок не было (`git diff --stat 9a44e26d..HEAD -- '*.java'`
пуст), пересборка перед замером не нужна, только `deploy_jar.sh` при живом стенде.

<!-- NAVBREAK-DOES-NOT-DISCRIMINATE-2026-08-31 -->
## ⏳ nav_break НЕ РАЗЛИЧАЕТ ПРАВКУ: КУРС ПРОХОДИТСЯ ЗА 9 СЕКУНД (2026-08-31)

Своды по прохождению умирали вместе с сессией (окно сессии короче одного прогона), поэтому
взят короткий курс, который ЗАСТАВЛЯЕТ копать. Ветки чередуются, пин проверен на каждом прогоне.

```
прогон  ветка          итог   время
  1     A (контроль)   PASS   14.0с
  2     B (правка)     PASS   11.9с
  3     A (контроль)   PASS    8.8с
  4     B (правка)     FAIL   19.5с
  5     A (контроль)   PASS    9.4с
  6     B (правка)     PASS    8.9с
```

⛔ Я ЧУТЬ НЕ ЗАПИСАЛ ОТКАЗ В КОНТРОЛЬ. Свод начался с ветки A, а не B, — чередование надо
ЧИТАТЬ ИЗ ЛОГА, а не предполагать. Отказ лежит в ветке С ПРАВКОЙ.

ЧТО ЭТО ЗНАЧИТ: ПОЧТИ НИЧЕГО. Курс проходится за ~9 секунд, у него нет запаса, чтобы что-то
различить, и это было сказано ДО просмотра чисел. Сам отказ — гейт `freezes==1`, причём окно
зависания показывает `breakQ=null placeQ=null breakTicks=0`: в этот момент НИЧЕГО не копалось,
то есть отказ не лежит на пути правки. Цель при этом достигнута (19.5с, final_dist=0.7).

ВЫВОД: ИСХОД ПО-ПРЕЖНЕМУ НЕ ИЗМЕРЕН. Ни за, ни против. Флаг `fireReleaseNeedsFire` ОСТАЁТСЯ
ВЫКЛЮЧЕННЫМ. Нужен парный замер по ПРОСТОЮ на прохождении (правило 4a2), а для него нужно
окно длиннее одного 5-10-минутного прогона.

ПРЕПЯТСТВИЕ СРЕДЫ, А НЕ КОДА: сессии рвутся каждые 5-6 минут, прогон плюс подготовка — 6.5.
Три свода подряд убиты на середине, каждый раз ОСТАВЛЯЯ пин в `tungsten.json` стенда.

<!-- ATTACK-KEY-THEFT-FOUND-2026-08-31 -->
## ⭐⭐⭐ НАЙДЕНО И НАЗВАНО: ЦЕПЬ ЗАЩИТЫ ОТ МОБОВ ОТБИРАЕТ КЛАВИШУ АТАКИ У КОПАЛЬЩИКА (2026-08-31)

СТРОКА `MobDefenseChain:645`:
...
Комментарий говорит «если БОЛЬШЕ НЕ НУЖНО». `wasPuttingOutFire` ставится двумя строками выше
и НЕ ЧИТАЕТСЯ, поэтому release срабатывает на КАЖДОМ тике, когда бот не стоит в огне, — то
есть почти всегда. А клавиша атаки — это то, ЧЕМ tungsten копает: он целится, жмёт, и ваниль
ломает то, что под прицелом.

ЦЕПОЧКА, КАЖДОЕ ЗВЕНО — СВОЙ ЗАМЕР:

```
копальщик РАБОТАЕТ         stallMiner 1143/6, 2260/6, 877/1   (99.5%)
прицел НА ПЛАНЕ            899 из 903 тиков в одном прогоне
клавиша НАЖАТА             breakAim нацелен 803, 1905, 899, 734
её ОТПУСКАЕТ чужой код     attackThief=[MobDefenseChain:645 x299]
ваниль почти не ломает     mine = 135/4180 = 3.1% на прохождении
                           против 73% на nav_break, где эта цепь простаивает
```

ПРОВЕРКА ПРАВКИ, ПЕРВАЯ ВЕТКА (флаг `fireReleaseNeedsFire`):

```
             ДО                              ПОСЛЕ
attackThief  MobDefenseChain:645 x299        (none)
mine         135/4180 = 3.1%                 51/1 = 98%
механизм     —                               fireReleaseSkipped=12227
```

МЕХАНИЗМ И ПРОМЕЖУТОЧНАЯ ВЕЛИЧИНА СДВИНУЛИСЬ ВМЕСТЕ — ровно то, чего не хватало правке
«ломай то, что мешает», где счётчик срабатывал сильнее всего в единственной паре, ушедшей
ПРОТИВ правки.

⛔ ЧТО ЕЩЁ НЕ ИЗМЕРЕНО: ИСХОД. Простой и ступени по парам НЕ сняты — своды падали вместе с
сессией. Три правки в этой линии уже имели полностью рабочий механизм и НУЛЕВОЙ сдвиг исхода
(цель блуждания 94%->0%, холостой замок 45-55%->10%, «ломай то, что мешает» 137 срабатываний).
Поэтому флаг ОСТАЁТСЯ ВЫКЛЮЧЕННЫМ до парного замера по простою (правило 4a2).

⛔ ЛОВУШКА СТЕНДА, СРАБОТАВШАЯ ДВАЖДЫ ЗА СЕССИЮ: убитый на середине свод с `--pin-alt`
ОСТАВЛЯЕТ пин в `tungsten.json` стенда. Оба раза стенд оставался с `fireReleaseNeedsFire=True`,
и любой следующий прогон исполнял бы ветку с правкой, записываясь как контроль. ПРОВЕРЯТЬ
ФЛАГИ СТЕНДА ПЕРЕД КАЖДЫМ ЗАПУСКОМ, а не после.

<!-- INWAY-NOT-ESTABLISHED-2026-08-31 -->
## ⛔ ПРАВКА «ЛОМАЙ ТО, ЧТО МЕШАЕТ» — ВЫИГРЫШ НЕ УСТАНОВЛЕН, И СЧЁТЧИК ЭТО ДОКАЗЫВАЕТ (2026-08-31)

Пара чистая по правилу 4a1: механизм ненулевой в КАЖДОЙ ветке с правкой (137, 3, 2) и НОЛЬ
в каждом контроле, ветки чередуются B/A/B/A/B/A по правилу 4r.

```
земля              ступени  простой правка / контроль     перекрытий в ветке с правкой
-308 150 -1439      5 /4    177с (30%)    65с (11%)              137
-8   150 -1439      4 /2     67с (11%)   265с (45%)                3
1192 150 -1439      1 /3     66с (11%)   476с (82%)                2

простой  -165.3 с/прогон  t -1.09  НЕ УСТАНОВЛЕНО  [+112, -198, -410]
доля     -28.4%/прогон    t -1.09  НЕ УСТАНОВЛЕНО
ступени  +0.33/прогон     t +0.28  НЕ УСТАНОВЛЕНО
```

⛔ ГЛАВНОЕ: НАПРАВЛЕНИЕ МЕХАНИЗМА ПРОТИВОРЕЧИТ НАПРАВЛЕНИЮ ИСХОДА. Пара, где правка
СРАБОТАЛА 137 раз, дала +112 с ПРОТИВ правки. Пары, где она сработала 3 и 2 раза — то есть
практически не работала, — дали весь выигрыш (-198 и -410 с). Правка НЕ МОЖЕТ давать
наибольший эффект в прогонах, где она почти не исполнялась, значит -165 с/прогон — это
межпрогонный разброс, а не действие правки.

БЕЗ СЧЁТЧИКА МЕХАНИЗМА ЭТО БЫЛО БЫ ЗАГОЛОВКОМ: «-165 с простоя на прогон, -28% доли». Именно
для этого правило 4a1 и существует, и здесь оно сработало не на пустой паре, а на паре,
которая ВЫГЛЯДЕЛА информативной.

ФЛАГ `mineTheBlockInTheWay` ОСТАЁТСЯ ВЫКЛЮЧЕННЫМ. Код и счётчики остаются: диагноз под ними
установлен (0 из 8016, 0 из 3548), не установлено ТОЛЬКО то, что эта правка чинит простой.

ЧТО ОСТАЁТСЯ ВЕРНЫМ НЕЗАВИСИМО ОТ ВЕРДИКТА: цепочка измерена по каждому звену — застойная
копка не ломает ничего (0 из 8016), сошедшийся прицел попадает в другую клетку (46-90%
промахов), перекрывающий блок НИКОГДА не в плане (0 из 3548). Дефект реален; починка его
простоя — нет.

СЛЕДУЮЩИЙ ЗАХОД: аварийные выходы ПО ДАЛЬНОСТИ, 151 против 1 по таймауту, и целый прогон
(72 из 72) где `breakAim` вообще нулевой — до прицеливания не дошло ни разу. Бот планирует
ломать клетки, до которых стоит дальше 4.5 блоков. Это крупная установленная величина,
которой до `breakAbort` не существовало нигде.

<!-- OCCLUSION-AND-REACH-2026-08-31 -->
## ⭐⭐⭐ ПРИЦЕЛ СОШЁЛСЯ, А ЛУЧ УПЁРСЯ В СОСЕДНЮЮ КЛЕТКУ: 46% ПРОМАХОВ — ПЕРЕКРЫТИЕ (2026-08-31)

```
прогон  нацелен  мимо  в_другой  в_пустоту | аварий т/д | перекрыто/в_пути | образец промаха
  1       485     214    212        2      |   0/4      |    85/127        | 1529,66,-18!=1529,65,-18@dy0/dp0
  2       220       0      0        0      |   0/24     |     0/0          | -
  3        23     329    328        1      |   0/84     |   255/73         | -301,115,-238!=-300,115,-238@dy0/dp0
  4       105     196    196        0      |   1/39     |     0/196        | -300,115,-237!=-299,114,-236@dy101/dp25
```

ПЕРЕКРЫТИЕ УСТАНОВЛЕНО: 340 из 736 промахов «в другой блок» (46%) идут с СОШЕДШИМСЯ прицелом
(меньше 2° по обеим осям). В прогоне 3 прицел стоял на цели лишь 23 раза из 352 — 6.5%.

ОБА ОБРАЗЦА С НУЛЕВЫМ ОСТАТКОМ НАЗЫВАЮТ СОСЕДНЮЮ КЛЕТКУ — по Y в прогоне 1, по X в прогоне 3.
Это и есть картина копания стены: целишься в клетку, а луч останавливается на ближней грани.
Спусковой крючок завязан на ТОЖДЕСТВО, поэтому не ломается ни ближняя, ни дальняя, и очередь
не пустеет.

⭐⭐ ВТОРАЯ НАХОДКА, КОТОРУЮ НЕ ИСКАЛИ: АВАРИЙНЫЕ ВЫХОДЫ ПО ДАЛЬНОСТИ ПРЕОБЛАДАЮТ — 151
против 1 по таймауту. Бот раз за разом планирует ломать клетки, до которых сам стоит дальше
4.5 блоков. В прошлом своде было 19/19, то есть разброс по рельефу огромный, но здесь
дальность крупнее таймаута на два порядка. Раньше этой величины не существовало нигде:
место аварийного выхода различало половины только в ЛОГЕ.

ОГОВОРКА ПО ЗНАМЕНАТЕЛЮ: в этом своде застойной копки почти не было (`stallExecWork` = 0/4,
0/0, 0/1, 0/0), тогда как в предыдущих сводах она давала 0/8016. Разброс между прогонами
огромный, и числа выше — про копку ВООБЩЕ, а не про застойную копку. Смешивать эти две
популяции нельзя: на этом я уже ошибался дважды.

СЛЕДУЮЩИЙ ЗАХОД: `breakOccluder=в_плане/чужой` — лежит ли перекрывающая клетка в самой очереди
слома. В ПЛАНЕ значит очередь опустошается в порядке, запрещённом геометрией: ближняя клетка
обязана падать первой, и ломать её — уже разрешённая планом работа, правка ЗДЕСЬ, в порядке.
ЧУЖОЙ значит планировщик выдал исполнителю цель, которую тот не видит по своей же линии
взгляда, — это дефект СЛОЕМ ВЫШЕ, в другом файле. Правки противоположные, поэтому счётчик.
⛔ ПРОВЕРЕНО ЧТЕНИЕМ 2026-09-01 (стенд по-прежнему недоступен весь этот заход — см. C8.1 —
поэтому только код, не замер): счётчик СУЩЕСТВУЕТ и подключён правильно. `breakOccluderInPlan`/
`breakOccluderForeign` (`PathExecutor.java:733,735,884`) считаются БЕЗУСЛОВНО внутри ветки
`breakOccluded` (сама она тоже безусловна, `:725-726`), а не за флагом `mineTheBlockInTheWay` —
последний гейтит ТОЛЬКО действие `clearTheWay` (`:758`), не счётчик, ровно по правилу 4u
(«счётчик — наблюдение, флаг — поведение»). Уже прокинут в py4j-строку статуса
(`Py4jEntryPoint.java:3238`, поле `breakOccluder=%d/%d`) и в отдельный сброс
(`Py4jEntryPoint.java:2717-2718`). НИ ОДНОГО ЗАМЕРА С РЕАЛЬНЫМИ ЧИСЛАМИ ЕЩЁ НЕТ — `grep -n
"breakOccluder=[0-9]" TODOS.md` пуст. Инструмент готов, вопрос ждёт живого прогона.

<!-- STALLED-MINING-NEVER-BREAKS-2026-08-31 -->
## ⭐⭐⭐ УСТАНОВЛЕНО НА 6 ПРОГОНАХ: В ЗАСТОЕ ОЧЕРЕДЬ СЛОМА ЕСТЬ, А ВАНИЛЬ НЕ ЛОМАЕТ — 0 ИЗ 8016 (2026-08-31)

```
прогон  breakAim(нацелен/мимо/в_другой/в_пустоту)  нацелен%  breakAbort(таймаут/дальность)  застой видно/слепо
  1     3040/723/722/1                               80%            6/8                        0/1590
  2     2542/49/49/0                                 98%            7/7                        0/1931
  3     1988/959/829/130                             67%            5/3                        0/1614
  4     297/246/246/0                                54%            1/1                        0/125
```

`stallExecWork` = 0 видно / 5260 слепо в этом своде и 0/8016 вместе с двумя ранними случаями —
ШЕСТЬ независимых прогонов, ни одного «видно». Правило ПЯТЬ выполнено, это МЕХАНИЗМ, а не выборка.

СТОРОЖЕВОЙ ТАЙМЕР ОПРАВДАН: 38 аварийных выходов на 4 прогона (19 таймаутов, 19 по дальности).
Значит дело НЕ в том, что попытки крутятся по 300 тиков и сдаются. Счётчика на это не было
вообще — место аварийного выхода различало половины только в ЛОГЕ, а частота была невидима
в каждой строке вердикта, что этот проект когда-либо собирал.

ПРОТИВОРЕЧИЕ, КОТОРОЕ ТЕПЕРЬ ТОЧНО СФОРМУЛИРОВАНО: прицел стоит на цели 54-98% ВСЕХ тиков
копки, но на 5260 ЗАСТОЙНЫХ тиках копки ваниль не сломала НИЧЕГО. Это возможно, только если
именно в эти тики прицел НЕ на цели. Разные популяции — та самая ловушка знаменателя, на
которой я уже дважды ошибся, поэтому её меряют, а не рассуждают о ней.

⛔ ГИПОТЕЗА «ПРИЦЕЛ НЕ СХОДИТСЯ» ОТКЛОНЕНА: `в_пустоту` = 1, 0, 130, 0 — прицел почти всегда
находит КАКОЙ-ТО блок, а `нацелен` доходит до 98%.

ЧТО ВИДНО В ОБРАЗЦАХ `breakMiss` (последний промах, по одному на прогон):

```
-3,79,-827   != -1,79,-827   @dy0/dp0     остаток НОЛЬ, две клетки по одной строке
1500,105,-1146 != 1501,105,-1145 @dy46/dp0
1498,58,-842 != 1499,58,-841  @dy2/dp0    остаток почти ноль
1571,48,-551 != 1571,50,-551  @dy1/dp39
```

Два образца с ПОЧТИ НУЛЕВЫМ остатком: прицел СОШЁЛСЯ и всё равно указывает на другую клетку —
значит на луче стоит блок БЛИЖЕ. Два с большим остатком — прицел просто в пути. Спусковой
крючок завязан на ТОЖДЕСТВО с запланированной клеткой, поэтому в первом случае не ломается
ни ближний блок, ни дальний, и очередь не может опустеть.

СЛЕДУЮЩИЙ ЗАХОД МЕРЯЕТ ДОЛЮ: `breakMissWhy=перекрыто/в_пути` — промах с остатком меньше 2°
по обеим осям против остальных. Это ДВЕ РАЗНЫЕ ПРАВКИ (порядок/планирование против «ничего
не делать, само пройдёт»), поэтому счётчик, а не рассуждение.

<!-- MINING-IS-INVISIBLE-TO-THE-CHECKER-2026-08-31 -->
## ⭐⭐⭐ ЗАСТОЙНЫЙ ИСПОЛНИТЕЛЬ КОПАЕТ, А ВАНИЛЬНАЯ ДОЛБЁЖКА НЕ ИДЁТ: 0 ИЗ 1166 (2026-08-31)

`stallExec=слом/постройка/повтор/взведён/индекс_замер` и `stallExecWork=видно/слепо`,
знаменатель — застойный тик, на котором исполнитель заявил, что работает.

```
прогон  stallWhy(замок/поиск/исп/оч/ходок)   stallExec              stallExecWork
  1     3326/75/884/0/0                      884/0/0/0/883          0/884
  2      581/21/3/0/0                          0/0/3/0/1            0/0
  3      923/635/7/0/0                         0/0/7/0/0            0/0
  4      849/282/285/0/0                     279/3/3/0/279          0/282
```

ДВА РЕЗУЛЬТАТА, ОДИН СИЛЬНЫЙ И ОДИН СЛАБЫЙ — РАЗЛИЧАТЬ.

СИЛЬНЫЙ: `повтор` = 5 тиков из 4083 за прошлый свод и 3+3 здесь. Исполнитель ПРАКТИЧЕСКИ
НИКОГДА не крутит бесплодный повтор пути. Когда он «работает» в застое — он КОПАЕТ или
МОСТИТ. Замороженный индекс (99-100%) — не симптом, а то, что код и описывает: путь держится
незавершённым, пока не опустеет очередь слома.

СЛАБЫЙ ПО ЧИСЛУ ПРОГОНОВ, НО ЧИСТЫЙ ПО ТИКАМ: `stallExecWork` = 0 видно / 1166 слепо.
На КАЖДОМ из 1166 тиков, где очередь слома непуста, `ControllerExtras.isBreakingBlock()`
ЛОЖЬ — то есть ваниль не долбит вообще. Прогоны 1 и 4 дали 884 и 282; прогоны 2 и 3 в
застое не копали совсем, так что случаев ДВА, а не четыре. По правилу ПЯТЬ это выборка,
а не механизм, и на ней НЕЛЬЗЯ строить правку.

ГИПОТЕЗА, НАЗВАННАЯ ДО ЗАМЕРА (не утверждение): tungsten копает, НАЖИМАЯ КЛАВИШУ АТАКИ, и
жмёт её ТОЛЬКО при совпадении прицела с запланированной клеткой:

```java
boolean onTarget = look instanceof BlockHitResult bhr
        && look.getType() == HitResult.Type.BLOCK
        && bhr.getBlockPos().equals(target);
options.attackKey.setPressed(onTarget);
```

Если прицел туда не приходит — не ломается ничего, очередь остаётся непустой, а
`breakingTicks` растёт до аварийного выхода на 300 тиках (15 секунд). Это в точности даёт
0/1166.

ЧТО МЕРЯЕТ СЛЕДУЮЩИЙ ЗАХОД: `breakAim=нацелен/мимо/в_другой_блок/в_пустоту` + `breakMiss`
(последнее расхождение с остаточной ошибкой прицела). Считается ВНУТРИ `tickBreaking`, где
`onTarget` и `target` уже вычислены, и его знаменатель — «тик, где цель реальна, ломаема и
в досягаемости», то есть он есть в ЛЮБОМ прогоне, который вообще копает, а не только когда
блуждание застряло. Счётчик проверен на неослеплённость: он стоит ПОСЛЕ проверок очереди,
цели, `BreakRules.canBreak` и 4.5 блоков, но НЕ за условием, которое сам измеряет.

`в_другой_блок` и `в_пустоту` требуют ПРОТИВОПОЛОЖНЫХ правок — планирование/досягаемость
против несошедшегося прицела, — поэтому это счётчик, а не рассуждение.

<!-- IDLE-LOCK-REJECTED-2026-08-30 -->
## ⛔ ПРАВКА «ХОЛОСТОЙ ЗАМОК — НЕ ЗАМОК» ОТКЛОНЕНА ЗАМЕРОМ, И ОНА ЖЕ ЗАОСТРИЛА ДИАГНОЗ (2026-08-30)

Пара чистая по правилу 4a1: механизм 8/0, 5/0, 22/0 — ненулевой в КАЖДОЙ ветке с правкой и
ноль в КАЖДОМ контроле. В отличие от свода по спирали, этот замер СОСТОЯЛСЯ.

```
земля             ступени   простой правка / контроль        механизм
892 150 661        0 /4      113с/596с (19%)    91с/598с (15%)   8/0
292 150 661        0 /4      183с/580с (32%)    22с/593с ( 4%)   5/0
892 150 -239       4 /2      249с/595с (42%)   338с/581с (58%)  22/0

ступени  -2.00/прогон  t -1.00   НЕ УСТАНОВЛЕНО   [-4, -4, +2]
простой  +31.3 с/прогон t +0.43  НЕ УСТАНОВЛЕНО   ПРОТИВ правки
доля     +5.1%/прогон  t +0.40   НЕ УСТАНОВЛЕНО   ПРОТИВ правки
```

МЕХАНИЗМ СРАБОТАЛ ПОЛНОСТЬЮ, И ПРОМЕЖУТОЧНАЯ ВЕЛИЧИНА ТОЖЕ. Доля холостых тиков замка:
...
Холостые замки действительно исчезли. Простой не уменьшился, ступени скорее просели.
ФЛАГ `idleLockIsNotALock` ОСТАЁТСЯ ВЫКЛЮЧЕННЫМ.

⛔ И ЭТО ПРЯМО ОПРОВЕРГАЕТ МОЁ СОБСТВЕННОЕ УТВЕРЖДЕНИЕ ИЗ КОММИТА b0209ec8, где написано
«ПОЧЕМУ УДЕРЖАНИЕ ЗАМКА И ЕСТЬ ПРОСТОЙ». Это была ПРИЧИННАЯ формулировка, сделанная до
проверки, а замер её снял: замок держится на 100% застойных тиков, но снятие холостого
замка простой не убирает. «Держится вместе с» и «вызывает» — разные утверждения, и я
записал второе, имея основание только для первого.

ЧТО ЗАМЕР ДАЛ ВЗАМЕН — БОЛЕЕ ТОЧНЫЙ ДИАГНОЗ. В ветках с правкой `stallWhy` по замку
ВСЁ РАВНО равен знаменателю (2028/2055, 1632/1632, 7474/7474), но холостым он уже не
является. Разбор замка по занятости:

```
с правкой:  исполнитель 49%  67%  84%   холостой 10%  6%  10%
```

То есть застойный замок НЕ ХОЛОСТОЙ — за ним РАБОТАЕТ ИСПОЛНИТЕЛЬ, а очередь движения и
ходок при этом НОЛЬ во всех прогонах обеих веток. Это вторая ветка той дихотомии, что
записана в javadoc `lockAnat`: не «никто не ведёт тело», а «привод держит тело и не сдвигает
его». `stallWhy` по исполнителю в ветках с правкой: 1120, 1371, 6447.

СЛЕДУЮЩИЙ ЗАХОД: исполнитель работает, очередь движения не идёт, тело стоит. Спросить
счётчиками, ЧТО исполнитель исполняет в эти тики и почему это не превращается в шаги
очереди.

ОГОВОРКА: n=3 пары, ничего не установлено на планке 2 сигма — в том числе и вред. Но
выигрыша нет ни по одной из трёх метрик, поэтому флаг выключен, а не «пока не решено».

<!-- STALL-IS-THE-LOCK-2026-08-30 -->
## ⭐⭐⭐ НАЗВАНО ЧИСЛОМ: ПРОСТОЙ БЛУЖДАНИЯ ДЕРЖИТ ЗАМОК, 100% ТИКОВ, 4/4 ПРОГОНА (2026-08-30)

`stallWhy=замок/поиск/исполнитель/очередь/ходок` снимается В САМОЙ ВЕТКЕ ОТКАЗА, поэтому его
знаменатель — застойный тик по построению.

```
прогон  отказов   замок   ==?   очередь ходок    вхолостую/всего замка   отсчитано замков
  1      1182     1182    ДА       0     0        1482/2809 (52%)            0/0
  2      3321     3321    ДА       0     0        2404/4504 (53%)            0/0
  3      1435     1435    ДА       0     0        1822/3291 (55%)            6/1
  4      3517     3517    ДА       0     0        3328/7372 (45%)           14/2
```

ЗАМОК ДЕРЖИТСЯ НА 100.0% ЗАСТОЙНЫХ ТИКОВ — не «преобладает», а СОВПАДАЕТ ТОЧНО, во всех
четырёх прогонах. Очередь и ходок — НОЛЬ везде: тело в эти тики не ведёт НИКТО. И около
половины всех тиков замка (45-55%) не заняты вообще ничем — ни поиском, ни исполнителем.

⛔ МОЙ ПРОГНОЗ БЫЛ НЕВЕРЕН, И ОСНОВАНИЕ ТОЖЕ. Я записал ДО замера «преобладает поиск, замок
около нуля», сняв замок с подозрения по строке `lock=0/0/0` в дампах. Эти три поля —
`lockBarren/lockProductive`, то есть СОБЫТИЯ ОТСЧЁТА замка, а НЕ «замок держится». Счётчик
прочитан по имени вместо определения — ошибка, которую эта же сессия задокументировала уже
четырежды. Прогноз лежит в scratchpad/prediction.md, написан до замера и не правился.

ВЕТКА СДАЧИ НИЧЕГО НЕ ДЕЛАЕТ — это факт КОДА, не замера: у `Nav.cancel()` ПУСТОЕ ТЕЛО,
намеренный no-op (версия, которая останавливала tungsten, убивала поиск на том же тике, где
он стартовал). Значит без моба рядом и без застревания в заборе вся ветка сдачи блуждания —
это `stuckCheck.reset()` и больше ничего: не перевыбор, не остановка поиска, не сброс замка.
А перевыбор ниже закрыт условием `!isExecutingRoute() && !isActive()`. Счётчик подтверждает:
`wanderTripBlocked` = 17/26/12/41 — КАЖДОЕ вхождение в ветку сдачи заставало перевыбор
по-прежнему закрытым. Ответ на вопрос «что происходит после срыва»: НИЧЕГО.

ВЕДУЩАЯ ГИПОТЕЗА (НЕ ДОКАЗАНА): замок продлевается вечно, потому что отсчёт его бесплодности
`scoreExpiredLock()` начинается с `if (lockStartDist < 0 || lockedEntity == null ...) return;` —
то есть отсчитываются ТОЛЬКО замки НА СУЩНОСТЬ. Блуждание берёт замок на `Vec3d`. В прогонах
1-2: 7313 тиков замка и НОЛЬ отсчитанных замков. В прогонах 3-4 отсчёты есть (6/1 и 14/2), и
геометрия в строке показывает именно сущности (предмет-кирка, свинья). Согласуется, но НЕ
доказано — распределение «сколько времени замка приходится на не-сущностные замки» не снято.

ЭТО НЕ ТОТ ЖЕ ДЕФЕКТ, ЧТО `barrenGateIsForEntityLocksOnly`. Тот снял ЗАЩЁЛКУ с блуждания и
давно включён и проверен. Здесь дыра дополнительная: не-сущностные замки не отсчитываются
ВООБЩЕ, поэтому `MAX_BARREN_LOCKS` для них недостижим.

<!-- SPIRAL-LEGS-NOT-TAKEN-2026-08-30 -->
## ⏳ ПРАВКА «СПИРАЛЬ СЧИТАЕТ НОГИ» — ЗАМЕР НЕ СОСТОЯЛСЯ, МЕХАНИЗМ НЕ СРАБОТАЛ (2026-08-30)

Свод 6 прогонов, 3 пары, пин проверен в логе, все 6 прогонов PASS (свод 6/6).

```
земля               ступени   простой правка / контроль        механизм
-308 150 -839        3 /3       46с/600с (8%)   136с/594с (23%)   0/0
-308 150 -1139       2 /4      224с/594с (38%)  269с/598с (45%)   0/0
-308 150 -1439       5 /5      176с/592с (30%)   46с/589с (8%)    0/0

ступени  -0.67/прогон   t -1.00    НЕ УСТАНОВЛЕНО
простой  -1.67 с/прогон t -0.02    НЕ УСТАНОВЛЕНО
```

⛔ ЧИСЛА ЧИТАТЬ НЕЛЬЗЯ: `wanderSpiralHeld` = 0 ВО ВСЕХ ШЕСТИ ПРОГОНАХ. Правка не сработала
НИ РАЗУ, поэтому ветка «с правкой» и контроль исполняли ОДИН И ТОТ ЖЕ код, а t≈-0.02 — это
шум двух одинаковых сборок, а не результат.

ПОЧЕМУ МЕХАНИЗМ МОЛЧАЛ — видно по `wanderTung=<выбрано>/<ведено>/u<нестоячих>`:

```
ветка с правкой:   4/4      10/10     25/25     -> отказов НОЛЬ
контроль:          9/9      58/14     184/21    -> отказов 0, 44, 163
```

`wanderSpiralHeld` растёт ТОЛЬКО когда `tryPathTo` ОТКАЗЫВАЕТ. В трёх прогонах с правкой
привод принял КАЖДУЮ цель, отказов не было вовсе — держать индекс было не на чем. А два
прогона, где отказы шли сотнями, выпали в КОНТРОЛЬ, где флаг выключен и счётчик не может
расти по построению. Раскладка невезучая, но замер от этого не становится замером.

ВЫВОД: вердикт НЕ ВЗЯТ, флаг `wanderSpiralCountsLegsNotTries` остаётся ВЫКЛЮЧЕННЫМ. Это НЕ
«правка отклонена» — её просто не проверили. Чтобы взять вердикт, нужны пары, где ветка с
правкой ВИДИТ отказы; на 10-минутных прогонах это лотерея (0-163 отказов на прогон).

ЧТО ЗАМЕР ВСЁ-ТАКИ ДАЛ: свод 6/6 при флаге, включённом в трёх прогонах, — слабое, но
ненулевое свидетельство ОТСУТСТВИЯ РЕГРЕССИИ.

ДЕФЕКТ, КОТОРЫЙ ЭТИ ЖЕ ЧИСЛА ПОДТВЕРЖДАЮТ НЕЗАВИСИМО ОТ ПРАВКИ: 184 выбора против 21
пройденной ноги. Индекс спирали (а с ним и радиус, до 120 блоков) накручивается на
ОТКАЗАХ, и худший по простою прогон — ровно тот, где отказов 163.

<!-- WANDER-GROUND-REJECTED-2026-08-29 -->
## ⛔ ПРАВКА «ЦЕЛЬ БЛУЖДАНИЯ ПО ЗЕМЛЕ» ОТКЛОНЕНА ЗАМЕРОМ (2026-08-29)

Механизм работает полностью, исход — против правки.

```
простой по парам:   свод 1  -44, +65      свод 2  +90, +23
4 пары:             +33.5 с/прогон ПРОТИВ правки,  t ~ +1.2   НЕ УСТАНОВЛЕНО
ступени:            -0.5/прогон
нестоячих целей:    контроль 98-100%  ->  ветка с правкой 0-3%
мёртвое время:      ОСТАЛОСЬ в блуждании, 56%, в ОБЕИХ ветках
```

ВЫВОД, КОТОРЫЙ ВАЖНЕЕ САМОЙ ПРАВКИ: 94% несуществующих целей — РЕАЛЬНЫЙ дефект и НЕ ПРИЧИНА
простоя. Механизм починен полностью, симптом не сдвинулся. Флаг `wanderTargetFollowsTheGround`
остаётся ВЫКЛЮЧЕННЫМ; код и счётчик остаются, потому что счётчик отделяет информативную пару
от пустой, а сам дефект стоит починить, когда найдётся причина, которой он мешает.

ЭТО ПЯТАЯ ВЕРСИЯ ЗА СЕССИЮ, СНЯТАЯ ЗАМЕРОМ: `truncateAtBreaks` в общем виде, обход
`salvage()`, leaf_litter, кража клавиши вперёд, и эта. Каждая называлась ДО замера и каждая
выглядела убедительно.

⛔ ЛОВУШКА ЧТЕНИЯ, КОТОРУЮ НАДО ПОМЕТИТЬ: `paired_ab.py` печатает «CONTROL ARM IS NOT CLEAN --
the mechanism counter should read 0 there». Это его СОГЛАШЕНИЕ: он ждёт счётчик, считающий
СРАБАТЫВАНИЕ ПРАВКИ. `wanderTargetUnstandable` считает ДЕФЕКТ, поэтому в контроле он обязан
быть ВЫСОКИМ, а в ветке с правкой нулевым — предупреждение здесь означает, что всё в порядке.
Полярность счётчика механизма надо называть явно, иначе следующий читатель примет это за
испорченный замер.

ОГОВОРКА НА ВСЕ ЧИСЛА ЭТОГО УЧАСТКА: стенд деградировал к концу сессии — прогоны уходили в
`INVALID` при 7-11 fps (порог 12), своды давали 3/6 и 5/5 вместо привычных 6/6. Хост
перегружен, и это ослабляет все поздние замеры.

<!-- WANDER-TARGET-DOES-NOT-EXIST-2026-08-29 -->
## ⭐⭐⭐ ЦЕЛЬ БЛУЖДАНИЯ НЕ СУЩЕСТВУЕТ В 94% СЛУЧАЕВ — НО СИМПТОМ ЭТИМ НЕ ОБЪЯСНЯЕТСЯ (2026-08-29)

Счётчик `wanderTung=<выбрано>/<ведено>/u<нестоячих>`, три прогона:

```
94/13/u85     164/28/u158     9/9/u9        ->  252 из 267 = 94%
```

МЕХАНИЗМ. `TimeoutWanderTask` строит цель как `(anchor.x + cos*r, ANCHOR.Y, anchor.z + sin*r)`
при r до 120 блоков. XZ идёт по спирали над настоящим рельефом, Y не двигается — точка
почти всегда в скале или в воздухе. И это не просто потраченный выбор: новую цель можно
взять ТОЛЬКО при `!TungstenHelper.isActive()`, а поиск недостижимой точки держит этот флаг
истинным. Отсюда тысячи тиков блуждания, полблока пройденного пути и отказ сброса проверки
прогресса на 83-94% тиков.

ПРАВКА В ЯДРЕ (флаг `wanderTargetFollowsTheGround`, ПО УМОЛЧАНИЮ ВЫКЛ): искать опору в полосе
вокруг запрошенной высоты (вниз 48, вверх 24) и ОСТАВЛЯТЬ точку как есть, если опоры нет, —
хуже стать не может по построению. Ищет от ЗАПРОШЕННОЙ высоты, а не от поверхности мира:
вершина горы в 120 блоках — не то, куда блуждание собиралось.

МЕХАНИЗМ СРАБОТАЛ ПОЛНОСТЬЮ:

```
контроль  u23/23 (100%)   u606/619 (98%)   u50/52 (96%)
с правкой u0/316  (0%)    u2/70     (3%)
```

⛔ НО ИСХОД НЕ ВЗЯТ, И ЭТО ОСЛАБЛЯЕТ ПРИЧИННУЮ СВЯЗЬ. Простой +10.5 с/прогон на 2 парах
(t +0.19), одна пара против. И ГЛАВНОЕ: в тех же прогонах мёртвое время ВСЁ РАВНО стоит в
блуждании (56%), в обеих ветках. То есть цель починена, а симптом на месте — значит
несуществующая цель НЕ БЫЛА (единственной) причиной простоя, как бы убедительно ни выглядели
94%. Записываю это здесь, чтобы следующий заход не принял механизм за исход.

Пар мало (одна потеряна: клиент 7 fps, стенд забраковал сам). Добирается. Отдельно: счётчик
печатался только внутри `wanderTung=.../uN`, и `paired_ab` читал `mech=?/?` — теперь есть
собственный токен `wanderTargetUnstandable=<n>` (в репозитории уже был коммит ровно про этот
урок).

<!-- FORWARD-THEFT-IS-AN-ARTEFACT-2026-08-29 -->
## ⛔ «КРАЖА КЛАВИШИ ВПЕРЁД» РЕАЛЬНА, НО МЕНЬШЕ В ДВА-ТРИ ПОРЯДКА (2026-08-29, n=3)

⛔ ПОПРАВКА К ПЕРВОЙ ВЕРСИИ ЭТОЙ ЗАПИСИ. При n=1 счётчик читался `(none)`, и я написал
«артефакт, явления нет». На трёх валидных прогонах он читается 20, 3 и ~106 за прогон:

```
исправленный инструмент, по прогонам:  TimeoutWanderTask:256 x20 · (нет) · x54
                                       + DestroyBlockTask x48, SafeRandomShimmy x4 ...
прежнее число (пожизненное, по вызовам):                       10164
```

Явление ЕСТЬ, но оно в два-три порядка меньше, чем показывал сломанный счётчик. Сотня
отнятых нажатий на ~12000 тиков прогона не может объяснить простой в 30%. Как ПРИЧИНА
мёртвого времени версия снята; как мелкий дефект — остаётся, чинить не в приоритете.

Ниже — разбор самого инструмента, он верен.

Счётчик `forwardStealers` печатал крупное и убедительное число:

```
WHO RELEASED MOVE_FORWARD UNDER THE CLOSE WALK:
TimeoutWanderTask:256x10164  DestroyBlockTask:421x1349  CustomBaritoneGoalTask:243x854 ...
```

На нём построен целый комментарий в коде («1290 краж за десятиминутный прогон... это и есть
шарканье, которое видит зритель»). У числа ДВА дефекта, и оба мои же правила:

1. ОНО ПОЖИЗНЕННОЕ. `forwardStealers` НЕ обнулялся в `resetRunCounters` — между прогонами
   значения росли 8797 -> 10087 -> 10164. Репозиторий на этом уже обжигался
   («inRange=2222 clicked=0 был пожизненным итогом, а не прогоном»).
2. ОНО СЧИТАЛО ВЫЗОВ, А НЕ КРАЖУ. `release()` по клавише, которую НИКТО НЕ ДЕРЖИТ, — пустая
   операция, а счётчик записывал её как кражу.

ПОСЛЕ ПОЧИНКИ (считать только отпускание РЕАЛЬНО зажатой клавиши + сброс на прогон):

```
свежий дамп новой сборки:   WHO RELEASED MOVE_FORWARD → (none)
в том же прогоне            walkMode=12093/0/0   (ходок работал 12093 тика)
                            closeWalkFwd=0/0/0   (ближний подход не нажимал ни разу)
```

То есть пока ходок вёл тело, MOVE_FORWARD в момент отпускания НЕ БЫЛ ЗАЖАТ: ходок движет
тело не этой клавишей, и отпускание безвредно. ВЕРСИЯ О КРАЖЕ КЛАВИШИ СНЯТА.

n=1: два прогона из трёх стенд забраковал сам (клиент 8-11 fps при пороге 12 — хост
перегружен, `INVALID`, не провал). Добирается.

ЭТО ЧЕТВЁРТЫЙ ОСЛЕПЛЁННЫЙ СЧЁТЧИК ЗА СЕССИЮ: `bsStub` (недостижим за ранним выходом),
`wallSkipRefused` (под собственным флагом), мой первый `guideFromCoarseNowhere` (мерил
проводник без опоры на цель), и этот. ПРАВИЛО, КОТОРОЕ СТОИТ ДЕРЖАТЬ ОТДЕЛЬНО: счётчик
должен мерить СОБЫТИЕ, а не участок кода рядом с ним, и обязан обнуляться на прогон.

<!-- WANDER-PATHS-AND-DOES-NOT-MOVE-2026-08-29 -->
## ⭐⭐⭐ ВНУТРИ БЛУЖДАНИЯ: НАВИГАЦИЯ «ВЕДЁТ ПУТЬ», А ТЕЛО СТОИТ (2026-08-29)

Счётчики блуждания прочитаны ПО ОПРЕДЕЛЕНИЯМ, а не по именам (я сперва прочёл их неверно,
и это важно): `wanderMoved` — САНТИМЕТРЫ, пройденные на тиках этой задачи, а `wanderDenied`
= `wanderResetDenied` — сбросы проверки прогресса, ОТКАЗАННЫЕ потому, что тело не двигалось
40+ тиков, ПОКА `Nav.isPathing()` ИСТИННО.

```
прогон  wander   прошло    отказов сброса   целей выбрано/ведено
  1      5414     52 см       5086 (94%)         12/12
  2      4662    173 см       4327 (93%)        124/18
  3      4487     17 см       3730 (83%)        192/24
  4      1872      0 см       1716 (92%)         76/9
  5      4174   1321 см       2720 (65%)        110/27
  6       134     76 см          0                0/0
```

ЧИТАЕТСЯ ТАК: блуждание держит тик тысячами, проходит ПОЛБЛОКА за прогон (в четвёртом —
НОЛЬ), и на 83-94% этих тиков навигация ЗАЯВЛЯЕТ, что ведёт путь, при неподвижном теле.
Это не «блуждание не выбирает цель»: цели выбираются (12-192 за прогон) и передаются приводу.

ЭТО ДРУГОЙ КЛАСС ДЕФЕКТА, ЧЕМ ВСЁ ОСТАЛЬНОЕ В ЭТОЙ СЕССИИ. Вся линия выше — про КАЧЕСТВО
ПРОВОДНИКА (куда ведёт маршрут). Здесь маршрут не при чём: слой навигации отвечает «веду», а
тело не движется. Вопрос следующего захода — доводит ли привод тело ВООБЩЕ, и что именно
возвращает `Nav.isPathing()` в эти тысячи тиков.

ЧТО НЕ УТВЕРЖДАЕТСЯ: что виновато блуждание. Оно ВОССТАНОВЛЕНИЕ и включается, когда
остальное сдалось, — «где стоит время» и «кто виноват» остаются разными утверждениями.
Установлено, что мёртвое время СТОИТ здесь, и что в нём навигация лжёт о движении.

ПРОВЕРКА ПРОГРЕССА ПРИ ЭТОМ СРАБАТЫВАЕТ: `wanderChk=5373/41` — 41 срыв за прогон. То есть
защита есть и трогается, но прогон всё равно умирает здесь. Следующий замер должен спросить,
ЧТО происходит после срыва: перевыбор цели, сброс навигации, или ничего.

<!-- DEAD-TIME-IS-WANDER-2026-08-29 -->
## ⭐⭐⭐ МЁРТВОЕ ВРЕМЯ ПРИПИСАНО: ДВЕ ТРЕТИ ЕГО — БЛУЖДАНИЕ (2026-08-29)

Сделано РАЗБОРОМ УЖЕ СНЯТЫХ ЛОГОВ, без стенда и без правки кода: `gamer_smoke` печатает
строку `TASK` рядом с каждым образцом позиции, так что каждый мёртвый образец можно
приписать самой глубокой активной подзадаче. Инструмент — `deploy/runner/deadtime_by_task.py`.

```
новая сборка,  6 прогонов:  мёртвых 32%   блуждание 44+21+2 = 67% мёртвого времени
старая сборка, свод 2:      мёртвых 40%   блуждание 66+5    = 70%
старая сборка, свод 1:      мёртвых 52%   блуждание 27+16   = 44%
```

ДЕРЖИТСЯ НА ОБЕИХ СБОРКАХ И НА 18 ПРОГОНАХ. Починка проводника уронила общий простой
(52/40 -> 32%), но НЕ тронула его состав: блуждание как было главным потребителем, так и
осталось.

ЧТО ЭТО ЗНАЧИТ. Блуждание — это ВОССТАНОВЛЕНИЕ, оно включается, когда что-то другое сдалось.
И бот стоит ИМЕННО В НЁМ. То есть механизм, задача которого — расклинить бота, сам и есть
место, где проход теряет время. Вся эта сессия (и предыдущие) копала в пасфайндере, а две
трети простоя лежали здесь.

ОГОВОРКИ, БЕЗ КОТОРЫХ ЧИСЛО НЕЛЬЗЯ ЦИТИРОВАТЬ:
- образцов мало (52 мёртвых на 161 в новом своде), и они идут примерно раз в 22 с;
- 17% мёртвого времени приписано «(нет строки задачи)» — дыра в атрибуции, не ноль;
- блуждание запускается ровно тогда, когда остальное не смогло, поэтому «где стоит время»
  и «кто виноват» — РАЗНЫЕ утверждения. Установлено первое.

СЛЕДУЮЩИЙ ЗАХОД: `Wander for Infinity blocks` — это `TimeoutWanderTask(POSITIVE_INFINITY)`,
та самая конструкция, из которой 2026-08-25 родилась цель-NaN. Спросить у неё счётчиками,
что она делает в мёртвых окнах: выбирает цель и не идёт, идёт и возвращается, или ей
отказывают (`wanderDenied=1452/0` в одном прогоне — число крупное и уже подозрительное).

<!-- NO-SHAPE-AND-A-REDIRECT-2026-08-29 -->
## ⭐⭐ У НЕПРОЙДЕННОГО ШАГА НЕТ ФОРМЫ, И ПРОСТОЙ ИДЁТ УЖЕ НЕ ОТ ПАСФАЙНДЕРА (2026-08-29)

Свод на 6 прогонов, гистограмма форм собрана ПО МНОГИМ прогонам, а не по одному дампу:

```
63 непройденных шага, 31 РАЗЛИЧНАЯ форма
самая частая  hop[0,0,0]   14%
hop[0,1,-1]   (вода/leaf_litter)  8 штук, 12.7%
```

Комментарий у `noteGuideVsPhysics` ставит развилку прямо: есть ли у шага ФОРМА (тогда это
нехватка типа хода) или нога встаёт каждый раз в новом месте (тогда это другой дефект с
другой починкой). При 31 форме на 63 случая и максимуме 14% — ФОРМЫ НЕТ.

ВЕРСИЯ ПРО `leaf_litter` НЕ ПОДТВЕРДИЛАСЬ. Её концентрация `0,1,-1x123` была ОДНИМ
заклиниванием в ОДНОМ прогоне; на n=63 та же форма даёт 12.7% и ничем не выделяется. Это
ровно та ошибка, от которой предостерегает правило пять, и здесь она поймана до того, как
на ней что-то построили.

И ГЛАВНОЕ — РАЗВОРОТ НАПРАВЛЕНИЯ. В тех же шести прогонах:

```
простой  44%, 33%, 19%, 62%, 19%, 19%   медиана ~26%
остановок физической ноги за ВСЕ шесть прогонов:  63
```

Проход теряет четверть времени, а нога останавливается 63 раза на шесть прогонов. ЗНАЧИТ
ПРОСТОЙ ИДЁТ УЖЕ НЕ ОТ ПАСФАЙНДЕРА. Починка проводника своё сделала (симптом 87% -> 3%), и
дальше копать в пасфайндере — копать не там.

СЛЕДУЮЩИЙ ЗАХОД: назвать, ЧТО потребляет мёртвое время теперь. Мерить не догадкой, а
разбором: у `deadtime.py` есть окна простоя — сопоставить их с тем, какая ЗАДАЧА и какая
ЦЕПЬ в эти моменты (в дампе уже печатаются `chains=[...]`, `TASK`, `RUNNER`), и получить
распределение мёртвого времени ПО ЗАДАЧАМ. Без этого любой следующий флаг снова окажется
правкой последнего шага пути, которого не происходит.

<!-- NEXT-BINDING-CONSTRAINT-2026-08-29 -->
## ⭐⭐ УЗКОЕ МЕСТО СМЕСТИЛОСЬ: НЕ «МАРШРУТ В НИКУДА», А «НЕ УШЁЛ С ПЕРВОГО ШАГА» (2026-08-29)

После починки арифметики дампы застоя читаются иначе. Проводник ЗДОРОВ (`tiny0`), а нога
стоит на старте:

```
stall_run3_ladder   gvp=129/tiny0/start123/end4
stall_run1          gvp=38/tiny2/start10/end6
stall_run1_ladder   gvp=20/tiny0/start6/end2
```

`start` — остановки, где нога не ушла дальше первого хопа. Он преобладает над `end` почти
везде. ЭТО и есть следующая цель: не «куда ведёт маршрут», а «почему тело не делает первый
шаг по здоровому маршруту».

ЗАЦЕПКА, НО НЕ НАХОДКА — ОДИН ПРОГОН, ОДНО МЕСТО. В `stall_run3_ladder` 123 из 129
остановок имеют ОДНУ форму непройденного шага:

```
gvpHop[0,1,-1x123 ...]
ranOut n3 idx1 bot(613.5,62.0,-535.5) tgt(614.1,63.0,-537.6) endToTgt0.5
hop[0,1,-1] from(613,62,-536)[dirt|water|air] to(613,63,-537)[grass_block|leaf_litter|air]
```

Бот стоит В ВОДЕ и должен подняться на блок вверх на берег, в клетке которого лежит
`leaf_litter`. 123 попытки, ни одной удачной. НО в остальных пяти дампах формы РАССЕЯНЫ
(`1,0,0x6 1,-1,-1x5 0,-2,-1x5 ...`), доминирующей нет. То есть это ЗАКЛИНИВАНИЕ НА ОДНОМ
МЕСТЕ в одном прогоне, а не общая причина — ровно тот случай, про который предупреждает
правило 4a3 (местность-защёлка, которую бот потом сам раскапывает).

ЧТО МЕРИТЬ СЛЕДУЮЩИМ, ЧТОБЫ НЕ ПОВТОРИТЬ ОШИБКУ ОДНОГО ДАМПА: собрать формы непройденного
шага ПО МНОГИМ прогонам и спросить, есть ли у них ФОРМА (правило пять: один дамп — образец,
не вывод). `gvpHopShapes` уже это копит; нужен свод и разбор гистограммы, а не чтение
одного файла. Отдельно проверить `leaf_litter`: у него есть коллизия, и подъём из воды на
клетку с ним — правдоподобный отказ физ-модели, но пока это ВЕРСИЯ.

<!-- WALL-ALREADY-AIR-CLOSED-2026-08-29 -->
## ✅ ЗАКРЫТО ЧИСЛОМ: ЗАСТОЙ «КОПАТЬ НЕЧЕГО, А ЯРЛЫК СРАБАТЫВАЕТ» НЕ ПРОИСХОДИТ (2026-08-29)

Счётчик развязан от флага, три прогона: `wallAlreadyAir=0`, `wallAlreadyAir=0`,
`wallAlreadyAir=0`. Описанный в комментарии застой (все запланированные блоки уже воздух →
ярлык срабатывает вхолостую → никто не проводит тело сквозь дыру) в этих прогонах НЕ
наступает. Флаг `wallShortcutNeedsAWall` включать не за чем — у него нет материала.
Линия закрыта ЧИСЛОМ, а не оставлена подозрением.

## ⭐⭐⭐ СВОДНОЕ ЧИСЛО СЕССИИ: СИМПТОМ С 87% ДО 3%

`gvpTinyGuide` — доля остановок физической ноги, где проводник вёл в никуда:

```
до    (сломанная арифметика, 12 прогонов):  290 остановок, 251 по огрызку  (87%)
после (правка в дефолте,      6 прогонов):  189 остановок,   5 по огрызку  ( 3%)
```

⛔ ОГОВОРКА, КОТОРУЮ ДЕРЖАТЬ ВМЕСТЕ С ЧИСЛОМ: это сравнение ДО/ПОСЛЕ между СБОРКАМИ, а не
парный замер — местность и время не уравнены. Парное число — выход самого грубого поиска на
одной земле, 3 прогона на ветку: 2/0/0 против 96/89/109.

РЕЛИЗ 0.93.0 ПОДГОТОВЛЕН: `docs/releases/0.93.0.md` написан, `mod_version` поднят. САМО
ТЕГИРОВАНИЕ НЕ ВЫПОЛНЕНО — `gradlew :1.21.11:githubRelease` публикует на GitHub, а в
контейнере нет учётных данных (по той же причине не сделан push). Остаётся за Windows-стороной:
`git push` и `gradlew :1.21.11:githubRelease`, затем проверить
`gh release view v0.93.0 --json assets` на наличие `unionclef-1.21.11-0.93.0.jar`.

<!-- FAST-HALF-IS-BY-DESIGN-2026-08-29 -->
## ⭐⭐ ПОЛОВИНА FAST: РАЗРЕЗ УСТАНОВЛЕН НА 95%, НО ЭТО НЕ ЗАСТОЙ (2026-08-29)

Привязка сделана НАПРЯМУЮ, а не корреляцией чисел: `truncateAtBreaks` и `noteGuideSource`
идут подряд в ОДНОМ потоке, так что ThreadLocal привязывает каждый маршрут к его причине.

```
run1  fastNowhere=6/11      (из-за разреза / по другой причине)
run2  fastNowhere=12/1
run3  fastNowhere=214/0
                       итого 232 из 244 = 95% — разрез
```

Версия из записи выше УСТАНОВЛЕНА: маршрут, чей первый шаг требует копки, возвращается как
одна стартовая клетка. (Раньше она держалась на совпадении чисел 120/121 и 27/28 — это было
слабое основание, и в третьем прогоне совпадение не выдерживалось.)

НО ВЫВОД ИЗ ЭТОГО — НЕ ТОТ, К КОТОРОМУ Я ШЁЛ. В тех же прогонах `gvp=0/tiny0`, `0/tiny0`,
`14/tiny0`: крошечных проводников, доходящих до физической ноги и её останавливающих, БОЛЬШЕ
НЕТ. После починки арифметики разрез уходит в передачу на копку — задуманное поведение
«обрежь у стены → копай → перепланируй», а не застой. Мой ярлык «в никуда» приписывал их
дефекту зря; счётчик, который говорит о вреде, — это `gvpTinyGuide`, и он ноль.

ЧТО ЗАТО ВСКРЫЛОСЬ РЯДОМ — ТРЕТИЙ ОСЛЕПЛЁННЫЙ СЧЁТЧИК ЗА СЕССИЮ. В том же обработчике
записан известный застой: если все запланированные блоки УЖЕ воздух, ярлык срабатывает
вхолостую и «ничего не проводит тело сквозь дыру, которую оно только что проделало». Защита
`wallShortcutNeedsAWall` на стенде ВЫКЛЮЧЕНА, а счётчик отказов стоял ПОД ЭТИМ ЖЕ ФЛАГОМ:

```java
if (!wallStillThere && ... && TungstenConfig.get().wallShortcutNeedsAWall) wallSkipRefused++;
```

То есть `wallSkipRefused=0` не значит «случая нет» — счётчик не мог сработать. Ровно та же
слепота, что у `bsStub`. Развязан: `wallAlreadyAir` считает СЛУЧАЙ безусловно, `wallSkipRefused`
— отказ. Контрольная ветка теперь сама скажет, сколько материала у флага есть.

ПРАВИЛО, КОТОРОЕ ИЗ ЭТОГО СЛЕДУЕТ И КОТОРОЕ СТОИТ ДЕРЖАТЬ: СЧЁТЧИК МЕХАНИЗМА НИКОГДА НЕ
СТАВИТЬ ПОД ФЛАГ, КОТОРЫЙ ОН ИЗМЕРЯЕТ. Три раза за одну сессию: `bsStub` (недостижим за
ранним выходом), `wallSkipRefused` (под своим же флагом), и мой собственный первый
`guideFromCoarseNowhere` (мерил проводник, не спрашивая про цель).

<!-- COARSE-DISTANCE-AUDIT-2026-08-29 -->
## ✅ АУДИТ ВКЛЮЧЁННОГО ДЕФОЛТА ПРОЙДЕН, И ОДНА МОЯ ФОРМУЛИРОВКА ИСПРАВЛЕНА (2026-08-29)

Полный nav на включённом по умолчанию флаге: 26/28. Два провала — `nav_ladder` и
`nav_notch` — проверены прямым A/B по 3 прогона на ветку: **12/12, оба курса зелёные в
ОБЕИХ ветках**. То есть это флаки стенда, а не регрессия правки. Дефолт остаётся включённым.

ПОПРАВКА К ПРЕДЫДУЩЕЙ ЗАПИСИ. Я написал, что течь через `exhaustedPartial` работает «уже
после починки формулы». Это неточно: те 17 маршрутов пришли из прогона, где был включён
`smartMoves`, а ЕГО ветка `bestSoFar` гейтит по `bd > 1.0` — один блок вместо
`MIN_DIST_PATH` = 5. В поставляемой конфигурации (арифметика без `smartMoves`) работает
ЛЕГАСИ-ветка с порогом в пять блоков, и в сводах по одной арифметике `coarseNowhereBy`
читается `[-]`, `[-]`, `[stalledx2]`. Эта течь принадлежит ветке `smartMoves`, которая
выключена.

НАСТОЯЩИЙ ОСТАТОК — ПОЛОВИНА FAST. В ветке с правкой `guideSrc` даёт 16 из 30 и 18 из 29
маршрутов в никуда со стороны FastPlanner, при почти нулевой доле у грубого поиска. Это и
есть следующий заход: версия про `truncateAtBreaks` (разрез `subList(0, max(i,1))`
оставляет ОДИН узел, когда первый же шаг требует копки) совпала с числом «в никуда» у FAST
в двух прогонах из трёх — 120 против 121 и 27 против 28, но 4 против 8 в третьем.
Сильная, НЕ установленная.

<!-- COARSE-DISTANCE-SHIPPED-2026-08-29 -->
## ⭐⭐⭐ ПОЧИНКА АРИФМЕТИКИ ГРУБОГО ПОИСКА ВКЛЮЧЕНА ПО УМОЛЧАНИЮ (2026-08-29)

`coarseDistanceIsADistance = true`. ОСНОВАНИЕ — КОРРЕКТНОСТЬ ПЛЮС ОТСУТСТВИЕ РЕГРЕССИИ,
А НЕ ВЗЯТАЯ ПЛАНКА ПО ПРОСТОЮ. Формулировать честно, чтобы флип не подразумевал большего.

1. АРИФМЕТИКА ПРОСТО НЕВЕРНА: `start.x` не может стоять в членах по Y и Z. Никакая
   изменчивость прохода этого не подтверждает и не опровергает.
2. МЕХАНИЗМ, 3 прогона на ветку: маршруты в никуда от грубого поиска 96/89/109 за прогон
   падают до 2/0/0. `bsStub` в контроле читается нулём ПО ПОСТРОЕНИЮ — он и не может
   сработать, пока формула сломана, и это же было доказательством диагноза.
3. РЕГРЕССИЯ, О КОТОРОЙ ПРЕДУПРЕЖДАЛ КОММЕНТАРИЙ, НЕ ВОСПРОИЗВОДИТСЯ для этой половины
   рычага. nav, перемежающийся A/B, 3 прогона на ветку:

```
   ветка с правкой  18 PASS / 0 FAIL
   контроль         16 PASS / 2 FAIL
   nav_staircase (он же «course A», названный как живущий на мусорных путях)  3/3 В ОБЕИХ
```

   Оба провала выпали КОНТРОЛЮ (nav_bridge, nav_notch).
4. ПРОХОД НЕ УСТАНОВЛЕН: 8 информативных пар, простой -29 с/прогон, t ≈ -0.6. Ступени
   взяли планку в одном своде из трёх, и ЭТО ЧИСЛО ЗДЕСЬ НЕ КОТИРУЕТСЯ: правило 4a2 велит
   судить правку застоя по простою, а правило 4a3 называет направленное смещение в пользу
   ветки, идущей ПЕРВОЙ, — а первой идёт ветка с правкой. В том своде оба контрольных
   провала имели `items gained: 0` на поляне, которую ветка с правкой перед этим вырубила.

ЧТО ОСТАЁТСЯ НЕЗАКРЫТЫМ И НАЗВАНО: (а) вторая, меньшая течь — выход `exhaustedPartial`,
который возвращается МИМО `salvage()`; в одном прогоне он дал 17 маршрутов в никуда уже
после починки формулы. (б) `smartMoves` как связка остаётся ВЫКЛЮЧЕННОЙ: его вторая
половина (другая ветка `bestSoFar`) стоила ступеней и челнока, и это отдельный вопрос.
(в) Половина «в никуда» у FAST-производителя объяснена разрезом `truncateAtBreaks` в двух
прогонах из трёх — версия сильная, но НЕ установленная.

<!-- COARSE-DISTANCE-ONLY-2026-08-29 -->
## ⏳ ТОЛЬКО АРИФМЕТИКА: МЕХАНИЗМ ОГРОМЕН, ИСХОД ПОКА НЕ ВЗЯТ (2026-08-29, идёт добор пар)

Флаг `coarseDistanceIsADistance` покупает ТОЛЬКО починку `getDistFromStartSq`, без второй
половины `smartMoves`. Два свода, 5 зачтённых пар.

МЕХАНИЗМ — САМЫЙ ЧИСТЫЙ ЗА ВСЮ ЛИНИЮ. Маршруты в никуда от грубого поиска:

```
ветка с правкой (ON)    2, 0, 0        bsStub=1/1, 1/0, 4/4
контроль       (OFF)   96, 89, 109     bsStub=0/0, 0/0, 0/0
```

Со ста маршрутов в никуда за прогон до нуля, при контроле, читающемся нулём по построению.
`bsStub` в ветке с правкой мал ИМЕННО ПОТОМУ, что правка работает: с верной формулой гейт
`bestSoFar` перестаёт пропускать негодные пути, и спасать нечего.

ИСХОД ПО ПРОСТОЮ (правило 4a2), по парам:

```
-19, -89, -66, +47, -63   →  -38 с/прогон,  t ≈ -1.6   НЕ УСТАНОВЛЕНО
```

Четыре пары из пяти за правку, одна против. Направление устойчивое, планка 2σ НЕ взята.
Флаг остаётся ВЫКЛЮЧЕННЫМ. Идёт третий свод; решение — по нему.

⛔ ЛОВУШКА ПРИ СЧЁТЕ, НА КОТОРУЮ Я ЧУТЬ НЕ СЕЛ: склеивать логи двух сводов для
`paired_ab.py` МОЖНО ТОЛЬКО если земли не повторяются. Здесь земля `-308 150 -239` есть в
обоих, и склейка спарила ветку одного свода с контролем другого — числа менялись на глазах
(135s против 112s, механизм 4 против 20). Своды с общей землёй считать ПОРОЗНЬ.

<!-- SMARTMOVES-BUNDLE-2026-08-29 -->
## ⛔ ДИАГНОЗ ПОДТВЕРЖДЁН СЧЁТЧИКОМ, НО РЫЧАГ `smartMoves` ЦЕЛИКОМ — НЕ ВЫИГРЫШ (2026-08-29)

Парный A/B, 3 пары, ВСЕ информативные. Механизм — `bsStub`, который по построению
недостижим, пока формула сломана:

```
ветка с правкой (ON)   bsStub=112/24   30/19   117/3
контроль       (OFF)   bsStub=0/0      0/0     0/0
```

ЭТО И ЕСТЬ ДОКАЗАТЕЛЬСТВО ДИАГНОЗА: счётчик оживает ровно тогда, когда меняется формула.
Маршруты в никуда от грубого поиска при этом падают: ON 0/17/0 против OFF 127/6/29.

НО ИСХОД НЕ ВЗЯТ, И ПО ДВУМ ПОБОЧНЫМ МЕРАМ ХУЖЕ:

```
земля              ступени  простой ON       простой OFF     механизм
1492 150 -1139      4/5      22s/582s ( 4%)  283s/582s (49%)  112/0
1492 150 -539       0/3      45s/583s ( 8%)   64s/582s (11%)   30/0
1492 150 61         5/5      86s/581s (15%)   21s/582s ( 4%)  117/0

простой  -71.7s/прогон  sd 169  t -0.73   НЕ УСТАНОВЛЕНО
ступени  -1.33/прогон           t -1.51   НЕ УСТАНОВЛЕНО, знак ПРОТИВ правки
челнок   ON 5/23, 11/22, 8/22   OFF 0/11, 6/19, 1/22   — с правкой ХУЖЕ
```

Это ровно то, о чём предупреждал комментарий в файле: «correcting it alone regresses the
blind-scan search». Флаг `smartMoves` остаётся ВЫКЛЮЧЕННЫМ.

ПОЧЕМУ НЕ ГНАТЬ ЕЩЁ ПАР, А РАЗДЕЛИТЬ РЫЧАГ. `smartMoves` связывает ДВЕ вещи: починку
арифметики (однозначно верную — `start.x` не может стоять в членах по Y и Z) и другую ветку
`bestSoFar` (спорную, и именно она правдоподобно стоит ступеней). Добавлять пары к
связанному рычагу — значит уточнять сумму двух эффектов разного знака. Заведён флаг
`coarseDistanceIsADistance`, который покупает ТОЛЬКО арифметику; он и меряется следующим.

ОСТАТОК, ВИДНЫЙ УЖЕ СЕЙЧАС: в ветке с правкой один прогон дал 17 маршрутов в никуда через
`exhaustedPartial` — тот самый выход, который ОБХОДИТ `salvage()`. То есть после починки
арифметики останется вторая, меньшая течь, и она уже названа.

<!-- DIST-FROM-START-IS-NOT-A-DISTANCE-2026-08-29 -->
## ⭐⭐⭐ КОРЕНЬ: «РАССТОЯНИЕ ОТ СТАРТА» В ГРУБОМ ПОИСКЕ — НЕ РАССТОЯНИЕ (2026-08-29)

Обе загадки — почему наружу идут маршруты в никуда И почему `bsStub` читается нулём —
объясняются ОДНОЙ функцией.

```java
private static double getDistFromStartSq(BlockNode n, Vec3d start) {
    boolean fix = TungstenConfig.get().smartMoves;      // на стенде FALSE
    double xDiff = start.x - n.getPos().x;
    double yDiff = (fix ? start.y : start.x) - n.getPos().y;   // start.X минус узел.Y
    double zDiff = (fix ? start.z : start.x) - n.getPos().z;   // start.X минус узел.Z
    return xDiff*xDiff + yDiff*yDiff + zDiff*zDiff;
}
```

Опечатка известна и НАМЕРЕННО оставлена под флагом («correcting it alone regresses the
blind-scan search»). Но `smartMoves` на стенде выключен, то есть живёт именно она.

СЛЕДСТВИЕ ПЕРВОЕ — ГЕЙТ, КОТОРЫЙ ВСЕГДА ОТКРЫТ. `bestSoFar` обязан отдавать путь только
при продвижении больше MIN_DIST_PATH (в этом классе = 5). Сравнивает он с этой величиной.
У бота на x=1792 член `start.x - n.y` порядка 1700, квадрат — миллионы: гейт проходит
ВСЕГДА, независимо от реального продвижения. Так «частичный путь», никуда не ведущий, и
попадает к физической ноге. Гарантия в комментарии выхода по исчерпанию («bestSoFar only
returns a path when there's real forward progress») НЕ ВЫПОЛНЯЕТСЯ.

СЛЕДСТВИЕ ВТОРОЕ — СЧЁТЧИК, КОТОРЫЙ НЕ МОЖЕТ СРАБОТАТЬ. `salvage()` первой строкой берёт
`movedSq` из той же функции и делает ранний выход при `movedSq > MIN_DIST_PATH*MIN_DIST_PATH`.
Число огромное, выход срабатывает всегда, `bsStub++` НЕДОСТИЖИМ. Отсюда `bsStub=0/0` во
всех прогонах — и отсюда же ошибочный вывод записи от 2026-08-28, что огрызков нет.

ЧЕМ ЭТО ИЗМЕРЕНО (3 прогона, счётчик по выходам):

```
run1  coarseNowhereBy[stalledx11]   из 11
run2  coarseNowhereBy[stalledx20]   из 20
run3  coarseNowhereBy[stalledx17]   из 17
```

48 из 48 — выход `stalled`, ноль через `exhaustedPartial`. Это ОПРОВЕРГЛО мою же догадку,
что `bsStub` слеп из-за обхода `salvage()`: `stalled` идёт ЧЕРЕЗ salvage. Слеп он из-за
арифметики внутри, а не из-за маршрута исполнения.

ЗАВИСИМОСТЬ ОТ КООРДИНАТ, КОТОРУЮ ЭТО ПРЕДСКАЗЫВАЕТ И КОТОРУЮ НАДО ПРОВЕРИТЬ: искажение
растёт с |start.x|. Свод шёл на x = -8, 892, 1192, 1792 — везде далеко от нуля. У базы
координаты малы, и там дефект должен быть слабее. Это проверяемое предсказание, а не
объяснение задним числом.

ПРАВИТЬ НЕ ВСЛЕПУЮ. Комментарий утверждает, что одиночная починка ломает слепой скан
(курс A живёт на этих мусорных частичных путях). Рычаг уже есть — `smartMoves`, который
включает и корректную формулу, и другую ветку `bestSoFar`. Следующий заход: парный A/B
`smartMoves=true` по ПРОСТОЮ, счётчик механизма — `bsStub` (в контроле он обязан быть 0
по построению, в ветке с правкой обязан ожить: это и есть проверка, что формула сменилась).

<!-- GUIDE-STUB-SPLIT-2026-08-29 -->
## ⛔ ОГРЫЗОК ПРОВОДНИКА — ЭТО ДВА РАЗНЫХ ДЕФЕКТА, И ОДНА МОЯ ОШИБКА ИЗМЕРЕНИЯ (2026-08-29)

Счётчик `guideSrc` поставлен, снят и ПЕРЕДЕЛАН, потому что первая версия мерила не то.

ОШИБКА В МОЁМ СОБСТВЕННОМ СЧЁТЧИКЕ. Первая версия спрашивала только длину проводника
(порог 1.8) и НЕ спрашивала, далеко ли цель. Проводник в полблока к цели в полблока — это
верный план, а не огрызок, и в профиле застоя таких много. Числа «coarse 91-99% в никуда»
из первого захода ЗАВЫШЕНЫ и не годятся ни для какого вывода. Это ровно та ошибка, за
которую я двумя часами раньше критиковал `fast.path.size() >= 2`: мерить проводник, не
спрашивая, куда он должен был вести.

И ЛОВУШКА, НА КОТОРОЙ Я НА НЕЁ СЕЛ: `MIN_DIST_PATH` — ДВЕ РАЗНЫЕ КОНСТАНТЫ С ОДНИМ ИМЕНЕМ.
`PathFinder.MIN_DIST_PATH = 1.8`, `BlockSpacePathFinder.MIN_DIST_PATH = 5`. Гейт `bestSoFar`
работает по пятёрке, мой счётчик мерил по 1.8. Ссылаясь на это имя, ВСЕГДА называть класс.

ГИПОТЕЗА `truncateAtBreaks` НАЗВАНА ДО ЗАМЕРА И ОПРОВЕРГНУТА В ЭТОМ ВИДЕ. Она гласила:
разрез `subList(0, max(i,1))` на первом же шаге, требующем копки, оставляет ОДИН узел, и
это объясняет всё. Прогон с `trunc=1/0` и 32 маршрутами в никуда её в общем виде убил.

ЧТО ГОВОРЯТ ЧИСЛА ПОСЛЕ ПОЧИНКИ СЧЁТЧИКА (3 прогона):

```
        fast всего/в никуда   coarse всего/в никуда   разрезов/до одного узла
run1          25/8                  43/25                   15/4
run2         155/121                96/71                  146/120
run3          34/28                 53/49                   31/27
```

ДЕФЕКТ РАСПАДАЕТСЯ НАДВОЕ:

1. У FAST «в никуда» почти точно равно числу разрезов до одного узла: 120 против 121 и
   27 против 28. В run1 — 4 против 8, вдвое меньше. То есть совпадение в двух прогонах
   из трёх: ВЕРСИЯ СИЛЬНАЯ, НО НЕ УСТАНОВЛЕННАЯ. Механизм правдоподобен и назван:
   маршрут, чей первый шаг требует копки или моста, возвращается как одна стартовая
   клетка вместо «план копки плюс проводник, по которому есть куда идти».
2. У COARSE (71, 49, 25 в никуда) разрез не объясняет НИЧЕГО, и `bsStub` читается нулём
   во всех прогонах. Это отдельная, ещё не названная причина.

СЛЕДУЮЩИЙ ЗАМЕР: разделить `guideFromCoarseNowhere` ПО ВЫХОДУ грубого поиска. Выход по
исчерпанию (`partial` из `bestSoFar`) возвращается МИМО `salvage()` — а `bsStub++` живёт
внутри `salvage()`. Ноль счётчика означает «мой выход молчит», и здесь это буквально так.

<!-- GUIDE-IS-A-STUB-2026-08-28 -->
## ⭐⭐⭐ ПОЧЕМУ ПРИВОД НЕ ДОВОДИТ: ПРОВОДНИК ВЕДЁТ ИЗ КЛЕТКИ В НЕЁ ЖЕ (2026-08-28)

Замер по логам двух сводов выше (12 прогонов), без единого нового прогона — данные уже
были собраны, читать их никто не пробовал. 78 строк `guide halts`, 69 различных ситуаций:

```
проводник короче 2 БЛОКОВ (ведёт в никуда)      59/69
из них два узла в ОДНОЙ И ТОЙ ЖЕ клетке          58
непройденный шаг                          hop[0,0,0] — из клетки в неё же
цель в этот момент                        медиана 32 блока
```

Десять случаев, где проводник настоящий, дают 3.9 блока к цели в 3.3 блоках. То есть
строятся ТОЛЬКО короткие локальные маршруты, а на всём остальном физическая нога получает
маршрут «иди туда, где стоишь», честно не находит прибытия и встаёт.

ЭТО ОБЪЯСНЯЕТ ЗАПИСЬ ВЫШЕ. Правка прибытия по снапу чинит последний шаг пути, которого
не происходит: `snap=6484/5188/1296` при `arrivedAtSnap=0` — пять тысяч сдвинутых целей и
ни одного случая, когда бот встал в выбранную клетку.

ДВА ОБСТОЯТЕЛЬСТВА, ИЗ-ЗА КОТОРЫХ ЭТО ИСКАЛИ ТАК ДОЛГО:

1. СЧЁТЧИК УЖЕ ЭТО ГОВОРИЛ, И ЕГО НЕ ЧИТАЛИ. `gvpTinyGuide` печатается в строке вердикта
   каждый прогон: `gvp=70/tiny68`, `44/tiny43`, `15/tiny15`. Доля «крошечных» проводников
   подавляющая во ВСЕХ двенадцати прогонах.
2. `bsStub` ЧИТАЕТСЯ НУЛЁМ ВО ВСЕХ ДВЕНАДЦАТИ. Запись от 2026-08-28 («механизм не
   срабатывает, флаг остаётся выключенным») из этого нуля заключила, что огрызков нет.
   Огрызки есть — их отдаёт НЕ ТОТ ВЫХОД, на который поставлен `bsStub`. Ноль счётчика
   означает «мой выход молчит», а не «явления нет», и это стоило захода.

ПРОИЗВОДИТЕЛЕЙ ПРОВОДНИКА РОВНО ДВА: `FastPlanner` (принимается по `fast.path.size() >= 2`
— проверка В УЗЛАХ, ровно та ошибка, про которую в `salvage()` уже написано «огрызок
меряется в блоках») и `BlockSpacePathFinder.search`. Поставлен счётчик
`guideSrc=<fast>/<fastНикуда>/<coarse>/<coarseНикуда>` — считает В БЛОКАХ, против
MIN_DIST_PATH, чистая диагностика без изменения поведения. Он и назовёт виновника.

<!-- ARRIVAL-VS-SNAP-MEASURED-2026-08-28 -->
## ⛔ ПРИБЫТИЕ ПО СНАПУ: ШЕСТЬ ПАР, ЭФФЕКТ НЕ УСТАНОВЛЕН, ФЛАГ ОСТАЁТСЯ ВЫКЛЮЧЕННЫМ (2026-08-28)

Правка из записи ниже (`arrivalAgreesWithTheSnap`) измерена двумя сводами `gamer_smoke
--pin-alt`, 12 прогонов по 10 минут, 6 пар на одной земле. Исход по ПРОСТОЮ (правило 4a2).

```
земля                ступени  простой ON        простой OFF       механизм ON/OFF
-8 150 -1439          3 /3     175s/586 s (30%)  108s/583 s (19%)    0/0
1192 150 -1439        4 /5     305s/583 s (52%)  476s/585 s (81%)  845/0
1792 150 61           5 /2      45s/585 s ( 8%)  454s/582 s (78%) 2128/0
1792 150 661          0 /4     457s/585 s (78%)   24s/580 s ( 4%)    0/0
892 150 661           4 /4     151s/581 s (26%)   21s/580 s ( 4%) 2465/0
892 150 -239          2 /4       0s/580 s ( 0%)  517s/581 s (89%)    0/0

как есть, 6 пар     простой  -77.8s/прогон  sd 357  t -0.53   НЕ УСТАНОВЛЕНО
только информативные (3)     -150s/прогон   sd 270  t -0.96   НЕ УСТАНОВЛЕНО
```

ПАРЫ РАСХОДЯТСЯ ЗНАКОМ: из трёх информативных две за правку (-171s, -409s), одна против
(+130s). Это не «почти взяли планку» — это разброс, накрывающий ноль с обеих сторон.
Флаг остаётся ВЫКЛЮЧЕННЫМ. Разница между сводами (первый -171s, второй +15s на тех же
настройках) — та же изменчивость земли, ради которой правило 4a и вводило парность.

ЧТО ПРИ ЭТОМ СДЕЛАНО ПРАВИЛЬНО: контроль читается нулём во ВСЕХ шести парах, то есть
флаг действительно был единственным различием (правило 4u). Пустых пар три из шести —
их числа приведены и не удалены (правило 4a1), но в вывод не идут.

ГЛАВНОЕ, ЧТО ВЫЯСНИЛОСЬ ПОПУТНО, И ЭТО ВАЖНЕЕ САМОГО ФЛАГА: механизм срабатывает
в 3 прогонах из 12, ХОТЯ материал есть почти везде. Крайний случай — прогон, где
`snap=6484/5188/1296` при `arrivedAtSnap=0`: пять тысяч сдвинутых целей, то есть пять
тысяч невыполнимых по построению проверок прибытия, и НИ ОДНОГО случая, когда бот
оказался в клетке, куда его вёл привод. Значит невыполнимая проверка — не то, что
держит бота: он до сдвинутой клетки ПРОСТО НЕ ДОХОДИТ. Правка чинит последний шаг
пути, которого не происходит.

СЛЕДУЮЩИЙ ЗАМЕР ОТСЮДА: не «ещё пар на этот флаг», а почему привод не приводит бота
в клетку, которую сам выбрал. Мерить расстояние от бота до `snappedGoalCell` в момент,
когда задача сдаётся или перезаказывает подход.

РИСК, ЗАПИСАННЫЙ ДО ЗАМЕРА (челнок), ПРОВЕРЕН И НЕ ПРИПИСЫВАЕТСЯ ПРАВКЕ. Челночные окна
(`shuttle.py`, net/path < 0.25) по ветке с правкой: 0/19, 0/8, 0/22, нет окон, 0/13,
4/10. Единственный челночивший прогон с правкой — из ПУСТОЙ пары (механизм 0), а в
информативной паре челночил как раз КОНТРОЛЬ (4/22 против 0/13). Челнок в этом своде
есть в обеих ветках и о флаге не говорит.

ОСТАЁТСЯ В КОДЕ: счётчик `arrivedAtSnap` (он и отделил информативные пары от пустых)
и сам флаг, выключенный, до замера, где механизм срабатывает не в четверти прогонов.

<!-- ARRIVAL-VS-SNAP-2026-08-28 -->
## ⭐⭐⭐ ПРОФИЛЬ A НАЗВАН ПОЛНОСТЬЮ: ПРИВОД И ПРОВЕРКА ПРИБЫТИЯ ЦЕЛЯТ В РАЗНЫЕ КЛЕТКИ (2026-08-28)

Счётчик, который TODOS просил с 2026-08-26 («сперва счётчик на источник цели»), поставлен
и называет источник одной строкой:
...
Одна задача, ОДИН блок, десять планов подряд в клетку, где бот уже стоит. Все совпадения
ТОЧНЫЕ — значит слои согласны, ГДЕ бот, и расходятся в том, КУДА его послали.

МЕХАНИЗМ (структурный, а не баг одной из половин):
`driveTungstenPrimary` целит в `snapGoalToStandable(goal)`, а снап двигает цель ТОЛЬКО
если в ней НЕЛЬЗЯ СТОЯТЬ — здесь 197 из 200 запросов. `isFinished()` при этом спрашивает
`AltoGoal.Block.reached`, которому нужно ЗАНЯТЬ запрошенную клетку. То есть для каждой
сдвинутой цели проверка прибытия НЕВЫПОЛНИМА ПО ПОСТРОЕНИЮ: привод ставит бота рядом с
бревном, задача говорит «не там» и просит снова каждый тик. `snapToSelf` считает только
тики, где `isFinished()` УЖЕ вернул false (его гейт стоит выше снапа) — значит эти 134
это 134 невыполнимых проверки, а не «почти дошёл».

ПРАВКА (флаг `arrivalAgreesWithTheSnap`, ПО УМОЛЧАНИЮ ВЫКЛ): запомнить клетку, которую
выбрал снап, и считать задачу законченной, когда бот в ней стоит. Пересчитывается каждый
тик (снап зависит от мира и от позиции бота). Может только ДОБАВИТЬ завершения, которые
иначе невозможны: в неопорной клетке стоять нельзя, точная проверка там не прошла бы никогда.

РИСК, записанный ДО замера: вызывающий, которому нужна именно занятость клетки, или тот,
кто заново заказывает подход, едва оказавшись вне доступа, — это превратит застой в
ЧЕЛНОК. Мерить простой И челнок (`shuttle.py`). Механизм — `arrivedAtSnap`.

## ⛔ ОГРЫЗОК ГРУБОГО ПОИСКА: МЕХАНИЗМ НЕ СРАБАТЫВАЕТ, ФЛАГ ОСТАЁТСЯ ВЫКЛЮЧЕННЫМ (2026-08-28)

Свод остановлен на второй паре, и остановлен правильно: счётчик механизма читается НУЛЁМ.

```
run1 (ветка с правкой)  bsEnd[c2  t65 s1 x0]  bsStub=0/0@0cm bsClosestUsed=0  ступеней 4
run2 (контроль)         bsEnd[c254 t38 s0 x0] bsStub=0/0@0cm bsClosestUsed=0  ступеней 5
```

103 незавершившихся поиска на два прогона — и НИ ОДИН не отдал огрызок. То есть
`bestSoFar` по тайм-ауту всё-таки отдаёт реальное перемещение (>MIN_DIST_PATH), а
пойманный в `stall_run2` случай (конец проводника в 0.3 блока от бота при цели в 29.8)
РЕДОК, а не типичен. По правилу 4a1 пара, где механизм не сработал, о флаге не говорит
ничего — гнать ещё десять таких пар значит потратить два часа стенда на ноль.

ЧТО ОСТАЁТСЯ В КОДЕ: счётчики `bsStub/bsStubHadCloser/bsStubCloserCm/bsClosestUsed`
навсегда, и почина мёртвой ветки `closestToGoal` (объявлена, прокомментирована, НИКОГДА
не присваивалась — ветка «последнего шанса» из #67 не могла сработать ни разу).
Флаг выключен и ждёт прогона, где счётчик покажет, что случай наступил.

И ОТДЕЛЬНО — ОШИБКА МОЕГО ЖЕ ГЕЙТА, пойманная ДО замера: первая версия считала огрызком
проводник из двух узлов и меньше. `stringPull()` удаляет все промежуточные клетки, чей
угол срезается, так что здоровая тридцатиблочная прямая тоже схлопывается в два узла —
и дампы это доказывают: `run2 guide=n2 bsEnd[c0 t105]` против `run3 guide=n2 bsEnd[c63 t0]`,
одинаковое число узлов, противоположное здоровье. Огрызок меряется В БЛОКАХ.

<!-- COARSE-SEARCH-STUB-2026-08-28 -->
## ⭐⭐ НАЙДЕНО: ГРУБЫЙ ПОИСК ПО ТАЙМ-АУТУ ОТДАЁТ ОГРЫЗОК (2026-08-28)

Три дампа застоя из одного свода, читать только вместе (правило пять):

```
run1  bsEnd[c24 t0   s1  x0] closest0.96m  guide=n11   грубый поиск здоров
run3  bsEnd[c63 t0   s0  x0] closest0.50m  guide=n2    здоров, цель в одном шаге
run2  bsEnd[c0  t105 s11 x0] closest18.05m guide=n2    НИ ОДИН поиск не дошёл
```

Профиль run2: 117 грубых поисков, ни один не дошёл до цели, 1.48 М узлов раскрыто,
а проводник, отданный физической ноге, — ДВА узла, чей конец в 29.8 блоках от цели.
Мелкий бюджет отказа — 1920 мс, и `failing` не снимается, пока ни один узел не отошёл
от старта на MIN_DIST_PATH, — то есть больше трёх минут десятиминутного прогона ушло на
вычисление маршрута, который никуда не ведёт, при `wander=137 wanderMoved=0`.

ПОЧЕМУ ОГРЫЗОК. Выход по тайм-ауту отдаёт `bestSoFar()` — МОНОТОННЫЙ рекорд
улучшения эвристики. Если ни один потомок не перебил затравку, рекорд остаётся на
стартовом узле, и вызывающий получает старт плюс один ход — это не маршрут. У выхода
по ИСЧЕРПАНИЮ запасной вариант есть с #67 («ближайшая достигнутая клетка»), а у
выхода по ТАЙМ-АУТУ — того самого, который берёт проход, — не было ничего.

И ЭТОТ ЗАПАСНОЙ ВАРИАНТ НИ РАЗУ НЕ РАБОТАЛ. `closestToGoal` объявлен над циклом,
прокомментирован, читается ниже — и НИГДЕ НЕ ПРИСВАИВАЕТСЯ. Всегда null, ветка
недостижима. Рассоединённый ИСПОЛНИТЕЛЬ — зеркальный случай правила один.

ПРАВКА (ФЛАГ `coarseFallsBackToClosestCell`, ПО УМОЛЧАНИЮ ВЫКЛ, НЕ ИЗМЕРЕНА):
присваивать `closestToGoal` на снятии с кучи и обслуживать ОБА выхода одним
`salvage()`. Счётчики пишутся независимо от флага, так что контрольная ветка сама скажет,
сколько материала у правки есть:

```
bsStub=<огрызков>/<из них было что-то ближе>@<самый дальний такой>cm/used<n>
```

`used` — счётчик механизма: 0 в контроле и не 0 в ветке с правкой, иначе пара ничего
не измерила (правило 4a1). ГЕЙТ: парный gamer_smoke --pin-alt по ПРОСТОЮ, не по
ступеням (правило 4a2).

ВАЖНО: режим с тайм-аутами НЕ всеобщий — два дампа из трёх показывают здоровый
грубый поиск. Это один из трёх профилей застоя, а не единственная причина.

## ⛔ ПРОФИЛЬ A НАЗВАН ЧИСЛОМ ПОКРУПНЕЕ: СНАП ЦЕЛИ САДИТСЯ НА БОТА (2026-08-28)

В том же своде, run3 (грубый поиск там ЗДОРОВ, то есть это ДРУГОЙ профиль):

```
snap=944/809/135/self747      79% снапов цели сели НА КЛЕТКУ САМОГО БОТА
plan=98/8256/254ms/sz25/zero61(...,atGoal=61(ex61,ytol0)@block(1480,60,30))
                              62% планов — маршрут в клетку, где бот уже стоит
```

Запись от 2026-08-26 давала 35% и требовала «счётчик на источник цели». Счётчик есть
и он называет источник: `CustomBaritoneGoalTask.snapGoalToStandable`. НО НЕ СПЕШИТЬ С
ВЫВОДОМ — «снап сел на бота» может означать и «бот УЖЕ стоит где надо, а задача
всё равно заказывает ходьбу вместо действия», и это совсем другая правка. Сначала
набрать n по этим двум числам по всем прогонам ближайшего свода, потом заход.

<!-- ROOT-FIXED-NAN-TARGET-2026-08-25 -->
## ⭐ КОРЕНЬ ЗАСТОЯ НАЙДЕН И УСТРАНЁН: ЦЕЛЬ ПОИСКА БЫЛА NaN (2026-08-25)

    agent[0.5,-60.0,0.5] -> tgt[Infinity,-60.0,NaN]   d=NaN

Цель, передаваемая физическому поиску, была БЕСКОНЕЧНОСТЬЮ по X и NaN по Z. Сравнение с NaN
всегда ложно, поэтому `isPathComplete` не могла сработать НИКОГДА — поиск разворачивал до
полутора миллионов узлов, не находил прибытия, исчерпывался и возвращал молчание.

ОТКУДА: `TimeoutWanderTask(Float.POSITIVE_INFINITY)` = "блуждать без предела", нормальная
конструкция. Мой порт блуждания (вырезка баритона) считал из этого радиуса точку:
r = Infinity, а `0 * Infinity = NaN`. Ошибка МОЯ, внесена при G-0.

ЭТО ОБЪЯСНЯЛО РАЗОМ: застой на 1219.5,104.1,-843.5, 135 добычей без шага, wanderMoved=0.0.

РЕЗУЛЬТАТ:
    nav    14/14, отказов 0
    craft  22/22, отказов 0   (было 15/22)
    проход 2/2 PASS, до УГЛЯ и ЕДЫ
    wander_recovery  0.0 -> 20.5 блока

## ⛔ ЗАКРЫТО: РАСХОЖДЕНИЯ СЛОЁВ ПО ЛИСТВЕ НЕТ (2026-08-25)

Проверил, как каждый слой относится к spruce_leaves, по которым идёт вся линия подхода:

    MovementHelperB.mustBeSolidToWalkOn -> LeavesBlock: return TRUE

То есть блок-спейс считает листву ОПОРОЙ, и это совпадает с ванилью (у листвы есть
коллизия) и с физическим слоем. Версия "один слой считает листву проходимой, другой нет"
СНЯТА.

СОСТОЯНИЕ ОХОТЫ НА 1219 (всё измерено, ничего не додумано):

    goal = 99517/1132/agent[1219.5,104.0,-842.5]->tgt[1205.5,104.0,-839.5] d14.3
    snap = 1938/1938/0     привязка цели работает, отказов нет
    tryEmit = 0/0          прибытие ни разу не засчитано
    ближайшее приближение  11.3 блока из 14.3

Закрыты числами: NaN-цель (ПОЧИНЕНА), ширина перебора (три точки), допуск прибытия,
невалидная цель, расхождение слоёв по листве.

СЛЕДУЮЩИЙ ЗАМЕР: выгрузить блок-путь, который построил грубый поиск, и сравнить с точкой,
где встаёт физика. Это скажет, ведёт ли проводник туда, куда физика физически не может
шагнуть, — единственная оставшаяся версия.

## ⛔ ОТЗЫВ: "У ЦЕЛИ НЕТ ПОЛА" — Я ЩУПАЛ НЕ ТУ ЛИНИЮ (2026-08-25)

Ниже записано, что у цели пол = воздух. ЭТО НЕВЕРНО. Я читал блоки по z=-841, а цель стоит
на z=-839. Промах в две клетки, вывод построен на нём.

НАСТОЯЩАЯ КАРТИНА в целевой клетке (1205,104,-839):

    y=105 воздух      y=104 СНЕГ      y=103 spruce_leaves

и вся линия подхода при z=-839 — сплошная spruce_leaves.

То есть опора у цели ЕСТЬ, но это ЛИСТВА, а сама клетка занята снегом. Бот идёт к точке
ВНУТРИ КРОНЫ, и подход идёт сквозь листву.

И привязка цели РАБОТАЕТ: snap=1938/1938/0 — сдвигает цель к стоячей клетке каждый раз,
отказов ноль. То есть версия "цель невалидна и никто не проверяет" тоже снята.

ЧТО ОСТАЁТСЯ ИЗМЕРИТЬ: почему поиск не проходит сквозь/по листве. Листва имеет коллизию,
но проходима при разрушении; в блок-спейсе она может считаться проходимой, а в физике нет —
это расхождение двух слоёв и следующая цель замера.

## ⭐ ЗАСТОЙ НА 1219 ОБЪЯСНЁН: ЦЕЛЬ НЕВАЛИДНА, ПОИСК ПРАВ (2026-08-25)

Прочитал местность по линии бот(1219) -> цель(1205), z=-841:

    x=1219  пол=snow_block        бот стоит
    x=1217  НОГИ = ПЕЧЬ           его собственная печь перегораживает
    x=1215  ноги=spruce_leaves, пол=ВОЗДУХ    крона над пустотой
    x=1213  ноги=spruce_leaves, пол=ВОЗДУХ
    x=1211  пол=snow
    x=1209  пол=spruce_leaves
    x=1207  пол=snow
    x=1205  пол=ВОЗДУХ            У ЦЕЛИ НЕТ ОПОРЫ

Бот идёт к точке, под которой НЕТ ПОЛА, через кроны над пустотой, а на выходе стоит его же
печь. Поиск честно не находит маршрут туда, где стоять нельзя, и встаёт за 11 блоков — ровно
там, где кончается проходимая земля. ПОИСК ВЕДЁТ СЕБЯ ПРАВИЛЬНО.

⛔ ЗНАЧИТ ДЕФЕКТ ВЫШЕ ПО ТЕЧЕНИЮ: цель выбрана невалидно. Задача переформулируется —
не "почему поиск не доходит", а "почему целью назначена клетка без опоры, и почему это не
отбраковывается ДО девяти миллионов развёрнутых узлов".

В коде уже есть проверка "цель не стоячая" (комментарий про goal cell that isn't standable
в CustomBaritoneGoalTask) — надо измерить, срабатывает ли она здесь.

## ⛔ ЗАСТОЙ НА 1219: ПОИСК ЗДОРОВ, НО НЕ ДОХОДИТ 11 БЛОКОВ (2026-08-25)

После починки NaN-цели тот же репро читается уже как НОРМАЛЬНЫЙ поиск:

    goal = 106726/1113/agent[1219.5,104.1,-842.1]->tgt[1205.5,104.0,-839.5] d=14
    tryEmit 0/0   emit 0/0/0/0   mqStarted 16   children 9171298/792638/...

  - проверок цели 106 726 против 93 при NaN — поиск больше не сдаётся мгновенно;
  - цель — НАСТОЯЩАЯ координата;
  - ближайшее приближение 1113 см = 11.13 блока при цели в 14 блоках.

То есть за девять миллионов развёрнутых потомков дистанция сокращается с 14 до 11 и дальше
не идёт. ДОПУСК ПРИБЫТИЯ ИСКЛЮЧЁН: окно 0.45 блока не имеет значения, когда фронтир встаёт
за одиннадцать. Остаются две версии: физика не проходит этот участок, либо грубый проводник
ведёт туда, куда физика не пойдёт.

Репро и счётчики на месте, число для следующего захода есть.

## ⭐ ГОЛОДАНИЕ ПОИСКА УСТРАНЕНО: ДЕТЕКТОР ЗАСТРЕВАНИЯ УБИВАЛ ЕГО САМ (2026-08-25)

Курс 1219. Цепочка замеров, каждый шаг числом:

1. Спуск в проводнике реален и он ВНУТРИ проводника: `n0y104.1 -> n1 y101.0`, `dy01=-3.1`.
2. Страж падения, обязанный такой ход отвергнуть, НЕ ВЫЗЫВАЕТСЯ: `fallChk 0/0`. У
   `getChildren` два выхода, и «умный» уходит ранним `return` ДО фильтра — `SmartMoves`
   разрешает себе спуск до трёх блоков (`MAX_DESCEND=3`) мимо общего гейта. Свёл оба
   выхода к одному гейту, но ЗАСЧИТАТЬ НЕЛЬЗЯ: на этом курсе он не сработал (`smart 0-0`).
3. Настоящий след: `searchAborted=28` при `tryEmit=0` — физическую ногу убивают ДО первой
   попытки отдать маршрут. Две правки по УГАДАННЫМ местам (resume в PathExecutor) не
   сработали ни разу (`resumeKept0`). Третья подряд несработавшая правка — перестал гадать.
4. Пометил КАЖДЫЙ `stop.set(true)`. Первый заход — `stopBy: none`: пометил только tungsten,
   а убийцы в altoclef. Расширил сеть — виновник назвался точно:

   ```
   CustomBaritoneGoalTask@609 : 21   (сброс нава через 5000 мс без прогресса)
   CustomBaritoneGoalTask@521 :  7   (сдача «цель недостижима» через 14 000 мс)
                         итого 28  =  searchAborted=28
   ```

Круг замыкался сам: бот замурован (`head:dirt`, `mine:Y`) и не движется -> через 5 с
детектор -> убивает поиск -> поиск не успевает до первой выдачи -> маршрута нет -> не
движется. Порог детектора КОРОЧЕ времени поиска до первого маршрута, поэтому детектор
гарантировал ровно тот застой, который ловит.

ПРАВКА (`stallResetSparesAVirginSearch`): не поднимать порог (это хардкод), а запретить
сносить поиск, который ЕЩЁ НИЧЕГО НЕ ВЫДАЛ — такой поиск не «плохой маршрут», а
недоделанный, и его убийство помочь не может. Завершение по-прежнему ограничено
собственным бюджетом поиска.

РЕЗУЛЬТАТ: `spared27`, `searchAborted 28 -> 1`, остановок 2 вместо 28.

НО КУРС НЕ ЗЕЛЁНЫЙ: `tryEmit=0` осталось. Аборты МАСКИРОВАЛИ второй дефект. Теперь видно
чисто: тест цели не проходит ни разу, и причина выше по слою —
`guide end[1205.5,104.0,-839.5] toTgt6.7`: САМ ПРОВОДНИК кончается в 6.7 блока от цели.
Дальше копать здесь, на грубом слое, а не в передаче между слоями.

## ⭐ Я КОПАЛ НЕ ТОТ СЛОЙ: НА ЭТОМ КУРСЕ ВЕДЁТ BlockPathWalker (2026-08-25)

После того как детектор перестал убивать поиск (`spared27`, `searchAborted 28 -> 1`,
ворота `nav 14/14` + `craft 22/22`), `tryEmit=0` осталось. Замер исхода грубого поиска
дал НУЛИ по всем четырём выходам, и следующий замер объяснил почему:

```
bsIn[f0 s0 i0]      BlockSpacePathFinder.find() НЕ ВЫЗЫВАЕТСЯ НИ РАЗУ
bsEnd[c0 t0 s0 x0]  соответственно ни одного исхода
smart0-0 fallChk0-0 getChildren тоже не выполняется (перекрёстное подтверждение)
walkMode=139/0/0    а вот ЭТО живое
mqStarted=21
```

Вывод: `guide=n16` — СТАТИЧЕСКИЙ ОСТАТОК от давнего расчёта, грубый слой на этом курсе
спит целиком, физическая нога не выдаёт ничего, а бота реально ведёт `BlockPathWalker`
со своим BFS. Все заходы этой серии (спуск на 3 блока, общий гейт, страж resume) я
делал в слоях, которые здесь НЕ ИСПОЛНЯЮТСЯ — отсюда и три подряд несработавшие правки.

МЕТОДИЧЕСКИЙ ВЫВОД (дороже самой находки): прежде чем чинить слой, доказывать что он
ВЫПОЛНЯЕТСЯ. Счётчик на входе в функцию стоит один замер и снимает целую серию пустых
правок. В этой сессии цена ошибки — четыре захода.

СЛЕДУЮЩЕЕ: разбирать `BlockPathWalker` (`walkMode`, очередь ходов, `gaveUp=3/0`), а не
два спящих слоя.

## ⛔⛔ КУРС-РЕПРО 1219 НЕВАЛИДЕН — ЦЕЛЬ ЭТО СТВОЛ ДЕРЕВА (2026-08-25)

Пощупал мир через py4j `blk` (rcon отказывал: «position is not loaded»):

```
ЦЕЛЬ   1205,104,-846   spruce_log   (102 и 103 тоже log, 105-106 листва)
СТАРТ  1219,104,-843   white_carpet, на 105 dirt  -> отсюда head:dirt в состоянии
ЗАДАЧА Getting to block {x=1205, y=104, z=-846}
```

Репро гонит ГОЛЫЙ goto в сплошное бревно. Попасть туда нельзя по построению, поэтому
грубый поиск не завершается, тест цели физики не проходит, walker выключен — все три
слоя простаивают ПРАВИЛЬНО. Вот почему четыре захода подряд «не срабатывали»: они и
не могли, курс не про них.

ВАЖНО, не путать с боевым случаем: в реальном проходе ту же цель выдаёт
`DestroyBlockTask:715` через `GetToBlockTask` («займи клетку»), и там она достижима
ПОСЛЕ того как бревно сломано — по дороге бот его обычно ломает. Правка на этот случай
(`breakGoalIsReach`, цель = дистанция вместо занятия клетки) УЖЕ ИЗМЕРЕНА на n=8 и
нейтральна (ON 6/8 PASS, mean 1.5 ступени; OFF 7/8, mean 1.4), поэтому остаётся
выключенной. Не включать заново без нового числа.

ИТОГ: закрываю линию «застой на 1219» как артефакт собственного репро. То, что от неё
осталось полезного, уже отгружено и закрыто воротами: детектор застревания больше не
убивает поиск (`spared27`, `searchAborted 28 -> 1`, `nav 14/14`, `craft 22/22`).

ОСТАЁТСЯ ОДНА РЕАЛЬНАЯ ЦЕЛЬ: разброс прохода (3 против 7 ступеней). Мерить проходом,
а не точечным курсом.

## ⭐ ЗАМЕР ПРОХОДА n=4: РАЗБРОС СИДИТ В ДЕБЮТЕ (2026-08-25)

Четыре прогона по 10 минут, с записью, на сборке со стражем детектора:

```
прогон  ступеней  первая ступень   последняя
  1        4        299.1 s        stone tools 549.8 s
  2        4         21.9 s        stone tools 229.2 s
  3        6         22.0 s        FURNACE     229.8 s   (через coal 137.3 s)
  4        4         66.3 s        wood tools  269.7 s
```

ГЛАВНОЕ: время до ПЕРВОЙ ступени 22 / 22 / 66 / 299 с — тринадцатикратный разброс, и
прогон 1 явный выброс («LADDER STALLED: no new rung for 150s»). После того как дебют
состоялся, лестница идёт ровно. Значит разброс «3 против 7 ступеней» — это НЕ разброс
лестницы, а разброс ОТКРЫТИЯ: сколько бот ищет первое дерево.

Навигационные слои в бою при этом ЗДОРОВЫ, и это важно после серии по 1219:
`bsIn[f0 s235 i236]`, `bsEnd[c235 t0 s0 x0]` — грубый поиск отработал 235 раз и все 235
завершились УСПЕХОМ, ноль таймаутов, ноль исчерпаний; `fallChk192-0` — страж падения
здесь живой. `spared0` — правка детектора тут не понадобилась (`searchAborted=4`), она
лечит именно вырожденный случай.

СЛЕДУЮЩИЙ КАНДИДАТ (по числам того же прогона): `stopBy[TungstenHelper@600:794]` —
794 остановки поиска из общего `TungstenHelper.stop()`, плюс `noReach=1056`,
`dbBlocked=968/0/0` при `blockedBy=minecraft:stone` и `dbNearTick=3198`. Надо
разделить вызывающих `stop()` по местам (как с `stop.set(true)`), прежде чем что-то
трогать — метод уже один раз окупился.

## ⭐ ДЕБЮТ ПОЧИНЕН: ВЫБРОС 299 с ИСЧЕЗ, РАЗБРОС 22-299 -> 21.6-21.8 (2026-08-25)

Правка: часы бюджета погони за упавшим предметом перенесены С ЭКЗЕМПЛЯРА ЗАДАЧИ НА ЦЕЛЬ
(`dropBudgetSurvivesTaskRebuild`). Были `private` поля, задача пересоздаётся постоянно,
каждое пересоздание обнуляло часы — поэтому двухминутный потолок, включённый ещё
23 августа, читался НУЛЁМ за весь десятиминутный прогон.

```
время до первой ступени
  ДО    299.1   21.9   22.0   66.3   с
  ПОСЛЕ  21.8   21.8   21.6   21.8   с

dropBudget=1  pursuit=9/163s   потолок СРАБОТАЛ, длиннейшая погоня 163 с
перезапуски часов: 1 / 9 / 12 / 4  (не тысячи — значит ключ по цели верный)
```

ВОРОТА: `craft 22/22` со всеми шестью курсами подбора (flat, side, ledge, pit, vs_mine,
after_goto), `nav 14/14`.

ЧЕСТНО ПРО ВТОРУЮ ПОЛОВИНУ: по числу ступеней 4/5/4/2 против 4/4/6/4 — среднее не
выросло. При n=4 это шум, и главное: починен ДЕБЮТ, а не потолок лестницы. Прогон 4
встал уже ПОСЛЕ дебюта, и это отдельный отказ.

## ⛔ СЛЕДУЮЩАЯ ЦЕЛЬ: ЦИКЛ «ПОСТАВИТЬ ВЕРСТАК -> БЛУЖДАНИЕ -> ПОСТАВИТЬ»

Дамп нового выброса (прогон 4, 2 ступени):

```
1.1.2. <Place [Block{minecraft:crafting_table}] nearby>
1.1.3. <Place structure{crafting_table} at ...>
1.1.4. <Wander for 5.0 blocks>
TimeoutWanderTask:255x2338   walkMode=3471/0/0
wander=1521  wanderMoved=3617   <- блуждание теперь РЕАЛЬНО двигает (было 0)
dropPick=[spruce_planks cost=293 dy=-8.0 of=1]
```

Бот не может поставить верстак и крутится в цикле. Прошлая болезнь при этом ушла:
`wanderMoved` 0 -> 3617. Метод прежний: сперва доказать счётчиком, ЧТО именно отказывает
в постановке (нет опоры / нет места / отказ гейта), и только потом править.

## ⛔ ЧЕСТНЫЙ СВОД ПО ПРАВКЕ ДЕБЮТА, n=9 (2026-08-25)

```
дебют (время до первой ступени)
  ДО    (n=4)  299.1  66.3  22.0  21.9        медиана 44.2   худший 299.1
  ПОСЛЕ (n=9)  116.2 111.6  44.5  21.8 x6     медиана 21.8   худший 116.2

ступени
  ДО    (n=4)  4 4 6 4              среднее 4.50
  ПОСЛЕ (n=9)  4 5 4 3 2 2 5 3 4    среднее 3.56
```

Дебют улучшился (медиана вдвое, худший втрое, 6 из 9 открываются за 21.7 с). ПО ЧИСЛУ
СТУПЕНЕЙ УЛУЧШЕНИЯ НЕТ, среднее даже ниже. Выборки малы и разброс велик — скорее шум,
но записываю как есть: правка чинила ДЕБЮТ и починила дебют, не потолок.

## ⛔ ПОТОЛОК ЛЕСТНИЦЫ: ДВА РАЗНЫХ ОТКАЗА, ПРИБОР ПОСТАВЛЕН

Дампы застоя дали ДВА разных листа, не один:

1. `Place structure{crafting_table}` -> блуждание. Гипотезу «виновата PlaceBlockNearbyTask»
   ОТМЕНИЛ замером до постройки решения: `pnb=2/0/0/2` — задача тикнула за прогон дважды
   и оба раза успешно. Не она.
2. `Craft 2x2 dark_oak_planks` -> `Moving [logs]`. Здесь `mc=3226/0/0/1/0` при
   `cgTick=0`: каждый тик уходит в ветку заполнения слота, в сетку не попадает ничего.
   Совпадает с записью прошлого захода (`mc=1051/0/0/0/0`), где уже указано читать
   `MoveItemToSlotFromInventoryTask`.

ПОСТАВЛЕН ПРИБОР на доставку: `mv=ticks/blocked/pickup/place/noSource`. Здоровые прогоны
читаются `mv=86/66/6/14/0` и `mv=183/143/12/28/0` — механизм рабочий, ~77% тиков
придерживает ограничитель частоты (это норма). ЗАСТОЙ В ЭТИХ ДВУХ ПРОГОНАХ НЕ
ВОСПРОИЗВЁЛСЯ, поэтому виновник пока НЕ НАЗВАН. Прибор назовёт его при следующем
повторении: если `mvTicks` велик при `mvBlocked ~ mvTicks` — душит ограничитель; если
`mvPickup`/`mvPlace` велики, а в сетке пусто — клик не долетает.

ОСТОРОЖНО (сегодняшняя ошибка): `ciGrid` это `ciGridStranded` — «ингредиенты ЗАСТРЯЛИ в
сетке». Ноль здесь ЗДОРОВОЕ чтение, а не дефект. Я едва не построил на нём вывод.

## ⛔ ОТЗЫВ: «pnb=4353/4350» — ЭТО СУММА ЗА ВЕСЬ СВОД, А НЕ ЗА ПРОГОН (2026-08-25)

Я прочитал `pnb=4353/4350/0/3` как показания ОДНОГО прогона и сказал, что бот 99.9%
тиков смотрит в пустоту. Это неверно. Числа по прогонам свода росли монотонно:

```
3708 -> 4110 -> 4353 -> 4480     приросты ~400, ~240, ~130
```

то есть счётчик НАКОПИТЕЛЬНЫЙ, а за отдельный прогон это две-четыре сотни тиков.

ПРИЧИНА — МОЯ: вставку сброса в `resetRunCounters` я сделал под `if m2:` без проверки,
регулярка не совпала, и `pnbTicks = 0` / `mvTicks = 0` в код НЕ ПОПАЛИ. Сам стенд сброс
зовёт исправно (`gamer_smoke` -> `py4j("resetstats")`). Сброс дописан рядом с уже
работавшим `pursuitRestarts = 0`, так что замер ДЕБЮТА (`pursuit=9/163s`) корректен —
он сбрасывался всегда.

УРОК, ТРЕТИЙ РАЗ ЗА СЕССИЮ ОДИН И ТОТ ЖЕ: прибор надо проверять до чтения показаний.
Отличие от прошлых двух — на этот раз я поймал это ДО того, как построил на числе
правку. Проверка «счётчик вообще сбрасывается?» стоит один grep.

Что при этом ОСТАЁТСЯ ВЕРНЫМ и не отзывается:
  - `mc=0/0/0/0/0` в двухступенчатом прогоне — крафт там не запускался, застой НЕ крафтовый;
  - в другом прогоне `pnb` вообще не рос (`2/0/0/2`), значит постановка участвует не всегда;
  - детерминированный путь постановки СУЩЕСТВУЕТ (`locateClosePlacePos` -> `PlaceBlockTask`),
    а прицел — лишь быстрая тропа, поэтому «смотрит в пустоту» само по себе не дефект.

Поставлены счётчики на ВЕТКИ ВЫХОДА (`pnbExit=wander/progFail/locate/noSpot`) — они и
скажут, доходит ли управление до детерминированного пути или крутится в блуждании.

## ⭐ ПОТОЛОК: ОТКАЗ ПОДХОДА НАЗВАН ЧИСЛОМ — 8 ЦЕЛЕЙ, 0 ДОСТИГНУТО (2026-08-25)

Свод из трёх прогонов с ИСПРАВЛЕННЫМ сбросом счётчиков (7 / 0 / 0 ступеней):

```
run1  7 ступ.  dbTick=6459   цели 68/35  noReach=156  bestDist=2    pnb=15/15/0/0
run2  0 ступ.  dbTick=11989  цели 17/9   noReach=908  bestDist=1    pnb=0
run3  0 ступ.  dbTick=12322  цели  8/0   noReach=0    bestDist=128  pnb=0
```

RUN3 — самый чистый отказ, какой удалось поймать: за 12322 тика разрушения бот выбрал
ВОСЕМЬ целей и НЕ ДОШЁЛ НИ ДО ОДНОЙ, ни разу не подойдя ближе ~12.8 блока. При этом
`noReach=0` и `dbTargetAir=0` — ни луч, ни «цель это воздух» ни при чём. Это отказ
ПОДХОДА, отдельный от профиля play5, где `noReach` доходил до 3967.

Постановка блоков в нулевых прогонах НЕ УЧАСТВУЕТ ВООБЩЕ (`pnb=0`), крафт тоже
(`mc=0`) — обе линии, которые я разбирал ранее, к этому отказу отношения не имеют.

ЛИНИЯ САМОПОЛА ЗАКРЫТА (не мной сегодня, а прошлыми замерами — проверил перед тем, как
трогать): `dbBlocked=968/0/0` действительно весь приходится на `dbBlockedSelfFloor`, но
оба флага против этого измерены и нейтральны — `noRetreatWhenOwnFloorBlocks` (срабатывает
666 раз за прогон, курсы не лучше) и `stepOverWhenOwnFloorBlocks` (4/6 против 4/6).
Их javadoc сам указал третье прочтение — «цель просто не тот блок», числа `dbTargetAir`
и `dbBestDist`. Я их снял: `dbTargetAir=0`, то есть и это прочтение здесь НЕ подтвердилось.

СЛЕДУЮЩИЙ ЗАХОД: почему выбранная цель не достигается подходом — смотреть
`dbApproachStall`, `dbFar/dbNear`, `dbDistSum` в прогоне с профилем run3, и доказать
счётчиком, КАКАЯ ветка подхода отказывает, прежде чем что-либо править.

## ⭐ ОТКАЗ ПОДХОДА СУЖЕН ДО ОЧЕРЕДИ ДВИЖЕНИЯ: 600 СТАРТОВ, 0 ПРИБЫТИЙ (2026-08-25)

```
                   run1 (7 ступ.)   run2 (0)   run3 (0)
mqStarted                44            602        617
mqSteps                 101           1013       1005
mqRefused(short)       1460              0       1229
mqNullEdge             1454              0       1229   = tickBfs 1229 РОВНО
exArrived               198              0          0
pdWalking               211              0          0
pdNear                 2721            638          0
wanderMoved             455              0          0
walkMode           6104/2086/0     5738/0/0   4836/1229/0
```

ЧТО ЭТО ЗНАЧИТ: в мёртвых прогонах очередь ЗАПУСКАЕТ ~600 ходов и делает ~1000 шагов,
то есть бот РЕАЛЬНО ШАГАЕТ, но `exArrived=0` — не прибывает никуда, а в run3 ещё и
`pdNear=0`, то есть ни разу не оказывается даже РЯДОМ с целью. Сдача выносится на
средней дистанции 14.2 блока (`dbUnreachMove=28`, все дальние, `dbApproachStall=0`).

ЧТО ИСКЛЮЧЕНО ЧИСЛОМ (важно, чтобы следующий заход не тратил на это время):
  - НЕ пустые рёбра BFS: `mqNullEdge=1454` есть и в ЗДОРОВОМ прогоне, он работает;
  - НЕ разрушение: `noReach=0`, `dbTargetAir=0` в run3;
  - НЕ постановка (`pnb=0`) и НЕ крафт (`mc=0`) — они в нулевых прогонах не участвуют;
  - НЕ самопол: обе правки против него измерены и нейтральны (см. выше).

СЛЕДУЮЩИЙ ЗАХОД: почему `exArrived` не наступает НИ РАЗУ при тысяче сделанных шагов.
Сперва счётчик на ветки прибытия исполнителя (и на то, чем заканчивается каждый
запущенный ход), и только потом правка. Метод в этой сессии окупился трижды и трижды
же поймал враньё собственных приборов — проверять, что счётчик ВЫПОЛНЯЕТСЯ и
СБРАСЫВАЕТСЯ, до чтения показаний.

## ⭐ ПОТОЛОК СУЖЕН ДО BFS ХОДОКА: МАРШРУТ ИЗ ОДНОЙ КЛЕТКИ, 100% ТИКОВ (2026-08-25)

```
run1 (7 ступ.)   tickBfs=2086   qNullEdge=1454   -> 70%   pdNear=2721  цели 68/35
run3 (0 ступ.)   tickBfs=1229   qNullEdge=1229   -> 100%  pdNear=0     цели  8/0
                 mqRefused(short=1229)   bestDist=128 (12.8 блока)
```

В мёртвом прогоне КАЖДЫЙ тик BFS даёт маршрут, который после чистки схлопывается и
отклоняется как КОРОТКИЙ — то есть фактически маршрут из ОДНОЙ клетки при цели в
четырнадцати блоках. В здоровом прогоне это лишь 70% тиков, и он едет.

ДВЕ ПОПРАВКИ К СОБСТВЕННЫМ ЧТЕНИЯМ (обе поймал до правки):
  - `exArrived=0` НЕ дефект: он считает завершения пути ФИЗИЧЕСКИМ исполнителем, а здесь
    ведёт ХОДОК. При работающем ходоке ноль там ожидаем.
  - `pdPlan=0/0/0` НЕ дефект: планирование стоит в ветке «цель ближе 4 блоков», а бот
    всегда дальше (`pdNear=0`), поэтому всегда берётся ветка ходока — это правильно.
  - `qNullEdge` — НЕ «пустое ребро», а счётчик ДУБЛИРУЮЩИХ клеток, удалённых при чистке
    маршрута. Имя вводит в заблуждение; смысл — маршрут схлопывается.

ИСКЛЮЧЕНО ЧИСЛОМ: разрушение (`noReach=0`, `dbTargetAir=0`), постановка (`pnb=0`),
крафт (`mc=0`), самопол (обе правки измерены нейтральными), досрочные выходы привода
(`pdNotPrim/pdNoGoal/pdFinished/pdNoVec` — все нули), притяжка цели (`snap=12371/12371/0`).

СЛЕДУЮЩИЙ ЗАХОД: читать BFS ходока и выяснить, почему он возвращает маршрут из одной
клетки при цели в 14 блоках — радиус поиска, отказ по проходимости или отсечение
диагоналей (`gridBfsRefusesCornerCut` включён). Сперва счётчик на причину отказа BFS,
потом правка.

## ⭐⭐ НАЗВАНО ЧИСЛОМ: ПОЛОВИНА ПЛАНИРОВАНИЙ НЕ РАСКРЫВАЕТ НИ ОДНОГО УЗЛА (2026-08-25)

```
plan=105/6156/101ms/sz74/zero53
  planCalls        105
  planLastExpanded 6156     когда работает — расширяет тысячи узлов
  planLastMs        101     за сотню миллисекунд
  planLastSize       74     и отдаёт маршрут из 74 клеток
  planZeroExpand     53     А ПОЛОВИНА ВЫЗОВОВ РАСКРЫВАЕТ <=1 УЗЛА
```

`FastPlanner.plan` — обычный A*, у которого `best = startNode`. Если ни один преемник не
порождён, результат это ОДИН узел, `FastNavigator` отвергает его как короткий
(`navRes=123/0/0/0/0` в другом прогоне: 123 отказа и НИ ОДНОГО принятого маршрута), и
очередь получает схлопнутый маршрут (`mqRefused(short=1229)` при `tickBfs=1229`).

ВЫВОД: отвергается СТАРТОВАЯ КЛЕТКА — из точки, где стоит бот, не порождается ни одного
преемника, в половине случаев. Это и есть механизм «тысяча шагов без сближения».

ЦЕПОЧКА, КОТОРОЙ ЭТО ДОСТИГНУТО (каждое звено — число, а не рассуждение):
  потолок лестницы -> отказ ПОДХОДА (цели 8/0, сдача на 14.2 блока)
  -> очередь: 600 стартов, 1000 шагов, 0 сближения
  -> маршрут схлопывается: 100% тиков BFS против 70% в здоровом прогоне
  -> источник: `FastPlanner.plan` отдаёт <2 клеток
  -> причина: 53 из 105 планов не раскрывают НИ ОДНОГО узла от старта

СЛЕДУЮЩИЙ ЗАХОД: почему от стартовой клетки нет преемников. Смотреть генерацию соседей
и `PlayerFit` для клетки, где стоит бот (стоя в воде/на краю/в блоке?). Сперва счётчик
на причину отказа соседей, потом правка.

## ⛔ ПРАВКА «ДОВЕРЯТЬ ФИЗИКЕ НА СТАРТЕ» — ИЗМЕРЕНА И ОТКЛОНЕНА (2026-08-26)

Гипотеза: `FastPlanner` отказывается планировать из клетки, где бот СТОИТ
(`supportTop = NaN` -> `continue` -> ноль преемников -> маршрут из одной клетки).
Основание было сильное: `plan=95/8448/253ms/sz39/zero57(e0=0,e1=57)` — бюджет не
срабатывал ни разу, а 57 планов из 95 умирали со стартовым узлом без детей.

```
БАЗА  (флаг off)   nav 14/14   craft 22/22
ФИКС  (флаг on)    nav 13/14   nav_water FAIL
```

Отклонена, и посылка была неверна: `supportTop = NaN` НЕ значит «мир ошибся». Это
значит «бот НЕ СТОИТ» — он в воздухе, в падении или ПЛЫВЁТ. Подсунутая наземная опора
заставляет строить наземный маршрут оттуда.

Перенос спасения за ветку воды/лестниц НЕ помог — `nav_water` упал снова, потому что
спасение срабатывает и на плавучих стартах, которые `isWater()` водой не считает. Два
независимых подтверждения.

ОТДЕЛЬНО ОТОЗВАНО: вылет бота из мира на `nav_gaps` («min Y -149.6 при полу -60») я
сперва приписал этой правке. НЕВЕРНО — он воспроизводится и с ВЫКЛЮЧЕННЫМ флагом, а
сьют сам переизмеряет его на свежих клиентах и называет износом стенда. Единственной
настоящей регрессией была `nav_water`.

ЧТО ОСТАЁТСЯ ВЕРНЫМ И ЦЕННЫМ — ИСПРАВЛЕННОЕ ПРОЧТЕНИЕ `e1=57/95`:
это не «планировщик зря отвергает старт», а **60% ПЛАНОВ ЗАПРАШИВАЮТСЯ, ПОКА БОТ НЕ
ИМЕЕТ ОПОРЫ** — и они отказывают ПРАВИЛЬНО. Лечить надо не опору, а момент запроса:
либо разрешать старт в клетку, на которую бот СЯДЕТ (`BlockSpacePathFinder.resolveStart`
уже умеет ровно это), либо не выдавать план, пока он не приземлился.

СЛЕДУЮЩИЙ ЗАХОД: перенести логику `resolveStart` в `FastPlanner` (старт = клетка
приземления) и померить `e1` до/после. Это правка того же корня, но без ложной посылки.

## ⛔ СНАП СТАРТА: ОТГРУЖЕН, БЕЗВРЕДЕН, НО ПОЧТИ НЕ СРАБАТЫВАЕТ (2026-08-26)

```
ВОРОТА   база     nav 14/14   craft 22/22
         со снапом nav 14/14   craft 22/22     регрессии нет, nav_water проходит

ПРОХОД   run1  plan=8/21504/251ms/sz13/zero0(e1=0,  snap=0)   4 ступени
         run2  plan=554/18/1ms/sz8/zero134(e1=134, snap=5)    6 ступеней
                                                              5 ступеней (run3)
```

ГЛАВНОЕ ЧИСЛО: `e1=134` против `snap=5`. Бездетных стартов по-прежнему сто тридцать
четыре, а снап сработал ПЯТЬ раз. Правка гейты проходит и ничего не ломает, но эффекта
ей приписывать НЕЛЬЗЯ — она почти не выполняется. Ступени 4/6/5 к ней отношения не имеют.

ПОЧЕМУ НЕ СРАБАТЫВАЕТ (гипотеза для следующего замера, НЕ вывод): `FastNavigator`
планирует не всегда от ног бота — есть путь, где старт это ХВОСТ текущего отрезка
(`legTail`, плюс флаг `planFromActualPosition`). Мой снап ищет опору по коллизии ИГРОКА,
поэтому для стартов, не совпадающих с позицией бота, кандидатов не находит и возвращает
вход без изменений.

СЛЕДУЮЩИЙ ЗАХОД: сперва ЗАМЕР — сколько бездетных стартов совпадает с позицией бота, а
сколько приходит от хвоста отрезка (счётчик на равенство `start` и позиции игрока).
Если преобладает хвост — снапить по геометрии КЛЕТКИ (столб вниз от неё), а не по
коллизии игрока. Правку не трогать, пока это не измерено.

## ⭐⭐ НАЙДЕНО: 35% ПЛАНОВ — МАРШРУТ В КЛЕТКУ, ГДЕ БОТ УЖЕ СТОИТ (2026-08-26)

```
plan=495/2817/49ms/sz7/zero171(e0=0,e1=171,resc=0,snap=251,noSup=0,supNoKids=0,atGoal=171)

atGoal = 171 = e1 = 171     СОВПАДЕНИЕ ПОЛНОЕ
```

Все бездетные планы до единого — это планы, чей СТАРТ УЖЕ ЯВЛЯЕТСЯ ЦЕЛЬЮ. Поиск выходит
на первой же итерации по проверке цели, отдаёт маршрут из ОДНОГО узла, и
`FastNavigator` отвергает его как короткий — ПРАВИЛЬНО отвергает.

Это НЕ отказ планирования: `noSup=0` (опоры хватает), `supNoKids=0` (соседи не
заблокированы). Боту 171 раз из 495 велят идти туда, ГДЕ ОН УЖЕ СТОИТ.

ПОПУТНО СНЯТ ВОПРОС ПО СНАПУ: `snap=251` — правка РАБОТАЕТ и потому `noSup=0` (старты
чинятся до цикла). Раньше читалось `snap=5` просто потому, что в том прогоне почти не
было стартов без опоры. Никакого эффекта на ступени ей всё равно не приписываю.

ЦЕПОЧКА ЦЕЛИКОМ (каждое звено — число):
  потолок -> отказ подхода (цели 8/0) -> очередь: 600 стартов, 0 сближения
  -> маршрут схлопывается (100% тиков BFS) -> `FastPlanner` отдаёт <2 клеток
  -> старт бездетен -> И ПРИЧИНА: старт == цель, 171 из 495

СЛЕДУЮЩИЙ ЗАХОД: найти, КТО заказывает маршрут в текущую клетку. Смотреть, откуда
приходит `goal` в `FastNavigator.start()` и почему прибытие не засчитывается
(`GetToBlockTask` целит В блок, `DestroyBlockTask` — тоже: цель занята и потому
«не достигнута», хотя бот на ней). Сперва счётчик на источник цели, потом правка.

## ⛔ ПОТОЛОК МНОГОПРИЧИНЕН: ТРИ РАЗНЫХ ПРОФИЛЯ, НЕ ОДИН (2026-08-26)

Это важнее любого из трёх по отдельности: я несколько заходов искал ОДНУ причину
потолка, а их минимум три, и они не связаны. Профили различаются счётчиками однозначно:

```
A. МАРШРУТ В ТЕКУЩУЮ КЛЕТКУ   atGoal=171 = e1=171 из 495 планов (35%)
   noSup=0 supNoKids=0        поиск выходит на первой итерации, путь из 1 узла,
                              навигатор отвергает как короткий — ПРАВИЛЬНО

B. ЦЕЛЬ НЕДОСЯГАЕМА           noReach=3144..3967, dbUnreachMove=28 (все дальние),
                              цели 8/0, сдача на средних 14.2 блока

C. НЕ ОТКРЫВАЕТСЯ ВЕРСТАК      <Interact using (empty) at {x=315,y=109,...}> -> Wander
   plan=63/../zero0 atGoal=0   ПЛАНИРОВАНИЕ ЗДОРОВО, навигация не участвует вовсе
                              (walkMode=3077/0/0, mqStarted=8), wander=1914/moved=142
```

Профиль C — свежий и самый чистый: 63 плана, ноль отказов, а прогон взял 2 ступени.
Значит потолок в нём НЕ навигационный, и все правки в пасфайндере его не сдвинут.

СЛЕДУЮЩИЙ ЗАХОД: профиль C. Найти, почему `Interact using (empty)` не открывает
верстак — пустой предмет в руке при взаимодействии выглядит как неверно собранный
запрос. Сперва счётчик на ветки этой задачи (выдан ли клик, дошёл ли экран), потом
правка. Профили A и B держать отдельно и не смешивать замеры.

## ⭐⭐ ПОЙМАН ПРОФИЛЬ КРАФТА: 2255 ПОДЪЁМОВ ПРОТИВ 3 УСТАНОВОК (2026-08-26)

Прибор на доставку предметов, поставленный несколько заходов назад, наконец сработал
на прогоне с застоем крафта досок (2 ступени):

```
лист:  <Craft 2x2 Recipe{spruce_planks x3}> -> <Moving [acacia_log,birch_log,...]>

mc = 11373/0/0/1/0            родитель просил 11373 раза, ВСЁ в ветку заполнения слота
mv = 11373/9115/2255/3/0
     тики  ограничитель  ПОДЪЁМОВ=2255  УСТАНОВОК=3  нетИсточника=0
cgTick=0  ciGrid=0        сетка крафта не тикает, в неё ничего не ложится
iw=0/0/0/0                взаимодействие тут вообще не участвует
```

ЧТЕНИЕ: ветка подъёма выбирается, когда курсор ПУСТ или держит не то. `MOVEMISMATCH`
в дампе не печатается, значит курсор ПУСТ — то есть клик подъёма НЕ ДОЛЕТАЕТ. Предмет
берут 2255 раз и кладут 3 раза.

Для сравнения, здоровые прогоны того же свода: `mv=135/106/15/14/0` — 15 подъёмов
против 14 установок, то есть механизм в норме кладёт почти всё, что берёт.

ЭТО НЕ ПАСФАЙНДЕР И НЕ ЛОГИКА КРАФТА, а слой кликов по слотам (`SlotHandler`).
`noSource=0` снимает версию «нечего брать»: источник есть всегда.

СЛЕДУЮЩИЙ ЗАХОД: почему `clickSlot(..., PICKUP)` не оставляет предмет в курсоре.
Сперва счётчик в `SlotHandler` (выдан ли клик серверу, что вернулось, каков курсор
после), затем правка. Профили A (маршрут в текущую клетку) и B (недосягаемая цель)
держать отдельно — это третий, независимый.

## ⛔ ПРИБОР НА ИСТОЧНИК КЛИКОВ ПОСТАВЛЕН, ОТКАЗ НЕ ПОВТОРИЛСЯ (2026-08-26)

В застойном прогоне было `shIssued=603671` — ШЕСТЬСОТ ТЫСЯЧ кликов по слотам за десять
минут, при том что сам грузчик выдал около 2255. Это правдоподобно объясняет пустой
курсор (`mv` 2255 подъёмов против 3 установок): чужой клик убирает предмет раньше,
чем грузчик успевает положить.

Поставлена ВЫБОРОЧНАЯ атрибуция вызывающих (1 из 1024 — полная трассировка при такой
частоте сама исказила бы прогон). Читается как `shWho=[...]`.

```
свод из 3 прогонов: 5 / 5 / 4 ступени — САМЫЙ РОВНЫЙ за сессию, ни одного нулевого
shIssued=3230 и 4184     против 603671 в застойном — отказ НЕ ПОВТОРИЛСЯ
shWho=[BeatMinecraftTask:364x2 PlayerInteractionFixChain:106x1 BeatMinecraftTask:362x1]
```

Выборок мало (3-4), потому что кликов было мало — это ЗДОРОВЫЕ прогоны. Виновник
НЕ НАЗВАН: при 600 тысячах кликов выборок будет под шесть сотен, и доминирующий
вызывающий проявится однозначно. Прибор ждёт повторения.

ПОДОЗРЕВАЕМЫЙ (гипотеза, НЕ вывод): `PlayerInteractionFixChain` — «цепочка починки
руки», которая по устройству трогает слоты и курсор. Но выборки из здоровых прогонов
этого не доказывают, и приписывать ей 600 тысяч кликов сейчас нельзя.

## ⛔ ПРОФИЛЬ A: ГИПОТЕЗА О РАСХОЖДЕНИИ СЛОЁВ СНЯТА ЗАМЕРОМ (2026-08-26)

Гипотеза была: тест цели планировщика допускает ±1 по высоте, а `AltoGoal.block` требует
точного занятия клетки — значит слои спорят о прибытии, и задача заказывает маршрут
вечно. ЗАМЕР ЕЁ НЕ ПОДТВЕРДИЛ:

```
atGoal=50(ex50, ytol0)      ВСЕ совпадения ТОЧНЫЕ, ни одного по допуску
```

Слои согласны: бот действительно стоит В целевой клетке. Значит холостой сам ЗАПРОС,
а не разбор прибытия.

НОВАЯ ЛИНИЯ (согласуется, но НЕ доказана — числа малы): цель притягивается к проходимой
клетке каждый тик, и для недостижимой цели ближайшей проходимой может оказаться та, где
бот СТОИТ. Поставлен счётчик `snapToSelf`.

```
run1  snap=1013/1013/0/self19    atGoal=1     5 ступеней
run2  snap=9419/2687/6732/self0  atGoal=0     5 ступеней
```

Связь self <-> atGoal держится (19<->1, 0<->0), но застойный прогон в своде не выпал,
поэтому подтверждать нечем. Прибор ждёт повторения.

ПОБОЧНАЯ НАХОДКА, отдельная и крупная: в run2 притяжка НЕ НАШЛА проходимой клетки
6732 раза из 9419 — ДВЕ ТРЕТИ запросов. При этом прогон здоров (5 ступеней), так что
сам по себе отказ притяжки не фатален, но масштаб стоит объяснить отдельно.

СОСТОЯНИЕ СВОДОВ: последние два свода 5/5/4 и 5/5/4 — заметно ровнее середины сессии,
где выпадали прогоны на 0 и 2 ступени. Приписывать это конкретной правке нельзя.

## ⭐ ВСЕ КЛИКИ ПО СЛОТАМ — ЭТО ОДНА ПРОТЯЖКА ИНВЕНТАРЯ (2026-08-26)

Атрибуция заработала после уплотнения выборки (1/64 вместо 1/1024 — на здоровом
прогоне 1/1024 давала три пробы, ранжировать было нечего):

```
shRefresh=21/0      21 проход, из них 0 начаты при ЗАНЯТОМ курсоре
shIssued=1512       21 x 72 = 1512   ТОЧНОЕ совпадение
shWho=[PlayerInteractionFixChain:106 x23]     100% выборок
```

Механизм: `PlayerInteractionFixChain` раз в 30 с зовёт `SlotHandler.refreshInventory()`,
который проходит 36 слотов и делает на каждый ДВА клика (поднять/положить) — 72 клика
за один тик, ЧЕРЕЗ `clickSlotForce`, то есть В ОБХОД ограничителя частоты. Защита
единственная: «не ломаем ли блок».

ПРАВКУ НЕ ОТГРУЖАЮ, И ЭТО ГЛАВНОЕ: я собирался запретить протяжку при занятом курсоре
(она объясняла бы `mv` 2255 подъёмов против 3 установок). Но `withCursor=0` — в здоровых
прогонах она НИ РАЗУ не стартует с занятым курсором, значит защита была бы ПУСТЫШКОЙ.
Это пятый случай за сессию, когда замер на входе спас от отгрузки невыполняющейся
правки.

ЧТО ОСТАЁТСЯ НЕОБЪЯСНЁННЫМ: застойный прогон дал `shIssued=603671`. При 72 кликах на
проход это 8384 протяжки за десять минут, то есть 14 в секунду при таймере в 30 секунд.
Либо таймер сбрасывается не там, либо протяжку зовут ещё откуда-то. Прибор
(`shRefresh`, `shWho`) теперь назовёт это при первом же повторении.

## ⛔ ГИПОТЕЗА О ТАЙМЕРЕ СНЯТА ЧТЕНИЕМ, НЕ ЗАМЕРОМ (2026-08-26)

Версия была красивая: часы `TimerGame` это ТИКИ СОЕДИНЕНИЯ (`getTicks()/20.0`), при
смене соединения счётчик сбрасывается, таймер становится вечно сработавшим, протяжка
инвентаря идёт КАЖДЫЙ тик — 72 клика x 20/с = 1440 в секунду, то есть ~864 000 за
десять минут, что по порядку совпадает с наблюдёнными 603 671.

ЧТЕНИЕ БАЗОВОГО ТАЙМЕРА ЕЁ УБИВАЕТ:

```java
getDuration() { return currentTime() - prevTime; }
elapsed()     { return getDuration() > interval; }
```

Прыжок часов НАЗАД даёт ОТРИЦАТЕЛЬНУЮ разность, то есть таймер перестаёт срабатывать
вовсе — предсказание противоположно наблюдаемому. И `reset()` стоит сразу после
протяжки, поэтому эта ветка физически не может выдать больше одного прохода в 30 с.

ВЫВОД: 603 671 клик идут НЕ отсюда. Остаются другие места, обходящие ограничитель
частоты — `clickSlotForce` в `forceEquipItem` (SlotHandler:341,401,406,413,418). В
здоровых прогонах экипировка редка, поэтому выборка и показала 100% протяжки.

Шестой за сессию случай, когда проверка ДО правки сняла гипотезу. Здесь её сняло
простое чтение источника, а не прогон — это дешевле замера и должно идти первым.

## ⭐⭐ ПРОФИЛЬ D, СНЯТ НА ЖИВОМ ЗАСТОЕ: ДВА ИСТОЧНИКА МАРШРУТА, В ОЧЕРЕДЬ ИДЁТ ХУДШИЙ (2026-08-26)

Впервые снято ВО ВРЕМЯ застоя, а не восстановлено по логам после него.

```
plan=229/10816/251ms/sz48    229 планов FastPlanner, маршруты по 48 клеток — ЗДОРОВЫ
navRes=0/0/0/0/0             блок разбора результата в FastNavigator НЕ ВЫПОЛНЯЕТСЯ
mqRefused=2338(short=2337)   очередь отвергает 2337 маршрутов как короткие
mqNullEdge=2337              ровно столько же дублирующих клеток схлопнуто
mqSteps=23  mqStarted=14     шагов 23, запусков очереди 14
walkMode=10086/2299/0        тиков BFS 2299 ~ 2337 отказов: ПО ОДНОМУ НА ТИК
pdEnter=12310 dbTick=12253   привод и разрушение тикают по 12 тысяч
pdWalking=0  pdNear=0        бот НИ РАЗУ не идёт и не оказывается рядом
noReach=0  dbBlocked=0/0/0   досягаемость и обзор В ПОРЯДКЕ
shIssued=1513 shRefresh=21/0 клики в норме — НЕ профиль C
mv=0 mc=0 atGoal=0           не доставка, не крафт, не холостые планы
```

МЕХАНИЗМ: маршрутов ДВА, и в очередь идёт не тот. `FastPlanner` считает здоровые
48-клеточные маршруты, но `navRes=0` — они не проходят через разбор навигатора и никуда
не попадают. Очередь же кормит отдельный сеточный BFS (`CombatPathfinder.findPath`,
CustomBaritoneGoalTask:715), и его выдача состоит из ПОВТОРЯЮЩИХСЯ клеток: после чистки
дублей маршрут схлопывается и отвергается как короткий — 2337 раз, по одному на тик.

Это ЧЕТВЁРТЫЙ профиль, и он исключает все три прежних числом (см. строки выше).

СЛЕДУЮЩИЙ ЗАХОД: почему `CombatPathfinder.findPath` возвращает маршрут из повторяющихся
клеток. Рядом есть флаг `gridBfsRefusesCornerCut` — смотреть его и генерацию соседей.
Сперва счётчик на форму выдачи (сколько клеток, сколько уникальных), потом правка.

## ⛔ БАЗОВАЯ ЛИНИЯ БОЕВОГО BFS: 36% ВЫЗОВОВ СЪЕДАЮТ ВЕСЬ БЮДЖЕТ (2026-08-26)

```
здоровый прогон (5 ступеней):
cp=107/39/0/889/889
   вызовов 107   ИСЧЕРПАЛ БЮДЖЕТ 39 (36%)   огрызков 0   клеток 889   различных 889
mqRefused=2(short=0)     очередь почти ничего не отвергает
```

ДВЕ ПОПРАВКИ К СОБСТВЕННОЙ КАРТИНЕ:
  1. `distinct == cells` — дублей НЕТ, огрызков НЕТ. В норме этот BFS выдаёт нормальные
     маршруты (в среднем 8.3 клетки). Значит вырожденные маршруты из повторяющихся
     клеток (`mqNullEdge=2337` на застое) — СВОЙСТВО ЗАСТОЯ, а не поиска как такового.
  2. Зато в норме видно другое: 39 вызовов из 107 СЪЕДАЮТ ВЕСЬ бюджет в 800 узлов.

Это прямое проявление несоответствия назначения: по своему javadoc `findPath` —
«мгновенный BFS для `FollowEntityTask`, догнать сущность рядом, пока считает физика»,
а используется он источником ОБЫЧНОЙ ходьбы, где цель в 14+ блоках через рельеф.
Треть запросов упирается в потолок даже на здоровом прогоне.

НО САМО ПО СЕБЕ ИСЧЕРПАНИЕ НЕ ВРЕД: прогон взял 5 ступеней. Поэтому правку не
отгружаю по одному этому числу — нужен застойный прогон с `cp` на руках, чтобы
увидеть, во что превращается выдача именно там.

## ⭐ АРХИТЕКТУРА ЗАСТОЯ: ДВА НЕЗАВИСИМЫХ КОНВЕЙЕРА МАРШРУТА (2026-08-26)

Снято ЧТЕНИЕМ, без прогона — противоречие «plan=229 при navRes=0» разрешилось.

У `FastPlanner.plan` ЧЕТЫРЕ вызывающих: `PathFinder:1240`, `FastNavigator:604`,
`FollowEntityTask:351` и `:513`. На застое задача была «добыть кирку», не догон
сущности, а `navRes=0` — значит планы шли из ФИЗИЧЕСКОГО пасфайндера.

И они там НЕ ВЫБРАСЫВАЮТСЯ:

```java
boolean acceptable = fast.complete || arrivesAnyway || fastGuidePartial;
if (acceptable && fast.path.size() >= 2)
    return truncateAtBreaks(Optional.of(fast.toBlockNodes(goal, player)));
```

То есть 229 маршрутов по 48 клеток становятся БЛОК-ПРОВОДНИКОМ для физики.

ИТОГО НА ЗАСТОЕ РАБОТАЛИ ДВА КОНВЕЙЕРА, НЕЗАВИСИМО:
  физический — получал ХОРОШИЕ проводники и ничего не выдавал (`tryEmit`=0 ранее);
  ходовой    — получал ВЫРОЖДЕННЫЕ маршруты от боевого BFS (`mqRefused(short)=2337`).

Это и объясняет главный парадокс серии: планирование здорово, а бот стоит. Никакая
правка В ПЛАНИРОВЩИКЕ этого не сдвинет — расходятся ПОТРЕБИТЕЛИ маршрута.

СЛЕДУЮЩИЙ ЗАХОД: поймать застой с прибором `cp` на руках (базовая линия здорового
прогона уже есть: `cp=107/39/0/889/889`, дублей ноль) и увидеть, во что превращается
выдача боевого BFS именно там. Тогда будет видно, чинить ли его бюджет или переключать
ходовой конвейер на маршрут `FastPlanner`.

## ⛔⛔ ОПЕРАТОР ПОЙМАЛ ДЫРУ В МОЕЙ МЕТРИКЕ (2026-08-26)

Оператор посмотрел два присланных видео и сказал: бот ломает дерево, потом УСТАВЛЯЕТСЯ
В ОДНУ ТОЧКУ и стоит без дела последние 3-5 минут. Я мерил СТУПЕНИ и этого не видел:
ступени, взятые ДО смерти, остаются в зачёте, поэтому труп выглядит как успех.

Написал честную метрику (`deploy/runner/deadtime.py`): доля выборок, где позиция И
число предметов не изменились с прошлой.

```
play10   RUN1 31% мёртв   RUN2 19%   RUN3  4%
play15   RUN1 46% мёртв   RUN2 62%   RUN3 42%
```

Свод, который я называл «ровным 5/5/4», простаивал 42-62% времени. ЭТО МЕНЯЕТ ВСЕ
ОЦЕНКИ СЕССИИ, где я сравнивал прогоны по ступеням: сравнение было по неверной величине.

ДАЛЬШЕ СУДИТЬ ПО `deadtime.py`, а ступени держать как вторичное. Метрика видна снаружи
(позиция + предметы), поэтому не зависит от моих счётчиков внутри мода.

Наблюдение оператора совпадает с профилем D по приборам: `dbTick=12253` (целится в
блок — это и есть «уставился») при `pdWalking=0` (не подходит ни разу).

## ⛔ МЕТРИКА ПРОСТОЯ СРАЗУ ПОКАЗАЛА ГЛАВНОГО ВИНОВНИКА: ПРОФИЛЬ A (2026-08-26)

Первый же прогон под новой метрикой: 95% ПРОСТОЯ, ноль предметов. Раньше такой прогон
числился бы по ступеням как «1-2 ступени», то есть почти нормальный.

```
cp=4732/0/4725/4739/4739   вызовов 4732, ИСЧЕРПАЛ 0, ОГРЫЗКОВ 4725 (99.8%)
diag=0/0/0                 моя правка перевода диагоналей НЕ СРАБОТАЛА НИ РАЗУ
wander=4125 wanderMoved=0  блуждание крутится и НЕ ДВИГАЕТ тело
mqSteps=1  mqStarted=2     за весь прогон сделан ОДИН шаг
pdWalking=0  pdNear=0
```

ЧТЕНИЕ: `exhausted=0` при 99.8% огрызков значит, что поиск выходит РАНО, а не упирается
в бюджет. Его тест успеха — `isWithinDistance(goal, 1.5)`, то есть цель уже в полутора
блоках от бота. Это ПРОФИЛЬ A («маршрут в текущую клетку»), и именно он даёт 95%
простоя, а не бюджет и не диагонали.

ПРАВКА ПРО ДИАГОНАЛИ ОСТАЁТСЯ НЕПРОВЕРЕННОЙ (`diag=0/0/0`): она прошла ворота
(nav 0 провалов, craft 22/22) и ничего не сломала, но эффекта ей приписывать нельзя,
и этот прогон про неё ничего не говорит.

ПРИОРИТЕТ МЕНЯЕТСЯ: профиль A даёт 95% простоя в одном прогоне — это крупнее всего
остального. Копать надо, ПОЧЕМУ цель оказывается в полутора блоках от бота и почему
прибытие при этом не засчитывается. Ранее замерено: совпадения ТОЧНЫЕ (ex50/ytol0),
то есть слои о клетке согласны, а `snapToSelf` в тех прогонах был мал.

## ⭐⭐⭐ «УСТАВИЛСЯ И ВСТАЛ» ПОЧИНЕНО: ПРОСТОЙ 95% -> 19% (2026-08-26)

Наблюдение оператора по видео оказалось точнее моей метрики и запустило всю цепочку.

ПРИЧИНА (дословно из диагностики самого поиска):
...
BFS stuck at 1188,61,-1428 —
  ...
  1,1:OK?   <- единственный проход это ДИАГОНАЛЬ, отвергнутая срезом угла
```

Бот замуровывает себя в собственной яме. Пасфайндер ПРАВ — пути по ходьбе нет.
Не хватало РЕАКЦИИ, а не поиска.

ПЕРВОЕ ЛЕКАРСТВО ОТМЕНЕНО ЗАМЕРОМ: включил готовую ветку «есть цель, нет пути, не
двигался» — `stranded=127` за курс и `mine_diamond FAIL`. Причина структурная: бот,
копающий вниз, ТОЖЕ стоит с целью и не жмёт клавиш.

ПРАВИЛЬНОЕ ЛЕКАРСТВО: различитель по ЧАСАМ ПОСЛЕДНЕГО СЛОМА БЛОКА
(`PlayerExtraController.lastBreakProgressMs`, порог 6 с). Копающий держит их свежими,
замурованный (он блуждает) — нет.

```
ПРОСТОЙ   ДО    95%   96%    (типично 36-62%)
          ПОСЛЕ 23%   19%    12%

stranded=28/dig4   28 спасений, 4 подавления (узнал копание)
ВОРОТА  nav 14/14   craft 22/22   mine_diamond снова PASS
```

МЕТОДИЧЕСКИЙ ВЫВОД: метрика важнее правки. Счёт по СТУПЕНЯМ прятал этот отказ всю
сессию — ступени, взятые до смерти, оставались в зачёте, и труп читался как успех.
`deploy/runner/deadtime.py` теперь основная мера, ступени — вторичная.

## ⛔ АУДИТ ПОПРАВЛЯЕТ МОЙ ЖЕ ВЫВОД: МЕДИАНА ВДВОЕ, НО НЕ ИЗЛЕЧЕНИЕ (2026-08-26)

Вчера я подал результат как «простой 95% -> 19%», взяв ЛУЧШИЙ свод. Аудит на n=5
показал, что это был лучший прогон, а не типичный.

```
ДО    (n=8)   36 42 46 48 50 62 95 96    медиана ~49%
ПОСЛЕ (n=8)   12 12 19 23 23 54 85       медиана ~21%
```

МЕДИАНА УПАЛА ПРИМЕРНО ВДВОЕ — это настоящий сдвиг. Но худший прогон всё ещё 85%,
разброс огромный, и называть это излечением нельзя.

РАЗЛИЧИТЕЛЬ КОПАНИЯ ДЕРЖИТСЯ: `stranded=19/dig29`, `21/dig11` — подавлений больше,
чем пропусков, то есть защита добычи работает (`craft 22/22`, `mine_diamond PASS`).

ВАЖНОЕ: в худшем прогоне (85%) отказ ДРУГОЙ, не тот, что чинили:
```
cp=9255/100/0/19554   огрызков НОЛЬ — поиск здоров
diag=0/247/0          перевод диагоналей работает
mqRefused=3(short=0)  очередь почти ничего не отвергает
pdWalking=446         бот ИДЁТ
stranded=52/dig0      и всё равно 52 спасения
dbTick=220            при этом почти НЕ КОПАЕТ, предметов 11
```
То есть бот не замурован и не стоит — он ходит и НЕ ЗАНИМАЕТСЯ ДЕЛОМ. Это пятый
профиль, и он про выбор задачи, а не про движение.

СЛЕДУЮЩИЙ ЗАХОД: почему при здоровой навигации бот не добывает (`dbTick=220` против
тысяч в рабочих прогонах). Смотреть, чем занята цепочка задач в такие минуты.

## ⭐⭐⭐ ПРОСТОЙ 49% -> 8%: ПОГОНЯ ЗА ОДНИМ ПРЕДМЕТОМ ОГРАНИЧЕНА (2026-08-26)

```
ИСХОДНО    медиана ~49%,  худший 96%
после ямы  медиана ~21%,  худший 85%
СЕЙЧАС      8%   8%   4%      gave3 — потолок реально срабатывает
ВОРОТА     nav 14/14,  craft 21/22 при нуле провалов гейта
```

ПРИЧИНА: `dc=12145/1/0/0/0/same12070` — из 12145 тиков выбирающая задача 12070 раз
выбирала ТУ ЖЕ цель (булыжник в двух блоках под ботом) и ни разу от неё не отказалась.
Сдачи у неё не было вообще. Навигация при этом здорова (`cp` без огрызков,
`mqRefused=3`, `pdWalking=446`) — ни одна правка пасфайндера этого сдвинуть не могла.

ТРИ ПОПЫТКИ, ОДНА И ТА ЖЕ ЛОВУШКА (записать и не повторять):
  1. сравнение цели ПО ССЫЛКЕ — трекер отдаёт новый объект для того же предмета,
     часы обнуляются каждый тик.            gave0
  2. часы в ПОЛЯХ ЭКЗЕМПЛЯРА — задача пересоздаётся, бюджет уходит с ней.
     Ровно этот баг я чинил для подбора предметов ранее в сессии.   gave0
  3. собственное правило «приблизился — сбрось отсчёт» слишком щедро: бот кружит
     вокруг предмета и ставит новые рекорды близости.               gave0
  4. жёсткий потолок 3 минуты на цель, сбрасывается ТОЛЬКО сменой цели.  gave3

ОБЩИЙ ПРИНЦИП, ТРИЖДЫ ПОДТВЕРЖДЁННЫЙ: бюджет принадлежит ЦЕЛИ, а не задаче, и не
должен обнуляться ничем, кроме смены цели. Каждый провал ловился счётчиком `gave<N>`
ДО того, как правка записывалась в успех.

## ⛔⛔ ВТОРОЙ РАЗ ОТЧИТАЛСЯ ПО ЛУЧШЕМУ СВОДУ — ОШИБКА МЕТОДА (2026-08-26)

Аудит n=5 после «49% -> 8%»:

```
29%   69%   27%   44%   0%       gave0 — потолок погони НЕ сработал ни разу
```

ЧЕСТНАЯ КАРТИНА ПО ВСЕЙ ЛИНИИ (объединяя с 8/8/4):

```
ДО      медиана ~49%
СЕЙЧАС  медиана ~27%    (4 8 8 27 29 44 69)
```

Медиана улучшилась примерно ВДВОЕ — это настоящий результат. Но НЕ до 8%: тот свод был
лучшим, а не типичным.

ЭТО ВТОРАЯ ТАКАЯ ОШИБКА ЗА СУТКИ (первая — «95% -> 19%», поправлена тем же способом).
Вывод не про числа, а про метод: ОТЧИТЫВАТЬСЯ ТОЛЬКО ПО МЕДИАНЕ НА n>=5, никогда по
лучшему прогону. Один свод из трёх — это не результат, а наблюдение.

И ГЛАВНОЕ ПРО САМУ ПРАВКУ: `gave0` за весь аудит — потолок погони НЕ ВЫПОЛНЯЕТСЯ.
Значит улучшение медианы принадлежит ПРЕДЫДУЩИМ правкам (различитель «копает/застрял»),
а не ему. Потолок остаётся отгруженным как безвредный (ворота зелёные), но без зачёта.

ОТКРЫТО: почему `gave` срабатывает то 3 раза, то 0 при `same~10000` в обоих случаях.
Вероятно, цель успевает смениться до трёх минут (`samePursuit` рвётся), и тогда нужен
счётчик на ПРИЧИНУ сброса, а не на сам факт сдачи.

## ⭐ ПОПРАВКА: ПОТОЛОК ПОГОНИ НЕ СРАБАТЫВАЕТ ПОТОМУ, ЧТО НЕ ДОЛЖЕН (2026-08-26)

Вчера записал `gave0` как «правка не выполняется». Замер причины сброса это опровергает:

```
same6806/gave3/rst45@100cm
   тот же объект 6806 раз,  сдач 3,  СБРОСОВ ЧАСОВ 45,
   последний сброс вызван «прыжком» цели ровно на 100 см
```

Ровно ОДИН БЛОК — это не дрейф предмета, а ШАГ ПО СЕТКЕ: бот домайнил блок и перешёл
на соседний. Цель меняется ЗАКОННО, и сброс отсчёта в этом случае ПРАВИЛЬНЫЙ.

Значит потолок ведёт себя как задумано: молчит, пока цели честно ротируются (жила,
дерево), и срабатывает, когда одна цель действительно держит бота — тот случай с
`same12070`, где он и дал `gave3`.

ВЫВОД ДЛЯ МЕТОДА: «счётчик равен нулю» НЕ ВСЕГДА значит «код не выполняется». Иногда
это значит «условие правильно не наступило». Различать их можно только счётчиком на
ПРИЧИНУ, а не на факт. Здесь это стоило одного замера и сняло ложный вывод.

Простой в этом своде: 4% и 39%.

## ⛔ ПОРОГ РАСШИРЕН: МЕХАНИЗМ ЗДОРОВЕЕ, ПРОСТОЙ НЕ СДВИНУЛСЯ (2026-08-26)

Оба потолка побеждались дрейфом предмета: порог «та же погоня» был полблока, а замер
на живом застое дал прыжок 82 см (`rst114@82cm`, `pursuit=15/0s`, `dropBudget=0`).
Расширил до двух блоков — шаг по сетке (100 см) по-прежнему считается сменой цели.

```
МЕХАНИЗМ  сбросов часов  rst114 -> rst26..42      сдачи  gave0 -> gave1
ВОРОТА    craft 22/22,  nav 13/14 при нуле провалов гейта
ПРОСТОЙ   27  38  38  73  0  %      медиана ~38%
```

ЧЕСТНО: механизм стал работать правильно (это видно числом), а ПРОСТОЙ НЕ УЛУЧШИЛСЯ —
медиана в том же диапазоне, что и до правки (~27% в прошлом аудите). Приписывать ей
улучшение нельзя; она чинит корректность потолка, а не долю простоя.

Отдельно любопытное: один сброс вызван прыжком цели на 109 МЕТРОВ — то есть цель
меняется на совсем другую, далёкую. Это не дрейф и сброс там правильный.

ВЫВОД ДЛЯ СЛЕДУЮЩЕГО ЗАХОДА: линия «бюджеты погони» исчерпана — она вылечена и больше
не является узким местом. Остаток простоя (медиана ~30%) надо искать в другом месте,
и начинать снова с дампа застоя, а не с этой линии.

## ⛔ ПОДОЗРЕНИЕ НА СПАСЕНИЕ СНЯТО ЗАМЕРОМ: ОНО СТОИТ 2.5% ПРОГОНА (2026-08-27)

Дамп прогона с 73% простоя показал `Main task: <Shimmying>` при полностью простаивающем
остальном (`pdWalking=0`, `dbTick=0`, `wander=0`) — выглядело так, будто цепочка
«выбраться» сама и съедает прогон. Поставил счётчик владения:

```
stranded=35/dig2/own303
   35 спасений, 2 подавления, ВЛАДЕЛА БОТОМ 303 тика
   303 тика ~ 15 секунд из 610 = 2.5% прогона
```

Дамп — это СНИМОК одного момента, и он показал спасение просто потому, что застой его
и вызвал. Приписывать ему долю простоя было бы ошибкой рассуждения по одному кадру.

Простой в этом своде: 35% и 35%. Источник остатка НЕ найден — ни погоня (линия
исчерпана), ни спасение (2.5%), ни навигация (счётчики здоровы).

СЛЕДУЮЩИЙ ЗАХОД: мерить не подозреваемых по одному, а РАСПРЕДЕЛЕНИЕ — чем именно занят
бот в мёртвых выборках. Нужен счётчик владения по КАЖДОЙ цепочке задач, тогда 35%
разложатся на слагаемые вместо очередной гипотезы.

## ⭐ КАРТА ПРОГОНА: КУДА УХОДИТ ВРЕМЯ (2026-08-27)

Вместо проверки подозреваемых по одному — счётчик владения на КАЖДУЮ цепочку и разбор
основной по листьям. Впервые распределение, а не гипотеза.

```
chains=[UserTaskChain:11526  UnstuckChain:808]      93% / 7%

внутри основной цепочки (снимок, всего 8455 тиков):
  dc   выбор ближайшего объекта   6393   <- 76%
  dbTick  разрушение              2618
  wander  блуждание               2324
  pdEnter привод навигации        1513
  pnb  постановка блоков           606
  ciTick сбор                      430
  mv   слоты                       238
  iw   взаимодействие                0
```

ВЫВОД: простой (медиана ~31-35%) сидит ВНУТРИ основной цепочки, в выборе и преследовании
ближайшего объекта. НЕ в спасении (7% всей цепочки, 2.5% прогона по владению), НЕ в
навигации, НЕ во взаимодействии (ноль).

Ветки пересекаются (один тик проходит через несколько), поэтому сумма больше总а — но
доминирование `dc` однозначно.

СЛЕДУЮЩИЙ ЗАХОД: сузить внутри `dc` — коррелировать МЁРТВЫЕ выборки с тем, что в этот
момент делает выбор (преследует? ждёт? пересчитывает?). Бюджеты там уже вылечены, значит
дело не в сдаче, а в самой продуктивности преследования.

## ⭐⭐⭐ ЛУЧШИЙ РЕЗУЛЬТАТ СЕССИИ, ПО МЕДИАНЕ n=5 (2026-08-27)

```
                НАЧАЛО СЕССИИ        СЕЙЧАС (n=5)
простой         медиана ~49%         8 19 23 23 13  -> медиана 19%
предметы        6-24                 41 44 80 63 43
ступени         0-7, часто 0-2       5 5 5 3 5      -> медиана 5
ВОРОТА          nav 14/14,  craft 22/22,  оба без провалов гейта
```

ЧТО ЭТО ДАЛО (цепочка правок, каждая с замером):
  1. различитель «копает / застрял» по часам последнего слома блока;
  2. бюджет погони принадлежит ЦЕЛИ, а не задаче, и не сбрасывается ничем, кроме
     смены цели (три попытки, три ловушки, четвёртая сработала);
  3. порог «та же погоня» расширен до двух блоков — предмет ДРЕЙФУЕТ;
  4. сдача преследования, которое стоит вдали от цели, при исключении «в пределах
     удара» (первая версия без исключения роняла добычу: craft 22/22 -> 20/22);
  5. отсчёт простоя сбрасывается СБЛИЖЕНИЕМ с целью, а не любым движением —
     топтание на месте больше не считается за прогресс.

КЛЮЧЕВОЙ ЗАМЕР, приведший к 4-5: из 10445 тиков выбора тело двигалось 248 раз и стояло
10196 (98%), а разложение стояния дало 41% «у цели» (работа) против 59% «вдали»
(застревание). Лечили ровно вторую половину.

МЕТОД, КОТОРЫЙ ЭТО ВЫТАЩИЛ: не проверять подозреваемых по одному, а пометить всех и
дать распределению назваться. Карта прогона (`chains=[...]` + разбор по листьям) стоила
одного замера и заменила три захода угадывания.

## ⛔ ГИПОТЕЗА «ВЫБРАСЫВАЕТ КИРКУ И ГОНЯЕТСЯ ЗА НЕЙ» НЕ ПОДТВЕРЖДЕНА (2026-08-27)

Выглядело убедительно: `throwers=[BeatMinecraftTask:362 x546]`, в коде на 1440 стоит
`throwAwayItems(WOODEN_PICKAXE)`, а дамп застоя показывал
`<Pickup Dropped Items: [[wooden_pickaxe]]>`. Замер по типам это опроверг:

```
thrown=[spruce_stairsx23016 sandx11508 red_sandx11508 torchx11508 oak_saplingx11508]
dropPick=[spruce_log
```

Кирки в верхушке НЕТ, гонится бот за БРЕВНОМ, а выбрасывается ровно мусор — как и
задумано. Условие выброса кирки к тому же корректно: только при наличии каменной.

И ОТДЕЛЬНО — ИЗЪЯН МОЕГО ЖЕ ПРИБОРА: числа 11508 совпадают с числом тиков, потому что
я считал ВЫЗОВЫ `throwAwayItems`, а он зовётся каждый тик независимо от того, есть ли
этот предмет в инвентаре. Счётчик мерил намерение, а не действие. Чтобы мерить выброс,
считать надо внутри `throwAwaySlots` по фактически найденным слотам.

ЧТО ОСТАЁТСЯ ВЕРНЫМ: потолок лестницы — пять ступеней, и он держится ЖЁСТКО.
```
RUN1 каменные инструменты на 316.8 с -> осталось 290 с, ничего
RUN2 на 225.2 с -> осталось 385 с, ничего
RUN3 на  66.7 с -> осталось 545 с, НИЧЕГО
```
Девять минут активной работы (простой всего 19%) не дают следующей ступени.
Следующий заход — что именно требуется для печи/угля и на каком шаге это встаёт.

## ⭐ ПОТОЛОК ЛЕСТНИЦЫ НАЗВАН ТОЧНО: ОДНА НЕДОСТИЖИМАЯ ЦЕЛЬ (2026-08-27)

Вместо рассуждений по одному кадру — ОПРОС живой цепочки задач каждые 5 секунд
(`deploy/runner/tasksample.py`, новый инструмент):

```
98.7%  77 проб   Getting to block {x=1449, y=63, z=89}
 1.3%   1 проба  (none)
```

А в этой клетке — `spruce_log`, сплошной ствол. `GetToBlockTask` требует ЗАНЯТЬ клетку,
что невозможно, пока бревно стоит. Бот тратит СЕМЬ МИНУТ из десяти на одну недостижимую
цель. Вот и весь жёсткий потолок в пять ступеней.

ПЕРЕПРОВЕРКА `breakGoalIsReach` (прежний вердикт «нейтрально» считался по СТУПЕНЯМ —
метрике, которая, как показал оператор, прячет именно такие отказы):

```
             простой               ступени
  ON    0 27 15 4 14 -> мед 14%    6 4 2 0 5 -> мед 4
  OFF   8 19 23 23 13 -> мед 19%   5 5 5 3 5 -> мед 5
  dbReachGoal=7967 — ветка работает интенсивно, не спит
  ВОРОТА: nav 14/14, craft 22/22 — обе базы зелёные
```

Один прогон ПРОБИЛ потолок (6 ступеней, уголь на 246.7 с) — такого не давал ни один
прогон другой сборки. Но один из пяти это наблюдение, а не результат, а медиана ступеней
упала с 5 до 4 при разбросе 0-6 вместо 3-5. ОСТАВЛЕН ВЫКЛЮЧЕННЫМ.

ЧТО ЭТО ЗНАЧИТ: диагноз верен, лекарство — нет. Менять ТИП цели под задачей значит
чинить симптом. Правильное место — ИСТОЧНИК: задача, которой нужно СЛОМАТЬ блок, не
должна просить «займи клетку». Это следующий заход.

## ⛔⛔ КАПКАН СТЕНДА: КОНФИГ ПЕРЕЖИВАЕТ ОТКАТ В ИСХОДНИКЕ (2026-08-27)

Выключил `breakGoalIsReach` в исходнике, собрал, развернул — и следующий же опрос
цепочки показал ВЕТКУ ВКЛЮЧЁННОГО ФЛАГА (`Getting within 3 blocks of`,
`DestroyBlockTask:713`). Проверка объяснила:

```
deploy/run/data/tester1/config/tungsten.json:  "breakGoalIsReach": true
deploy/run/data/tester2/config/tungsten.json:  "breakGoalIsReach": true
```

Мод ЧИТАЕТ конфиг стенда, а не переписывает его из умолчаний. Значит правка флага в
исходнике + сборка + деплой НЕ МЕНЯЮТ поведение стенда, если флаг уже записан в json.
Привёл оба конфига в соответствие (false) и перезалил.

ЧТО ЭТО ЗНАЧИТ ДЛЯ ЗАМЕРОВ:
  - базовые прогоны «без флага» (5 5 5 3 5 ступеней) снимались ДО включения — корректны;
  - прогон с 20.9% на доминирующем листе шёл С ВКЛЮЧЁННЫМ флагом, хотя я считал его
    выключенным. Как сравнение он НЕ ГОДИТСЯ и в выводы не идёт.

ПРАВИЛО НА БУДУЩЕЕ: после смены флага ПРОВЕРЯТЬ `deploy/run/data/*/config/tungsten.json`,
а не доверять исходнику. В этой сессии я трижды попадался на «замер со старыми флагами»,
и это четвёртый случай того же класса — но пойманный до публикации вывода.

## ⭐ КОНТРОЛЬНАЯ ВЕТКА ПРОВЕРЕНА ЭМПИРИЧЕСКИ (2026-08-27)

После находки про конфиг стенда я обязан был убедиться, что «выключено» действительно
выключено, а не считается таковым. Опрос цепочки при `config=false`:

```
Collect cobblestone -> Mine And Collect -> Mining or Collecting
    -> Destroy block at X -> Getting to block {X}      <- ВЕТКА ВЫКЛЮЧЕННОГО ФЛАГА
```

Появилось «Getting to block» вместо «within 3 blocks» — значит конфиг главнее исходника,
моя правка конфига подействовала, и прежнее A/B было честным: «включённый» прогон шёл
включённым, а молча провалился именно ОТКАТ.

И цепочка дословно называет заказчика недостижимой цели: `DestroyBlockTask` просит
ЗАНЯТЬ клетку блока, который сам же собирается сломать.

ПОЧЕМУ ЗАМЕНА ЦЕЛИ (`breakGoalIsReach`) НЕ СРАБОТАЛА ЧИСТО — гипотеза для следующего
замера, НЕ вывод: `GetWithinRangeOfBlockTask(pos, 3)` считает задачу выполненной по
ДИСТАНЦИИ, не проверяя линию видимости. Бот встаёт в трёх блоках за препятствием,
подход «выполнен», а сломать нельзя. Это объясняло бы и рост разброса ступеней (0-6).
Проверять счётчиком `noReach`/`dbBlocked` при включённом флаге, а не рассуждением.

## ⭐⭐ ОПЕРАТОР НАШЁЛ ВТОРУЮ СЛЕПУЮ ЗОНУ МЕТРИКИ: ЧЕЛНОК (2026-08-27)

Он посмотрел видео и описал рисунок: бот идёт ломать блок, уходит что-то подбирать,
возвращается — и так бесконечно. Его слова: «наверное, это ещё хуже тупорылого
зависания». Верно, и вдвойне: `deadtime.py` этого НЕ ВИДИТ — тело движется, значит
прогон читается как здоровый.

Написал `deploy/runner/shuttle.py`: отношение ЧИСТОГО СМЕЩЕНИЯ к пройденному пути по
окнам. Около 1.0 — движение по делу, около нуля — челнок.

```
play44 (breakGoalIsReach ВКЛ)     play40 (ВЫКЛ)
  RUN1 10/21 челнок, мед 0.27       RUN1 1/21, мед 0.59
  RUN4  6/14 челнок, мед 0.28       RUN2 0/20, мед 0.59
  RUN5  3/14 челнок, мед 0.47       RUN3 0/18, мед 0.80
  RUN2  0/21,        мед 0.60       RUN4 0/19, мед 0.79
  RUN3  0/22,        мед 0.77       RUN5 0/7,  мед 0.95
```

ЭТО ОБЪЯСНЯЕТ РАЗЛЁТ СТУПЕНЕЙ (6/4/2/0/5) при включённом флаге: бот не зависал, он
БЕГАЛ ВХОЛОСТУЮ, а простой при этом был низким. Два моих замера — простой и ступени —
оба пропускали этот отказ.

ВЫВОД ДЛЯ ПРОЦЕССА: метрика должна ловить не только «стоит», но и «двигается зря».
Дальше гейт по трём числам: простой, челнок, ступени. Второй раз за сессию внешний
взгляд поправил измерение, а не код — и оба раза это было дороже правки.

## ⭐⭐ ЧЕЛНОК ИЗМЕРЕН: ЦЕПОЧКА ЗАДАЧ ОБНУЛЯЕТСЯ 59% ВРЕМЕНИ (2026-08-27)

Оператор описал по видео: бот идёт ломать блок, уходит подбирать, возвращается, и так
бесконечно. Гистограмма листьев это НЕ ловит — оба листа выглядят занятыми. Считаю
ПЕРЕКЛЮЧЕНИЯ:

```
ПЕРЕКЛЮЧЕНИЙ: 40 из 69 выборок — 59% времени задача МЕНЯЕТСЯ
  15  Getting to block ...  ->  (нет задачи)
  15  (нет задачи)          ->  Getting to block ...
  10  Getting to block X    ->  Getting to block Y
(нет задачи) занимает 29% всех выборок
```

ЭТО ДВА РАЗНЫХ ЯВЛЕНИЯ, а не одно:
  1. ОБНУЛЕНИЕ ЦЕПОЧКИ — 30 из 40 переключений. Бот идёт к блоку, цепочка пустеет,
     задача возникает заново. Почти треть прогона вообще без задачи.
  2. СМЕНА ЦЕЛИ — 10 переключений, блок X на блок Y: тот самый бег между позициями.

ПОБОЧНО РАЗРЕШИЛАСЬ СТАРАЯ ЗАГАДКА: я трижды натыкался на то, что бюджеты в ПОЛЯХ
ЭКЗЕМПЛЯРА обнуляются (подбор предметов, выбор ближайшего, часы простоя). Теперь ясно
почему: задачи ДЕЙСТВИТЕЛЬНО пересоздаются постоянно, и это видно числом.

СЛЕДУЮЩИЙ ЗАХОД: почему цепочка пустеет. Это дороже смены цели (30 против 10) и
объясняет, почему любое состояние внутри задачи ненадёжно. Начинать с того, кто
возвращает null/пустоту в `UserTaskChain` при живой главной задаче.

## ⛔ ЧЕЛНОК ИМЕЕТ ДВА ИСТОЧНИКА, ВТОРОЙ ОСТАЛСЯ (2026-08-27)

Выключение спасения убрало качание «шимми <-> блок» (21 переключение из 37), но НЕ
убрало челнок целиком. Базовая линия на пересозданном стенде, обе правки выключены:

```
простой   43%   52%   15%
челнок    5/16  0/13  2/8
ступени    0     3     4
```

ВТОРОЙ ИСТОЧНИК виден в раскладе переключений: `Getting to block X -> Getting to block Y`
(9-11 из 21 при выключенном спасении). Бот ПЕРЕИЗБИРАЕТ целевой блок и ходит между
двумя. Это классическая осцилляция «выбери ближайшее», когда сам выбирающий движется:
два блока по очереди становятся ближайшими.

ВАЖНО: это НЕ выбирающий предметы (`dc=343/3/1/...` — новых погонь всего 3), а выбор
БЛОКА внутри `MineAndCollectTask`. Разные механизмы, не путать.

СЛЕДУЮЩИЙ ЗАХОД: посчитать переизбрания блока и, если их много, добавить гистерезис —
не менять цель, пока новая не ЗАМЕТНО лучше. Сперва счёт, потом правка.

ОТДЕЛЬНО ПРО СТЕНД: он начал ронять py4j (из четырёх прогонов данные дал один).
Пересоздание контейнера `mc-tester1` вылечило — ноль обрывов после. Записать как
штатную процедуру при появлении `Py4JNetworkError` в логе свода.

## ⛔ ПЕРЕИЗБРАНИЕ БЛОКА — НЕ ИСТОЧНИК ЧЕЛНОКА (2026-08-27)

Версия была: «выбери ближайшее» осциллирует, когда выбирающий движется, поэтому бот
ходит между двумя блоками. Счётчик на смену цели её снимает:

```
mineRe=28@141cm/back2
   28 переизбраний, последний скачок 141 см (соседний блок),
   ВОЗВРАТОВ на только что покинутый блок — ВСЕГО 2
```

28 смен цели с шагом в полтора блока — это проход по жиле, а не качание. Два возврата
из двадцати восьми осцилляцией не являются.

ИТОГО ПО ЧЕЛНОКУ: один источник найден и убран (цепочка спасения, 21 переключение из
37), второй НЕ НАЙДЕН. Остаточный челнок (5/16 окон в одном прогоне из трёх) остаётся
необъяснённым, и версия про выбор блока закрыта отрицательно.

СЛЕДУЮЩИЙ ЗАХОД: брать конкретный прогон с высоким челноком и смотреть ТРАЕКТОРИЮ —
между какими двумя точками бот ходит и какая задача в этот момент активна. Разложение
по переключениям уже есть (`tasksample.py`), не хватает привязки к координатам.

## ⭐⭐ ОСТАТОЧНЫЙ ЧЕЛНОК НАЙДЕН НА ТРАЕКТОРИИ: ПОДХОД К ПРЕДМЕТУ (2026-08-27)

Две мои версии были отклонены счётчиками (переизбрание блока, выбор предмета в общем
чузере). Третий заход — смотреть саму ТРАЕКТОРИЮ (`deploy/runner/pacing.py`, новый
инструмент: возврат в точку, виденную 3+ выборки назад, с задачей на обоих концах):

```
выборок 69, возвратов: 12
  3x  назад в (-18.3, 87.0, -551.7)  там: Approach entity   сейчас: Wander 5.0
  1x  туда же                        Approach entity        Wander 10.0
  1x  ...                            Destroy block          Moving [logs]
```

ДОМИНИРУЮЩИЙ РИСУНОК — «подойти к предмету <-> блуждание» на ОДНОМ пятачке: бот идёт к
выпавшему предмету, не берёт его, уходит блуждать, возвращается, снова идёт. Четыре
возврата из двенадцати — на одну точку.

Это ТРЕТИЙ механизм, не совпадающий с двумя отклонёнными: не выбор блока и не общий
чузер, а `Approach entity` (`AbstractDoToEntityTask`) со своими счётчиками
(`dte=`, `entityCloseWalk=`).

И это ровно то, что оператор описал словами: «идёт подобрать, возвращается, и так
бесконечно».

СЛЕДУЮЩИЙ ЗАХОД: почему подход к предмету не завершается подбором. Сперва прочитать
`dte`/`entityCloseWalk` на прогоне с высоким челноком, потом править.

## ⛔ ПОДБОР ПО ДИСТАНЦИИ: ЧЕЛНОК ИСЧЕЗ, НО ПРИЧИННОСТЬ НЕ ДОКАЗАНА (2026-08-27)

```
                  простой              челнок
БАЗА (обе off)    43 52 15 %           5/16   0/13   2/8
С ПРАВКОЙ         12 42 40 22 %        0/7  0/15  0/4  0/4   <- ноль везде
ВОРОТА            nav 14/14,  craft 22/22
```

Челнок пропал во ВСЕХ четырёх прогонах против двух из трёх на базе. Но механизм-счётчик
`near0`: случай «предмет в двух блоках и не «ударяем»» НЕ НАСТУПИЛ НИ РАЗУ, значит
правка не выполнялась и заслугу ей приписывать НЕЛЬЗЯ.

Два объяснения, и я не могу их различить имеющимися данными:
  - другой мир/спавн (прогоны не на одинаковых условиях);
  - правка предотвращает состояние, а не считает его: бот доходит до предмета раньше,
    и «рядом, но не ударяем» просто не успевает возникнуть.

ПОКАЗАТЕЛЬНО ДРУГОЕ ЧИСЛО: `dte=1` против `dte=4720` до правки — подход к сущности почти
не используется, а `entityCloseWalk=479` против нуля: ближний подход ЗАРАБОТАЛ. Это
согласуется со вторым объяснением, но само по себе тоже не доказательство.

ЧТО НУЖНО: A/B на ОДИНАКОВЫХ условиях (один и тот же мир), флаг on/off, три метрики.
Пока правка остаётся включённой как безвредная (ворота зелёные), но БЕЗ ЗАЧЁТА.

## ⛔ ПАРНЫЙ A/B НА ОДИНАКОВЫХ МИРАХ ОТКЛОНИЛ ПРАВКУ ПОДБОРА (2026-08-27)

Непарный свод выглядел отлично (ноль челнока в четырёх прогонах), но счётчик механизма
читался `near0` — значит приписывать было нечего. У стенда есть `--pin/--pin-alt`,
который УДЕРЖИВАЕТ мир для ветки B: единственный честный способ сравнить.

```
пара 1   OFF 54% простоя, челнок 0/3   |   ON 62%, окон нет
пара 2   OFF 35%,              2/3     |   ON 56%, 1/5
пара 3   OFF 68%,              3/4     |   ON 60%, 5/7
медиана простоя:  OFF 54%    ON 60%
```

На равных условиях правка не лучше и по медиане хуже; челнок тоже не улучшился.
ВЫКЛЮЧЕНА — в исходнике и в обоих конфигах стенда.

ДИАГНОЗ ПРИ ЭТОМ ОСТАЁТСЯ ВЕРНЫМ САМ ПО СЕБЕ: спрашивать `canHitEntity()` — линию
видимости и досягаемость УДАРА — про выпавший предмет логически неверно, и `dte=4720/0`
показывает четыре тысячи отказов подряд. Но неверный вопрос в коде и причина потери
прогона — разные вещи, и парные числа говорят, что это не она.

ЦЕННОЕ ЗДЕСЬ — ИНСТРУМЕНТ: `--pin/--pin-alt` даёт сравнение на одном мире. Все прежние
сравнения этой сессии были НЕПАРНЫМИ, то есть слабее. Дальше спорные правки судить
только парным A/B.

## ⛔ ЗАВИСАНИЕ ОБЪЯСНЕНО: ЗАЩЁЛКА, КЛЮЧ ОТ КОТОРОЙ ЛЕЖАЛ ЗА НЕЙ (2026-08-28)

Жалоба юзера «уставился в одну точку и встал на 3-5 минут» имеет точную причину.
`barrenStreak` растёт ТОЛЬКО когда истекает захват СУЩНОСТИ без сокращения дистанции,
и обнуляется только удачным захватом или стартом нового. Но гейт стоял в `tryPathTo` и
резал ВСЕХ, включая блуждание. Два неудачных преследования -> счётчик 2 -> блуждание
отказано -> бот не двигается -> новых захватов нет -> обнулить некому. До конца прогона.

```
tp = 0/0/1444/0      отказы: загрузка / лимит / ЗАЩЁЛКА / задержка
wanderTung = 1449/24 выбрано целей / принято
wander = 7047 тиков, wanderMoved = 64   -> тело двигается в 0.9% тиков
```

Парный A/B: простой 62->38% и 15->0% в парах, где защёлка ВЗВОДИЛАСЬ; третья пара
(`tp=0/0/0/0`) о флаге ничего не говорит. Гейт теперь только для захватов сущностей.

## ⛔ ЧЕТЫРЕ ПРАВКИ ПРИНЯТЫ ПАРНЫМ A/B, ОДНА ОТКЛОНЕНА (2026-08-28)

ПРИНЯТЫ: якорь спирали блуждания (простой 35->19, предметы 11->23); бан цели, на которую
потрачен весь бюджет (простой 31->19 и 23->8 в информативных парах); эскалация спасения,
переживающая собственный рывок (челнок 0 из 3 прогонов); снятие защёлки с обычной ходьбы.
ОТКЛОНЕНА: подбор предметов по расстоянию (на равных мирах хуже по медиане).

МЕТОД, КОТОРЫЙ ВСЁ РЕШИЛ: `--pin/--pin-alt` удерживает мир для второй ветки. Все более
ранние сравнения были НЕПАРНЫМИ и потому слабее. И ключевое: счётчик механизма отделяет
ИНФОРМАТИВНУЮ пару от пустой — трижды за сессию это спасало от ложного вывода.

## ⛔ ФИЗИЧЕСКАЯ НОГА ЧИНИТСЯ: «ПРИБЫТЬ» И «ОСТАНОВИТЬСЯ» — РАЗНОЕ (2026-08-28)

Старая запись «физическая нога не даёт узлов» закрыта, но причина оказалась НЕ в
генераторе ходов. Он не голодает: `children = 3897306/187823/181743`. Ход событий:

```
tryEmit = 209/45     164 ЗАВЕРШЁННЫХ поиска выброшены
srch    = 173/0/0    ниже по цепочке — пусто
```

`tryExecutePath` достижим только из `isPathComplete`, то есть узел УЖЕ прошёл проверку
цели. Его отбрасывали за остаточную скорость — то есть за самый обычный случай: добежал
до места на ходу. Дальше пустой путь -> `tick==path.size()` -> «прибыл» -> перепланирование.

ПЕРВАЯ ВЕРСИЯ ПРАВКИ БЫЛА НЕВЕРНОЙ и это показал A/B: как ПРЕДПОЧТЕНИЕ она роняла сбор
предметов вдвое (медиана 22 -> 12) при улучшении простоя — подпись «прибываю с разгона».
Как ЗАПАСНОЙ вариант (после 40 неудачных проверок цели) она даёт простой 50 -> 36 и
предметы 14 -> 25, а разброс схлопывается: контроль 0-60%, ветка с правкой 35-40%.

## ⛔ ГДЕ ПРОХОД УПИРАЕТСЯ (измерено 2026-08-28)

Медиана 4-5 ступеней, потолок — каменные инструменты. ДВАДЦАТИминутный прогон даёт те же
4 ступени, что и десятиминутный: потолок НЕ временной.

Проб задачной цепочки: 51.4% прогона — `Wander for 5.0 blocks`, родитель —
`Place [crafting_table] -> Place structure -> Wander`. При этом `pnbExit=3885/4/6433/0`:
место под установку находится ВСЕГДА (noSpot=0), детерминированный путь достигается 6433
раза. То есть логика установки не виновата — виновата выдача пути (см. выше).

ЗАКРЫТО ЗА ОТСУТСТВИЕМ ПРЕДМЕТА: гипотеза «цель за 1800 блоков». `minePick=0/6586/max69`,
`dcFar=69`. Дальних целей нет; я неверно прочёл СВОЙ проб — принял координаты ЦЕЛИ за
позицию бота и вывел из этого расстояние.

## ⛔ ЧТО ОСТАЁТСЯ ОТКРЫТЫМ

  - разброс прохода: 3 ступени против 7 между соседними прогонами;
  - точечный застой на 1219.5,104.1,-843.5 УЛУЧШЕН, но не снят: бот теперь ИДЁТ
    (mqStarted=17, состояние "walking" вместо "planning"), но цель за 90с не берёт.
    Репро работает, счётчики на месте.

## ⛔ МЕТОД, КОТОРЫЙ СРАБОТАЛ (записать и держаться)

Перебор со счётчиками, а не рассуждение. Девять гипотез закрыты числами, включая ЦЕЛУЮ ОСЬ
"ширина перебора" (три точки: 170 / 959506 / 1407994 кандидата — ни одна не давала маршрут).
Трижды пойманы ошибки СВОИХ приборов (счётчик в мёртвой ветке, в непроходимой ветке,
переполнение int) — каждый раз следующим измерением, а не отгрузкой вывода.

<!-- CHAIN-CLOSED-AT-CREATENODE-2026-08-25 -->
## ⛔ ЦЕПОЧКА ЗАМКНУЛАСЬ: createNode ОТБРАКОВЫВАЕТ ПОЧТИ ВСЁ (2026-08-25)

    children = 168/82/82   сырых / валидных / вставлено, за 131 грубый поиск
                           = ОДИН сырой ход на узел

Генератор НЕ узкое место: он перебирает вперёд/вправо/влево по окну рыскания
desiredYaw +/- 134.4 градуса шагом 22.5, со спринтом и без — десятки кандидатов на узел.
А rawChildren считает то, что метод ВЕРНУЛ, то есть уже после отсева "if (n != null)".

Значит createNode возвращает null почти на всё — на ровном поле без препятствий.

ПОЛНАЯ ЦЕПОЧКА, ОТ СИМПТОМА ДО КОРНЯ:

    генератор предлагает десятки -> createNode отбраковывает почти все -> выживает один
    -> поиску нечем ветвиться -> исчерпание -> маршрут не выдаётся -> исполнителю нечего
    исполнять -> бот стоит

Это один корень для: застоя на 1219.5,104.1,-843.5, 135 добычей без шага, и
wander_recovery с wanderMoved=0.0 на пустом поле.

⛔ ОТДЕЛЬНО — ОШИБКА МОЕГО ПРИБОРА, ПОЙМАННАЯ САМИМ. Предыдущий замер дал children=0 и
я заключил "потомки не порождаются". Счётчик стоял в ветке, работающей только при >25
валидных потомков за одно разворачивание; реальные дают два-три, поэтому всегда шла другая
ветка. Ноль отозван, счётчик переставлен.

СЛЕДУЮЩИЙ ЗАМЕР, И ОН МАЛЕНЬКИЙ: какая именно проверка внутри createNode отбраковывает.
Симуляция хода — последнее неизмеренное звено.

<!-- PHYSICS-LEG-EMPTY-2026-08-25 -->
## ⛔ ДЕФЕКТ ЛОКАЛИЗОВАН: ФИЗИЧЕСКАЯ НОГА ПОИСКА НЕ ДАЁТ УЗЛОВ (2026-08-25)

Измерено на wander_recovery — плоское поле 45x45, НИ ОДНОГО препятствия, цель в ~30 блоках:

    wanderTung = 2/2      цель выбрана и ПРИНЯТА tryPathTo
    bs         = 194/194  блок-спейсовый (грубый) поиск отрабатывает целиком
    srch       = 1/0/0    физический поиск не выдаёт узлов
    exArrived  = 49       исполнитель 49 раз докладывает "прибыл"
    exSprint   = 0/0      воспроизведя НОЛЬ тиков
    mqStarted  = 0        очередь движений не стартовала
    wanderMoved= 0        тело не прошло ничего

Цепочка полностью: цель принята -> грубый путь найден -> физическая нога возвращает пусто ->
исполнитель получает ПУСТОЙ путь -> `tick == path.size()` истинно сразу -> "прибыл" ->
привод перепланирует -> по кругу.

⛔ ПРОВЕРЕНО, ЧТО ЭТО НЕ ВЕТКИ ПУСТОГО ПУТИ: в логе НОЛЬ строк "At the wall" и "At the gap"
(обе отдают `setPath(new ArrayList<>())` намеренно). На плоском поле их и не должно быть.
Значит пусто приходит из основной выдачи поиска.

ЭТО ТА ЖЕ БОЛЕЗНЬ, ЧТО ЗАСТОЙ ПРОХОДА на 1219.5,104.1,-843.5 — там тоже цель принята,
`mqStarted=0`, `pdWalking=0`. Два открытых пункта схлопнулись в один, и теперь у него есть
дешёвая репродукция: две минуты на плоском поле вместо 30-минутного прогона.

СЛЕДУЮЩИЙ ЗАХОД: читать физическую ногу (`PathFinder.search` после того, как блок-путь
получен) и найти, почему она отдаёт пустой список на ровной земле. Прибор уже есть: `srch`,
`bs`, `exArrived`/`exSprint`.

<!-- WANDER-COUPLING-2026-08-25 -->
## ⛔ wander_recovery: НАЙДЕНА СВЯЗЬ ДВУХ ПРАВОК, ЛЕЧЕНИЕ НЕ НАЙДЕНО (2026-08-25)

Курс падает, и причина — сцепка моих же двух изменений:

  1. Возврат `Nav.cancel()` в no-op вернул craft 15/22 -> 21/22 (шесть курсов). ЭТО ВЕРНО.
  2. Но моё блуждание гейтилось на `!Nav.isExecutingRoute()`, а этот признак становился
     ложным ИМЕННО потому, что `Nav.cancel()` глушил тунгстен. Побочный эффект убран —
     признак больше не сбрасывается — ворота не открываются.

Числа на одном и том же курсе, в одиночном прогоне:

    до возврата cancel:   wanderMoved 19.9   PASS
    после:                wanderMoved  0.0   FAIL   (и overallMoved 0.0 — тело стоит вообще)

⛔ ЧТО ПРОВЕРЕНО И НЕ СРАБОТАЛО (не повторять):
  - сброс лока в блуждании каждый тик — УРОНИЛ четыре базовых курса крафта, откачено;
  - чистка бесплодной серии при сбросе прогона — верна сама по себе, курс не лечит;
  - снятие условия `!isExecutingRoute()` из гейта — `wanderMoved` остался 0.0, откачено.

СЛЕДУЮЩИЙ ЗАХОД, И ТОЛЬКО ТАК: сначала ПРИБОР внутри самого блуждания —
`wanderTungPicked/Driven` уже есть в коде, но НЕ выведены в py4j. Вывести и посмотреть,
выбирается ли цель вообще и принимает ли её `tryPathTo`. Три правки подряд сделаны без
этого числа, и все три мимо.

<!-- G0-REPAIRED-2026-08-25 -->
## ⭐ РЕГРЕССИЯ G-0 ПОЧИНЕНА: craft 15/22 -> 21/22 (2026-08-25)

    было после вырезки:   craft 15/22, отказов гейтов 7
    стало:                craft 21/22, отказов гейтов 1  (остался wander_recovery)
    nav:                  14/14, цел всё время

⛔ ОДИН КЛАСС ОШИБКИ, ПЯТЬ РАЗ. Вырезая баритон, я заменял его вызовы «живыми
эквивалентами». Но эти вызовы были МЁРТВЫМИ — баритон давно не вёл тело, и они месяцами
не делали НИЧЕГО. Пять мест были настроены именно под это:

    Nav.pause()            requestPause()  -> holdStill()      реально тормозит тело
    Nav.cancel()           forceCancel()   -> Tungsten.stop()  35 вызывающих мест
    Nav.clearGoal()        onLostControl() -> Tungsten.stop()
    Nav.cancelEverything() cancelEverything() -> Tungsten.stop()
    Nav.stopExploring()    onLostControl() -> Tungsten.stop()

Худшее — внутри самого привода:

    if (!busy && pf != null) { pf.find(...); }   // пнули поиск
    Nav.cancel();                                // и тут же убили

Поиск запускался и глушился на одном тике, каждый тик.

ПРАВИЛО, КОТОРОЕ ИЗ ЭТОГО СЛЕДУЕТ: **заменять мёртвый вызов рабочим — НЕ бесплатно.**
Прежде чем подставлять эквивалент, проверить, делал ли старый вызов хоть что-то. Если нет —
правильная замена это НИЧЕГО, а не «то же самое, но на новом движке».

ОСТАЁТСЯ: `wander_recovery`. Странность измерена: **19.9 блока в одиночном прогоне и 0.0
внутри полной сюиты** на одном коде. Разница в состоянии между курсами. Попытка лечить это
сбросом лока в блуждании УРОНИЛА четыре базовых курса крафта и была откачена — записано,
чтобы не повторяли.

<!-- G0-REGRESSION-2026-08-25 -->
## ⛔ ВЫРЕЗКА G-0 ПРОСАДИЛА CRAFT — 22/22 -> 15/22, ЧИНЮ (2026-08-25)

Ворота поймали то, что я сам сломал:

    nav    14/14, отказов гейтов 0     — цел
    craft  15/22, отказов гейтов 7     — упал
    падают: mine_coal, mine_diamond, mine_stone, pickup_flat, wander_recovery

⛔ МОЯ ПРОЦЕССНАЯ ОШИБКА В ОСНОВЕ: craft 22/22 я проверил после ПЕРВЫХ коммитов G-0 (Nav и
блуждание), а поздние — аксессор, настройки, тип цели, списки запретов — воротами НЕ
закрывал. Рефакторинг не считается проверенным по тому коммиту, которому повезло попасть
под замер.

НАЙДЕНО И ПОЧИНЕНО (одна причина из нескольких):
`avoidBreaking(Block...)` — собственный API altoclef (портал Нижнего мира, блоки голема,
нажимная плита храма). Его список читал удалённый пасфайндер. Я перенёс хранилище в
AltoClef — сеттеры компилируются, а ЧИТАТЕЛЯ НЕТ. «Не ломай это» тихо перестало что-либо
значить. Подключил к `WorldHelper.canBreak`, рядом с позиционным запретом, который выжил.

ОСТАЁТСЯ ОТКРЫТЫМ: `wander_recovery`, `mine_stone`, `mine_diamond` падают и после починки
списка. При этом само блуждание ЕЗДИТ: `wander=1380, wanderMoved=825`. Значит причина не в
том, что порт не двигает тело.

СЛЕДУЮЩИЙ ЗАХОД: разобрать wander_recovery по одному прогону — что именно курс проверяет и
чего не хватает после замены легаси-explore.

## ⛔ ПРАВКА ВОЗОБНОВЛЕНИЯ ПОСЛЕ ДОБЫЧИ — МЕХАНИЗМ ВЕРЕН, СТОЙ НЕ ЛЕЧИТ

`resumeGotoAfterMining` гейтится на `TungstenMod.TARGET`, который ставят только ручные входы
(`;goto`, кейбинд, следование, py4j). Привод altoclef — тот, что ведёт ВЕСЬ проход — его не
ставит. Значит возобновление после каждой добычи выходило на первой строке.

Правка (возобновлять цель самого поиска) срабатывает `gotoResumedFromSearch=139` раз при 140
завершениях добычи — то есть чинит настоящий мёртвый код. Но `mqSteps=0`, `pdWalking=0`:
цикл сменился с «добыл → никто не возобновил» на «добыл → возобновил → снова добыл».
Застой не вылечен. Флаг ВЫКЛЮЧЕН до отдельного замера на здоровом craft.

<!-- STALL-MECHANISM-2026-08-25 -->
## ⛔ МЕХАНИЗМ ЗАСТОЯ НАЙДЕН: ЦИКЛ «ЛОМАЙ СТЕНУ — СОТРИ ПУТЬ» (2026-08-25)

Воспроизведено по точным координатам за 90 секунд вместо 30-минутного прогона
(`repro_stall.py 1205 104 -846 --from 1219 104 -843`):

    pdEnter=1921   pdWalking=0   mqStarted=0   bs=23/22   gridCorner=7640
    состояние: "Tungsten (primary) planning..." — 90 секунд, ноль движения

ЧТО ОПРОВЕРГНУТО ПРЯМО НА РЕПРО (по 90 секунд каждое):
  - `gridBfsRefusesCornerCut` — выключил, `gridCorner=0`, застой ОСТАЛСЯ. Не он.
  - «блок-путь не находится» — строка `Failed! No block path` в логе НОЛЬ раз,
    `bs=23/22` говорит, что поиск отрабатывает.

ЧТО ПОДТВЕРЖДЕНО ТРЕМЯ СТРОКАМИ ЛОГА:

    primDrive robustPath present=false sz=0 fresh=true    ← привод не видит пути
    [Tungsten] Mining done — passage open                  ← добыча идёт и ЗАВЕРШАЕТСЯ
    (Failed! No block path — ноль раз)                     ← путь при этом находится

Цикл замыкается в `PathFinder.search`, ветка «Already standing at the wall»:

    EXECUTOR.startBreaking(pendingBreaks);
    PathFinder.blockPath = Optional.empty();    // путь СТИРАЕТСЯ
    return;                                      // "the goto retry drives the rest"

Бот утыкается в стену → ломает → стирает блок-путь → возвращается. Привод видит пустой
путь → пинает новый поиск → тот снова находит короткий путь к стене → снова ломает.
«Retry, который доведёт остальное», входит в ту же ветку. Вечно.

СЛЕДУЮЩИЙ ЗАХОД: понять, почему после `Mining done — passage open` следующий поиск снова
даёт путь ≤2 узлов к той же стене — открылся ли проход на самом деле. Прибор: счётчик
входов в эту ветку и позиция стены, чтобы отличить «ломает одно и то же» от «ломает
разное и не продвигается».

<!-- G0-DONE-2026-08-24 -->
## ⭐ БАРИТОН И ШРЕДДЕР ВЫНЕСЕНЫ ПОЛНОСТЬЮ (2026-08-24)

    импортов baritone в altoclef        0
    живых ссылок в коде                 0
    shredder в сборке                   убран (исходники остались референсом)
    джар                                7.2 МБ -> 6.17 МБ

ПРОВЕРЕНО ПРОГОНАМИ, а не только сборкой:  **nav 14/14, craft 22/22, отказов гейтов 0.**

ЧТО ГОВОРИЛИ СЧЁТЧИКИ ДО ЭТОГО (я построил прибор и месяц в него не смотрел):

    legacyDrive = путь / перекрытие / explore
                  8558 / 18   / 9384
                  8079 / 2134 / 8603
                  7988 / 610  / 7952

Легаси-движок вёл тело ~8000 тиков за прогон (семь минут из четырнадцати), explore активен
~9000, и до 2134 тиков он рулил ОДНОВРЕМЕННО с тунгстеном. Два движка на одном теле — это
и есть фризы и «смотрит в одну сторону, действует в другую», о которых говорил оператор.

ПОТЕРЯНО, НАЗВАНО (не спрятано):
  - разбор схематик (.litematic/.schematic/.schem) жил в удалённом модуле; эндпоинт честно
    отвечает отказом, нужен свой ридер;
  - легаси-настройки цены пути остались без потребителя вместе с пасфайндером.

## ⛔ ЧТО ВСКРЫЛА ВЫРЕЗКА — ВОСПРОИЗВОДИМЫЙ ОТКАЗ ПЛАНИРОВАНИЯ (2026-08-24)

Два 30-минутных прогона подряд встают В ОДНОЙ И ТОЙ ЖЕ ТОЧКЕ:

    бот:        1219.5, 104.1, -843.5      (не менялась с t=854s по t=1432s)
    цель:       1205,   104,   -846        14 блоков по горизонтали, ТОТ ЖЕ уровень
    состояние:  <Getting to block> "Tungsten (primary) planning..."
    инвентарь:  items=0

Десять минут «планирования» ровного маршрута на четырнадцать блоков. Раньше такие случаи
подхватывал баритон — поэтому дефект не был виден. Теперь подхватывать некому, и это
правильно: он стал измеримым.

СЛЕДУЮЩИЙ ЗАХОД — СЮДА. Координаты точные, случай воспроизводится, есть repro_stall.py:
телепортироваться в точку и читать блоки, а не гадать.

<!-- CEILING-BROKEN-2026-08-24 -->
## ⭐ ПОТОЛОК ПРОБИТ — ВПЕРВЫЕ УГОЛЬ, ПЕЧЬ И ЕДА (2026-08-24)

Два 30-минутных прогона против двух базовых на том же окне:

    БАЗА     ничего не достигнуто  /  4 ступени БЕЗ каменных инструментов
    ПРАВКА   5 ступеней, каменные@44.3s        sleepDeclined 434
    ПРАВКА   8 ступеней: stone tools@111.8s,
             УГОЛЬ@247.5s, ПЕЧЬ@406.7s, ЕДА@905.4s   sleepDeclined 9915

Две правки, порознь ни одна не хватила:
  1. `toolsetYieldsToOre` — руда перестала проигрывать плоскому 520 по арифметике;
  2. `sleepNeedsAnObtainableBed` — ночь перестала уходить на крафт кровати с нуля.

⛔ УРОК ПРО ЗАМЕР, А НЕ ПРО КОД: блокер с ночью прожил СУТКИ незамеченным только потому,
что все прогоны шли по 14 минут и ночь не успевала наступить. Дневная игра мерилась,
дневные проблемы чинились. ДЛИНА ОКНА — ЭТО ЧАСТЬ ПРИБОРА. Для прохода нужен хотя бы один
30-минутный прогон на каждую значимую правку.

## ⛔ СЛЕДУЮЩАЯ ЦЕЛЬ НАЗВАНА ТОЧНО: ПОДХОД К ЖИВОЙ СУЩНОСТИ (2026-08-24)

Девятиминутное залипание из 30-минутного прогона разобрано до листа цепочки, и подпись
задачи обманывала так же, как обманул `_Bari`:

    видимая подпись:  "Doing stuff in crafting_table container"
    счётчики контейнера: cgTick=0 cgCraftable=0 cgNotCraftable=0 cgSent=0   -- НОЛЬ
    настоящий лист:   <Approach entity entity.minecraft.rabbit> "Going to entity"

Верстака бот не касался ни тика. Он девять минут подходил к КРОЛИКУ и не подошёл.

ПОЧЕМУ ЭТО НЕ ЗАКРЫТО ОТГРУЖЕННОЙ ПРАВКОЙ: `lockSkipsReplanForSettledDrops` покрывает
только осевшие ПРЕДМЕТЫ. Живая цель пересчитывает маршрут всегда — намеренно, иначе
ломается chase_terrain (измерено: порог 1.5 роняет погоню, порог 0.35 не даёт ничего).

ЧТО ИЗВЕСТНО ПРО ЭТОТ КЛАСС ИЗ СЕГОДНЯШНИХ ЗАМЕРОВ:
  - лок не паралич: простой 0.2% тиков, тело двигается 31-41% -- осцилляция;
  - маршрут пересчитывается каждые 3 секунды, десять раз за 30-секундный лок;
  - для ПРЕДМЕТА пропуск пересчёта помогает; для живой цели нужен другой различитель.

СЛЕДУЮЩИЙ ЗАХОД: счётчик геометрии подхода именно к ЖИВОЙ цели (расстояние, смещение
тела, смещение цели за интервал) -- отделить "цель убегает" от "мы не можем к ней пройти".
Правило прежнее: сначала число, доказывающее ветку, потом правка.

## ОТКРЫТО ПОСЛЕ ПРОБИТИЯ ПОТОЛКА (2026-08-24)

Обе задачи замечены в той же паре прогонов и НЕ закрыты:

  - **смерть УТОПЛЕНИЕМ** (`tester1 drowned` в логе сервера). Механизм не установлен;
    водный pathfinder есть, почему бот не всплыл — не измерено;
  - **залипание в `Doing stuff in crafting_table container`** с пустым инвентарём после
    смерти: бот открыл верстак, крафтить нечем, и не выходит. Позиция не менялась 9 минут.

Обе — по правилу «сначала счётчик, доказывающий, что ветка живая, потом правка».

<!-- CEILING-CHAIN-2026-08-24 -->
## ⛔ ЦЕПОЧКА ПОТОЛКА СОМКНУЛАСЬ (2026-08-24)

Приоритеты задач (BeatMinecraftTask):

    уголь и железная руда   1050      getOrePriorityTask(...)
    каменный набор           520      addStoneToolsTasks()

Руда ВДВОЕ приоритетнее набора. Значит бот сидит на наборе не потому, что тот блокирует, а
потому, что задача на руду НЕ АКТИВИРУЕТСЯ: её приоритет считает
`DistanceOrePriorityCalculator` — от расстояния до руды. Нет известной руды — нет приоритета.

ГДЕ БОТ НАХОДИТСЯ ПОСЛЕ КАМЕННЫХ ИНСТРУМЕНТОВ (29 прогонов, 862 замера позиции):

    медиана Y = 82        min 54     max 136
    ниже Y=60:  22 из 862  =  2.6%
...
97.4% остатка прогона — выше Y=60.

⛔ И ВОТ ЧЕГО ЭТИ ЧИСЛА ЕЩЁ НЕ ДОКАЗЫВАЮТ, чтобы следующий не принял удобное за истину.
В 1.18+ уголь генерируется И ВЫСОКО (до Y≈190, пик около Y=95), железо — тоже двумя
полосами. То есть «бот не спускается» НЕ означает автоматически «руды рядом нет». На
медиане Y=82 уголь вполне может быть в двух шагах.

НЕДОСТАЮЩЕЕ ЗВЕНО, И ОНО ОДНО: видит ли сканер руду вообще. Нужен счётчик «сколько раз
coal_ore / iron_ore попадали в BlockScanner и на каком расстоянии». Пока его нет, есть два
равноправных объяснения, и они требуют РАЗНЫХ правок:

  (а) руды рядом нет  ->  нужна способность идти ВНИЗ / искать пещеры;
  (б) руда рядом есть, но не видна  ->  дело в сканере/радиусе, а спуск не поможет.

СЛЕДУЮЩИЙ ЗАХОД: сначала этот счётчик, потом правка. Ровно то правило, которое сегодня
уже спасло от пятой пустой догадки.
...
⛔ ДОСКА УСТАРЕЛА (флаг 2026-09-01, не переписываю чужие заявки). Эта заявка — середина августа
(G-1.94/95 датированы 2026-08-14/16). «Всё боевое занято вторым воркером» больше не факт: боевой
движок с тех пор прошёл насквозь через C6 (эта же сессия, реестр ниже) — здоровье, щит, crit-тайминг,
`WeaponSelector`, всё переписано и переизмерено вплоть до 2026-08-31. Если это читает будущая сессия
и раздумывает, можно ли трогать бой — старую заявку здесь как причину не держать, смотреть на C6 и
на реальный `git log`, а не на эту строку. Сама доска (заявить нить ДО начала) — механизм рабочий,
просто эта конкретная запись на ней протухла вместе со своим содержимым.
...
- [ ] **G-1.89 ⭐⭐⭐ ДОБЫЧА НЕ МЕДЛЕННАЯ — ДОБЫТОЕ ПРОПАДАЕТ. КРИВАЯ НАКОПЛЕНИЯ (2026-08-14).**
...
- [ ] **G-1.88 ⭐⭐⭐ СИНТЕЗ ЗА ДЕНЬ: ЯМА — ПРИЧИНА ОБОИХ РЕЖИМОВ ОТКАЗА `mine_stone`, А НЕ ОДНОГО
...
- [ ] **G-1.87 ⭐⭐⭐ ОТВЕТ: ПОД TUNGSTEN `UnstuckChain` ОТКЛЮЧЁН НАМЕРЕННО, А СОБСТВЕННОГО
...
- [x] **G-0 СВЯЗНОСТЬ. Пока altoclef импортирует `baritone.*`, удалить shredder нельзя.**
      ...
      * ⛔ ТРЕТЬЯ МЁРТВАЯ ВЕЩЬ ТОГО ЖЕ РОДА: `hasBaritoneGoal()` — примитив ДЛЯ АГЕНТА («идёшь ли
      ...
      ⛔⛔ ЗАКРЫТО, НАЙДЕНО ЛОЖНО-ОТКРЫТЫМ 2026-09-02 — галочка не стояла ДЕВЯТЬ ДНЕЙ ПОСЛЕ
      фактического завершения. Последняя видимая правка выше — 2026-08-10 (18 импортов). Задача
      была реально закрыта коммитом `05d74f3d` (2026-08-24, «G-0 COMPLETE: baritone and shredder
      are out of the code and out of the build»), который прямо пишет: «imports of baritone in
      altoclef: 0», «live references to baritone in code: 0». Проверено СЕГОДНЯ живым grep-ом
      (`grep -rl "^import baritone\." src/main/java/ --include=*.java`), не по коммиту на веру —
      **0 файлов**. `settings.gradle.kts` подтверждает и вторую половину (shredder закомментирован
      как исходник, не собирается). Тот же класс дефекта, что G-1.83 уже называл для этого файла
      («17% ложно открытых пунктов — постоянный налог на выбор следующей задачи») — только в
      обратную сторону: не закрыто-но-не-почищено, а открыто-но-давно-сделано.
...
- [x] **G-1.42 ФЛЕЙК `craft_at_distant_table`: бот УХОДИТ ОТ ЦЕЛИ И ЗАМИРАЕТ (2026-08-07).**
      ...
      **`tbl=6018/510@29`**. Дистанция теперь 29 (наш стол в 28), а не 44 — подметание сработало,
      ...
      ГИГИЕНА РЕЕСТРА 2026-09-01 (найдено через G-1.83's список 21 ложно-открытого пункта):
      чекбокс стоял `[ ]` при собственном тексте «КУРС ПОЗЕЛЕНЕЛ, 5/5». Перепроверено по коду —
      `InteractWithBlockTask.java:300-315` (`CANT_REACH` через `GetWithinRangeOfBlockTask`) на
      месте, комментарий цитирует именно этот замер. Переставлено на `[x]`. Более поздний
      «recipe book crafting» тангенс внутри этой же записи — ОТДЕЛЬНЫЙ, уже прослеженный в другом
      месте реестра (G-1.30-G-1.34, «крафт по книге рецептов портирован») сюжет, к флейку этого
      конкретного курса не относится.
...
- [x] **G-1.40 КАНДИДАТ (НЕ ВЫВОД!): подход к блоку вне досягаемости рулит МЁРТВЫМ движком (2026-08-07).**
      ...
      стабильным гейтом.
      ⛔⛔ ЗАКРЫТО 2026-09-02 — И ЭТО ЗАВЕРШАЕТ ОБЕ ПОЛОВИНЫ ИСТОРИИ. Найдено при чтении
      `InteractWithBlockTask.java` напрямую (0 импортов baritone, подтверждено). «Чистый
      рефакторинг без обещаний выигрыша», который эта запись называла оставшейся работой,
      РЕАЛЬНО СЛУЧИЛСЯ — и он СЛОМАЛ подход: убрав цель для мёртвого движка, ветка `CANT_REACH`
      осталась совсем без движения (`setDebugState("Getting to our goal")` + сброс таймера, ни
      цели, ни хода) — это ДОСЛОВНО пример из `docs/CHECKLIST.md`, ПРАВИЛО ЧЕТЫРЕ («бот стоял
      `dist=28.0` от верстака пять минут, объявляя, что идёт»). Починено правильно, не заплаткой:
      подход теперь идёт через живой привод — `GetWithinRangeOfBlockTask` (тот же класс, что water/
      lava escape), закэшированный экземпляр вместо пересоздания на каждый тик — подтверждено
      чтением текущего кода (`InteractWithBlockTask.java:283-324`), не по старой записи. Обе
      половины закрыты: гипотеза этого пункта опровергнута ЖИВЬЁМ, а собственная «оставшаяся
      работа» пункта сначала сломала подход, а затем была починена настоящим кодом.
...
- [x] **G-1.45 ХВОСТ G-0 РАЗМЕЧЕН: 7 КЛАССОВ-ПЕРЕХОДНИКОВ, КОТОРЫЕ НЕ АЛГОРИТМ, А ТИП (2026-08-07).**
      ...
      ⛔⛔ ЗАКРЫТО 2026-09-02, той же проверкой, что закрыла саму G-0 СВЯЗНОСТЬ выше: `util/
      baritone/` больше НЕ СУЩЕСТВУЕТ как директория (проверено `find`). Все перечисленные здесь
      «оставшиеся» классы — `GoalAnd`, `GoalBlockSide`, `GoalDirectionXZ`, `GoalDodgeProjectiles`,
      `GoalFollowEntity`, `GoalRunAwayFromEntities`, `PlaceBlockSchematic` — не найдены НИГДЕ в
      дереве (удалены целиком); `CachedProjectile` переехал в `adris.altoclef.util.projectiles`
      (не переходник цели, просто был не на своём месте). Согласуется с `05d74f3d` («G-0 COMPLETE»,
      2026-08-24) и с общим счётом импортов baritone = 0, проверенным заново в этой же сессии.
...
- [x] **G-1.49 КАРТА ОСТАТКА BARITONE: «61 файл» — это на самом деле 15, и они разложены (2026-08-08).**
      ...
      ⛔⛔ ЗАКРЫТО 2026-09-02, третий пункт того же рода в этом заходе (после G-0 СВЯЗНОСТЬ и
      G-1.45). Все 11 классов, названных здесь поимённо (`EscapeFromLavaTask`,
      `GetOutOfWaterTask`, `RunAwayFromCreepersTask`, `RunAwayFromEntitiesTask`,
      `RunAwayFromHostilesTask`, `DodgeProjectilesTask`, `SignShopTask`,
      `GetToOuterEndIslandsTask`, `KillEnderDragonTask`, `DragonBreathTracker`,
      `CustomBaritoneGoalTask`) проверены ПОШТУЧНО — каждый существует и в каждом
      `grep -c "^import baritone\."` даёт 0, а не только по общему счёту. `G-1.52` (закрыт
      2026-08-08, релиз v0.71.0) уже подтверждает порт лавы/воды 5/5 живым прогоном — ровно тот
      «осмысленный кусок G-0», который эта запись называла ближайшим.
...
- [ ] **G-1.48 ТРИАЖ ВСЕХ 29 МЕТОК ПОРТА — «26 заглушек» превращаются в ОДИН пункт по делу (2026-08-07).**
      ...
      • **СТАРЫЙ RECIPE-API — 4**: `CraftingRecipeVer`, `RecipeManagerWrapper`,
        ...
        «чинить».
        ⛔⛔ ПРОВЕРЕНО 2026-09-01, И «НАДО УДАЛИТЬ» — НЕВЕРНЫЙ ВЫВОД ДЛЯ `CraftingRecipeVer`, ПОЧТИ
        ПОВТОРИЛА ОШИБКУ С `RecipeManagerWrapper` ИЗ ЗАПИСИ НИЖЕ. `CraftingRecipeVer.getOutput`
        (`multiversion/CraftingRecipeVer.java:9-17`) — ДЕЙСТВИТЕЛЬНО ноль вызывающих в любой ветке
        препроцессора (`//$$`-строки других версий тоже проверены построчным грепом, не только
        активная) — но метод НЕ ПРОСТО мёртвый обход, это `//$$ TODO [1.21.11] Recipe.getResult()
        API changed` — незакрытая заглушка НЕЗАВЕРШЁННОГО порта. И у неё есть ЖИВОЙ читатель ПО
        ИМЕНИ: `CraftingHelper.java:26-33`, комментарий о крупном историческом дефекте (NPE от
        `getRecipeForItem`, вышибавший клиента из мира — «the pack is empty», «items vanish»,
        18-минутный прогон с `inGame=false` две трети времени), объясняет ЭТУ САМУЮ заглушку как
        причину, почему рецептный API «only half ported», и цитирует `CraftingRecipeVer` по имени
        как часть объяснения. Удаление класса не сломает сборку, но оборвёт ссылку в чужом
        комментарии и сотрёт маркер незавершённой работы, выдав его за законченную уборку. НЕ
        УДАЛЯТЬ — это отслеживаемый долг, а не мёртвый код; закрывать имеет смысл только вместе с
        завершением порта рецептного API на 1.21.11, если та работа когда-нибудь берётся в фокус.
        `RecipeManagerWrapper` и `WrappedRecipeEntry` — не перепроверялись отдельно в этом заходе
        (по записи ниже первый уже подтверждён живым на 1.21.1 через `//#else`; второй активно
        используется в незавершённой работе крафта, см. `TODOS.md:11461,11474` — тоже не мёртв).
      ...
      ⛔ ДОПОЛНЕНИЕ 2026-09-01, ДЛЯ БУДУЩЕЙ УБОРКИ ИМЕННО В `tungsten/`: правило «проверять
      `//$$`-ветки всех версий» выше — про `src/main/java` (altoclef), который ДЕЙСТВИТЕЛЬНО
      мультиверсионный (три собираемые линии, только что подтверждённые компилятором). Для
      `tungsten/` это правило НЕ ПРИМЕНИМО, и важно не спутать одно с другим в обратную сторону:
      `tungsten/build.gradle` не подключает препроцессор-плагин вообще (нет блока `preprocess
      {}`), собирает СТРОГО одну версию (`minecraft "com.mojang:minecraft:1.21.11"`,
      `yarn:1.21.11+build.4`, без списка версий) — и тем не менее одиннадцать файлов внутри
      `tungsten/src/main/java` до сих пор несут комментарии `//#if`/`//$$` (`TungstenMod.java`,
      `TungstenConfig.java`, `Agent.java`, `MixinWorldChunk.java` и другие — grep
      `'//#if\|//\$\$'` по модулю). Это НЕ живые ветки другой версии — это остаток ОТ ТЕХ ВРЕМЁН,
      когда tungsten компилировался инлайново через `versions/X.Y.Z` вместе с altoclef (до
      перехода на «standalone mod», см. `build.gradle:39-40`: «Tungsten is now a standalone
      mod... No longer compiled inline via srcDir»). Раз препроцессор tungsten больше не
      обрабатывает, `//$$`-строки там — НАВСЕГДА инертный текст, а не код другой версии: не
      нужно останавливать удаление мёртвого кода в `tungsten/` из опасения «а вдруг это живо на
      другой версии», как остановило в примере выше для altoclef. Симметричная ошибка в обе
      стороны одинаково возможна: за пределами `tungsten/` — удалить то, что живо на другой
      версии; внутри `tungsten/` — испугаться удалить то, что мертво на ВСЕХ версиях, потому что
      выглядит как версионный код. Проверено сегодня же на практике: пять удалений в
      `tungsten/helpers`, `tungsten/task`, `tungsten/world`, `tungsten/TungstenMod.java` и двух
      миксинах (декрес-кей исключён — см. C1.3) не задели ни одной `//$$`-строки — все правки
      были в обычном, всегда компилируемом Java-тексте, что проверено построчно перед каждым
      удалением, а не предположено постфактум.
...
- [~] **C1.3 zero-caller code:** `AttackTiming.canAttack` + `isCritState` (so no crit/w-tap timing at
  ...
  * decrease-key ветки в кучах — ДОПРОВЕРЕНО 2026-09-01, СВОЯ ЖЕ ЛОВУШКА ПОЙМАНА НА СЕБЕ.
    ...
    `BlockSpacePathFinder.java:407-410` (`if (child.isOpen()) { openSet.update(child); } else
    { openSet.insert(child); }`) и ДВАЖДЫ в `PathFinder.java` (`:2232-2236` для ветки >25
    детей, `:2260-2264` для ветки <=25, обе реально исполняются; закомментированная ТРЕТЬЯ
    копия того же фрагмента на `:2307-2319` — вот это действительно мёртвый обрезок, но не он
    был вопросом). Это выглядело как «декрес-кей живой» — и было бы неверным выводом.
    Проверка на уровень глубже: `update()` вызывается ТОЛЬКО когда `child.isOpen()` истинно, а
    `isOpen()` — это `heapPosition != -1` (`BlockNode.java:208-209`, `Node.java:87-88`), и
    `heapPosition` инициализируется `-1` в КАЖДОМ конструкторе обоих классов узлов
    (`BlockNode.java:159,176,202`, `Node.java:74,84`) и меняется ТОЛЬКО самой кучей при
    вставке/удалении. То есть `child.isOpen()` может быть истинным лишь если ЭТОТ ЖЕ объект
    (не позиция, не значение — ссылка) уже когда-то был вставлен в кучу. А каждый кандидат —
    ВСЕГДА новый объект: `BlockSpacePathFinder.java:381-390`, комментарий на месте прямым
    текстом («there is no getNodeAtPosition here... A BlockNode is a per-EDGE object...
    Duplicate states are resolved lazily instead, at the pop above») подтверждает отсутствие
    карты позиция→узел для блок-пространства; в физическом поиске то же самое проверено
    отдельно — `parent.getChildren(...)` (`PathFinder.java:2122`) вызывает `Node.getChildren`
    (`Node.java:136`), которая на каждый кандидат делает `new Node(this, world, new
    PathInput(...), ...)` (`:341,364,419,453,457`) — ни разу не ищет и не переиспользует уже
    существующий узел той же позиции. РЕЗУЛЬТАТ: раз узла с уже установленным `heapPosition`
    для той же позиции просто не существует в системе, `child.isOpen()` структурно не может
    стать истинным ни при каких условиях — ветка с `openSet.update(...)` синтаксически жива
    (компилируется, вызывается интерпретатором), но её ГВАРД никогда не открывается. Это ТОЧНО
    та ловушка, что этот же файл называет в другом месте про счётчик, не про декрес-кей
    (`stranded`/`dteUnsafe`, а в `docs/CHECKLIST.md` — правило 4u, «счётчик внутри своего же
    флага читает ноль в контроле по построению») — здесь тот же почерк: строка, которая
    выглядит вызываемой, гарантированно не срабатывает по условию, а не по недостижимости
    кода. ИТОГ: исходная формулировка пункта («dead decrease-key branches in both heaps»)
    ПОДТВЕРЖДЕНА — в обеих кучах, с конкретным механизмом отказа, а не только по подозрению.
...
- [x] **C2.2 No g-cost accumulation.** `BlockSpacePathFinder.updateNode:345-364` does
  ...
  файл, независимая перепроверка). ⛔ УТОЧНЕНО ПОЗЖЕ ТЕМ ЖЕ ДНЁМ: коммит найден по `git log -S`
  — `00c48c84` (2026-08-02), «tungsten: the block-space search is an A* again — nav 12/12 with
  the costs live», ИЗМЕРЕННЫЙ фикс, а не случайная правка (см. полную запись под тем же C5.21
  чуть выше в файле и её же обновлённый вывод). `updateNode` сейчас: `child.cost = tentativeCost`
...
- [x] **C5.2 Break cost priced with the item CURRENTLY HELD** while the executor swaps to the best tool
  ЗАКРЫТО 2026-07-31 + прогон, ⛔ **ЧАСТИЧНО НЕВЕРНО — ИСПРАВЛЕНО 2026-09-02, ПОСЛЕ ПРОВЕРКИ
  ОДНОЙ ФУНКЦИИ НА УРОВЕНЬ ГЛУБЖЕ.** Оригинальная запись утверждала, что `strVsBlock` считает по
  ЛУЧШЕМУ инструменту — это НЕПРАВДА, и `nav_break PASS 3/3` НЕ проверял именно это.
  `MovementHelperB.strVsBlock` (`:1061-1066`) вызывает `state.calcBlockBreakingDelta(player,
  ...)` — читает ТО, ЧТО СЕЙЧАС В РУКЕ, ровно как `BlockNode.breakTicks` — по СВОЕМУ СОБСТВЕННОМУ
  javadoc-комментарию ("there is no ToolSet in tungsten"). Без таблицы поиска скорости по
  предметам симулировать гипотетический лучший инструмент, не надевая его физически, НЕЛЬЗЯ —
  `calcBlockBreakingDelta` не берёт инструмент параметром, только читает текущий. Держать
  инструмент в руке — забота altoclef-стороны через `equipToolHook` (`PathExecutor.java:670`),
  который срабатывает во время ИСПОЛНЕНИЯ, а не заранее для каждой клетки-кандидата поиска.

  ЧТО В ЭТОЙ ЗАПИСИ ДЕЙСТВИТЕЛЬНО ПОЧИНЕНО (и, скорее всего, ЭТО и объясняет `nav_break PASS`,
  а не ложно заявленное): переход на `MovementHelperB.getMiningDurationTicks` реально принёс
  (a) `avoidBreaking`/`avoidAdjacentBreaking` — вето на слом рядом с жидкостью/неподдержанным
  падающим блоком (подтверждено живьём этой сессией, `docs/BARITONE-PORT.md`, раздел block
  breaking); (b) `breakCostMultiplierAt` — `COST_INF` для защищённого/неломаемого, невозможный
  копок становится непланируемым вместо сюрприза на исполнении. Оба — реальные, проверенные
  находки, просто НЕ «лучший инструмент».

  НЕЗАВИСИМОЕ ПОДТВЕРЖДЕНИЕ, найдено ПОСЛЕ правки, в СОВСЕМ ДРУГОМ документе: `docs/features/
  TUNGSTEN_ALTOCLEF_API.md` (дизайн-документ от 2026-07-20, старше самого тикета C5.2) уже
  тогда правильно различал два разных шага — «Ступень 1» (`equipToolHook`, исполнитель надевает
  лучший инструмент ПЕРЕД копкой — это СДЕЛАНО) и отдельный, явно названный «следующим шагом»
  и НЕ сделанным: «cost в block-space от ЛУЧШЕГО доступного инструмента (второй хук-supplier
  `bestBreakTicks(BlockState)`), не от текущей руки». Пять недель спустя этот «следующий шаг»
  всё ещё не сделан — документ, который никто не трогал и не обновлял в связи с C5.2, оказался
  точнее самой записи о закрытии.

  → ~20× mismatch (число из оригинальной записи, при held-item против best-tool — актуальность
  цифры под вопросом, раз пересчёт всё ещё на held-item).
...
- [~] **C5.16 `MovementAscend` ПЕРЕНЕСЁН, но НЕ ПОДКЛЮЧЁН — и почему это правильно**
  ...
  * `chase_terrain`: 12 → 22 замирания.
  ...
  ПЕРЕПРОВЕРЕНО ПО КОДУ 2026-09-01: `TungstenConfig.queueClimbs = true`, и в
  `MovementQueue.java:577` он гейтит ОБА ребра разом — `if (cfg.queueClimbs && (isAscendEdge(a, b)
  || isDescendEdge(a, b))) return true;` — то есть подъём и спуск включены одним и тем же флагом,
  как и написано здесь и в C5.17. `queueDiagonals = false` — откат диагонали тоже на месте. Код
  не разошёлся с записью.
...
⛔⛔ НАЙДЕНО 2026-09-01 — ВЕСЬ СМЫСЛ БЕНЧА `chase_terrain`/`chase_flat` МОГ МОЛЧА ИЗМЕНИТЬСЯ
ПОСЛЕ G-0, И ЭТО НЕ ПЕРЕПРОВЕРЕНО НИ РАЗУ. Вся сага C5.15-C5.20 выше (и `RW-9`, и
`PVP_SUITE.md`) построена на предпосылке «жертва бежит на baritone (`@goto`), наш бот гонится
на tungsten (`@follow`)» — это же написано прямым текстом в `deploy/runner/uctest/
scenarios_pvp.py` («Victim loops a rectangle on baritone @goto», «victim is sent running on
baritone toward a far point»). Но САМА сага (C5.15-C5.20) датирована 2026-07-30/31 —
ДО миграции G-0 (2026-08-24, см. раздел G-0 в этом файле), когда `TungstenHelper.primary`
действительно был `false` по умолчанию, и `@goto` жертвы честно шёл через baritone/shredder.
С 2026-08-24 умолчание — `primary = true` (см. LIVE-C выше), и **grep по всему
`deploy/runner/uctest/` не находит НИ ОДНОГО вызова `setTungstenPathing`/`useTungsten`/
переключения `primary`** — ни для бота, ни для жертвы. Оба контейнера (`mc-tester1`/
`mc-tester2`) поднимают ОДИН И ТОТ ЖЕ джарник (`deploy/compose.test.yml`) с одними и теми же
дефолтами. Значит СЕГОДНЯ, если этот бенч запустить заново, жертва ТОЖЕ бежит на tungsten —
«tungsten должен догнать baritone» перестало быть тем, что бенч фактически измеряет, и стало
«tungsten должен догнать tungsten».
НЕ УТВЕРЖДАЮ, ЧТО СТАРЫЕ ЗАМЕРЫ (C5.15-C5.20) НЕВЕРНЫ — они честно сделаны ДО G-0, предпосылка
на тот момент была правдой. Утверждаю только: ЛЮБОЙ бенч `chase_terrain`/`chase_flat`, снятый
ПОСЛЕ 2026-08-24, нуждается в проверке до чтения чисел — иначе рискуем сравнивать tungsten
сам с собой и не знать об этом (тот же класс ошибки, что рулят правила 4a/4r в CHECKLIST.md).
ЧТО СДЕЛАТЬ ПРИ СЛЕДУЮЩЕМ ЗАХОДЕ НА ЭТОТ БЕНЧ: явно поставить жертве `setTungstenPathing(false)`
(или эквивалент через `;settings useTungsten false`) ДО `@goto`, и проверить чтением лога
(таблица движков в NAVIGATION.md), что жертва действительно на baritone/shredder — ОДНАКО
шredder тоже не компилируется с той же самой миграции G-0 (см. этот файл выше), так что «вернуть
жертву на baritone» может оказаться физически невозможным. Тогда честная формулировка бенча —
не «tungsten против baritone», а «tungsten-погоня против фиксированного паттерна движения»,
и `RW-9`/`PVP_SUITE.md` должны быть переписаны под это, а не молчать об изменившейся
предпосылке.
...
- [x] **C5.21 ⛔ ШЕСТЬ ДЕФЕКТОВ В ДВУХ A*-ДРАЙВЕРАХ (`PathFinder`, `BlockSpacePathFinder`) —
  ...
  просто не нашёлся ни в каком виде.
  ⛔⛔⛔ НАЙДЕНО 2026-09-01, ПОСЛЕ КОММИТА ВЫШЕ: ЭТО НЕ ДРЕЙФ, ЭТО КОЛЛИЗИЯ НОМЕРА С УЖЕ
  ЗАКРЫТОЙ ЗАПИСЬЮ ТОГО ЖЕ ФАЙЛА — та же болезнь, что регистр уже один раз ловил и лечил на
  себе под C6.10/C6.11 («было ПОВТОРНО помечено C6.10... перенумеровано в C6.11, чтобы поиск по
  ID не находил не тот пункт»), и я наступила на ту же грабли, потому что не поискала по
  соседним `C5.21`-заголовкам, а сразу начала перепроверять по тексту.
  `git log -S"tentativeCost = next.cost" -- .../BlockSpacePathFinder.java` называет коммит
  `00c48c84` («tungsten: the block-space search is an A* again — nav 12/12 with the costs
  live», 2026-08-02 18:13) — и его СОБСТВЕННОЕ сообщение открывается словами «Second attempt
  at C5.21, and this one measures. All six defects land together» — то есть автор коммита САМ
  назвал этот пункт регистра по номеру и перечислил все шесть в теле коммита. Запись об этом
  коммите УЖЕ ЕСТЬ в этом же файле, выше по тексту (не в другом файле, не потеряна) — под ТЕМ
  ЖЕ номером C5.21, заголовок «C5.21 ЗАКРЫТ (2026-08-02, попытка 3). ПОИСК В БЛОК-ПРОСТРАНСТВЕ
  СНОВА A* — nav 12/12, отказов гейта 0; pvp 9/12 — без регрессии», с полным замером (см. запись
  выше в этом же разделе, сразу перед `C5.21-ПОПЫТКА-2`/`C5.21-ПРОБА`). Так что: не «фикс
  потерялся в несвязанной правке» — фикс СДЕЛАН ИМЕННО ДЛЯ ЭТОГО ПУНКТА, ИЗМЕРЕН, ЗАКРЫТ И
  ЗАПИСАН — просто ПОД ТЕМ ЖЕ РЕГИСТРОВЫМ НОМЕРОМ ЗАВЕЛИ ВТОРУЮ, ОТДЕЛЬНУЮ ЗАПИСЬ (эту, с
  заголовком «ШЕСТЬ ДЕФЕКТОВ»), и с тех пор две записи с одним ID жили в файле одновременно —
  одна закрытая, одна открытая, и никто их не сверил друг с другом за месяц.
  ЗАКРЫВАЮ ЭТУ ЗАПИСЬ (`[x]`), а не оставляю `[~]`: пятое подпунктовое расхождение (лишний
  `continue`, не найденный в коде ни в каком виде) снимается тем же коммитом — то же сообщение
  перечисляет «5/6. the extra `continue` and the extra `&& !failing` are gone» одной строкой,
  подтверждая, что оба были убраны в этом же проходе, а не то, что один починили, а другой
  испарился сам. Дубликат-ID не переименовываю (в отличие от C6.10/C6.11) — эта запись больше
  не описывает открытый вопрос, переименование нужно только живым конфликтующим пунктам.
...
- [~] **C5.22 МЁРТВЫЕ ДУБЛИ И `Thread.sleep` В ПОИСКЕ (тот же аудит, 2026-08-02).**
  ...
  * `CornerJump.java:74,89` инкрементит `limit` дважды за итерацию — бюджет 40 тиков вдвое
  ...
  корректно: `newNode.agent.isInLava()` через `+=`). На тот момент ничего не почищено.
  ⛔⛔ ПЕРВЫЙ ПУНКТ УДАЛЁН 2026-09-01, А ЗАТЕМ ВОССТАНОВЛЕН В ТОЙ ЖЕ СЕССИИ — СВОЯ ЖЕ ОШИБКА,
  ПОЙМАННАЯ ЧТЕНИЕМ `docs/BARITONE-PORT.md`, А НЕ ВНЕШНИМ ЗАМЕЧАНИЕМ. Сначала вырезала
  `BlockStateChecker.fullyPassableBlockState` и `MovementHelper.canWalkOnBlockState` целиком
  (со всеми ставшими ненужными импортами), доверившись формулировке ЭТОГО пункта «устаревшие
  ДУБЛИ живых MovementHelperB-функций... удалить». Ноль вызывающих было подтверждено грепом
  честно — но «ноль вызывающих» и «безопасно удалить навсегда» НЕ одно и то же, и именно это
  смешение здесь произошло.
  ПОЗЖЕ, при чтении `docs/BARITONE-PORT.md` (более детальный, adversarially-verified аудит,
  58 re-derived находок) для ДРУГОЙ задачи — обнаружились ТРИ отдельные находки, называющие
  РОВНО эти два метода по файлу и строке (`tungsten/helpers/BlockStateChecker.java` и
  `tungsten/helpers/MovementHelper.java:29`) НЕ дублирующимся мусором, а НЕДОПОРТИРОВАННЫМИ
  ЗАГОТОВКАМИ — законным местом ДОДЕЛАТЬ порт пассабельности из baritone, а не выбросить:
  «Finish the port — add the water/lava MAYBE arms and canWalkOnPosition... — and then actually
  consult it from FastPlanner.step and PlayerFit.standable», и отдельно: «tungsten already
  contains a partial copy of baritone's canWalkOnBlockState table... but NOTHING calls it...
  the live planner re-derives standability from raw collision shapes, which is the root cause
  of the hazard and slab findings above». То есть «ноль вызывающих» аудит уже видел и назвал
  ЭТО ЖЕ ОТДЕЛЬНЫМ ДЕФЕКТОМ (незаконченное подключение), а не подтверждением, что код мусорный.
  `MovementHelperB`'s собственная версия ТОЖЕ неполная (сам этот пункт выше говорит «оба с
  выброшенными половинами условий») — то есть это НЕ «один хороший, один дублирующий мусор»,
  а два НЕЗАВИСИМЫХ частичных порта, ни один явно не превосходит другой по всем статьям.
  ⛔ ВОССТАНОВЛЕНО ДОСЛОВНО из `git show 6b882889^` (коммит подтверждён как единственный,
  трогавший эти файлы с момента создания репозитория — `git log` по обоим путям чист после
  него). Пункт регистра переписан, чтобы не повторить ту же ошибку: НЕ удалять эти два метода
  без отдельного решения ДОДЕЛАТЬ или ОКОНЧАТЕЛЬНО отказаться от порта пассабельности — это
  решение по существу, не безопасная уборка.
  Остальные три пункта (Thread.sleep в DivingMove, потерянная ветка WalkToNode, двойной
  инкремент CornerJump) — НЕ трогала: это или сознательные компромиссы производительности
  (`DivingMove`), или правки поведения физ-движка, помеченного неприкосновенным без отдельного
  A/B-прогона на живом стенде (стенд по-прежнему недоступен — C8.1), а не мёртвый, безопасный к
  удалению код. Пункт остаётся `[~]`: ни один из четырёх фактов не «исправлен» в смысле
  готового решения — первый теперь честно ОТКРЫТ (не удалён, не доделан), три остальные как были.
...
- [~] **C5.10 СТРОИТЕЛЬ НЕ УМЕЕТ ХОДИТЬ — вторая половина `BuilderProcess` не перенесена**
  ...
  ПЕРЕПРОВЕРЕНО ПО КОДУ 2026-09-01 — ЭТО ТОТ ЖЕ ФИКС, ЧТО У C5.11, ПРОСТО НИКТО НЕ СВЁЛ ДВЕ
  ЗАПИСИ (седьмой такой случай в этом же реестре). `BlockPlaceHelper.java` теперь несёт флаг
  `walkToBuild` (`:163`, default **true**) и API `setWalkToBuild`/`walkToBuild()` (`:253-255`).
  Когда ни одна клетка в очереди не ставится с текущей позиции, код (`:409-490`) ИДЁТ туда, где
  можно: если клетка «стоячая» — прыжком через `PillarTask.startTo` (тот же манёвр C5.11), иначе
  — `FastNavigator.startExact(head)`, с ограничением попыток (`MAX_WALK_ATTEMPTS = 4`) и
  реакцией на остановку навигатора («THE NAVIGATOR STOPPING IS AN EVENT, AND THE BUILDER WAS
  DEAF TO IT», `:496-507`). Это буквально порт «движущейся половины BuilderProcess», о которой
  просит этот пункт. Итог тот же, что у C5.11: механизм есть, включён по умолчанию, и тот же
  курс (`diag_build`) в комментариях этого же файла (`:698`, «4 passes in 5 to 1» после честной
  проверки прибытия, затем чинится дальше) сходится к тем же 17/18 — формально флаки, не
  полностью закрыто. Не отдельный незакрытый дефект, а дубль указателя на C5.11.
...
- [x] **C5.13 `bridgeForward` ПАДАЕТ С КРАЯ, поставив НОЛЬ блоков** (найдено 2026-07-31 при
  ...
  мост на 6 клеток от самой кромки) — 0 клеток из 6, 3 прогона из 3 ДО ФИКСА (это описание
  повторяемости старого бага, не текущий результат).
  ГИГИЕНА РЕЕСТРА 2026-09-01: пункт был помечен `[ ]`, хотя собственный текст заканчивается
  словом «ЗАКРЫТО» и чистым замером 6/6×3 — переставлено на `[x]`. Код перепроверен: `AIM_PATIENCE
  = 100` (`BridgeTask.java:42`), сообщение `"Bridge gave up aiming after "` (`:339`) и геометрия
  `MOVE_BACK`/BACKPLACE (`:227,281`) на месте — фикс не регрессировал.
...
- [~] **C6.5 Shield is NEVER raised by the combat engine.** `ShieldBlocker` is reachable only from
  ...
  ⛔ ДОПОЛНЕНО 2026-09-01: `combatShieldEnabled` всё-таки был измерен — не на дуэльном курсе, а
  на `mob_trio` (раздел G-2 этого файла, ЗАХОД 9): «щит НЕЙТРАЛЕН» — со щитом 14/15/20 урона
  против 12/14/20 без, разница в пределах шума. Это ДРУГОЙ контекст (толпа мобов, не PvP-дуэль),
  и не закрывает вопрос C6.10/C6.11 про дуэльный набор без щита, но снимает «щит вообще не
  измерялся» как факт — измерялся, и на этом конкретном курсе пользы не показал.
...
- [ ] **C10.1 `TimeoutWanderTask` can produce ZERO net movement over an entire episode while the bot
  ...
  для A/B (высокий в контроле, низкий в фикс-плече).
  ⛔⛔ ИСПРАВЛЕНИЕ 2026-09-01, СВОЯ ЖЕ ОШИБКА: строка выше («замер ещё не проводился») НЕВЕРНА.
  Я не прочитала верх ЭТОГО ЖЕ файла перед тем, как писать C10.1, а там уже стоит парный замер
  РОВНО по этому флагу — «⭐⭐⭐ ЦЕЛЬ БЛУЖДАНИЯ НЕ СУЩЕСТВУЕТ В 94% СЛУЧАЕВ — НО СИМПТОМ ЭТИМ НЕ
  ОБЪЯСНЯЕТСЯ» (2026-08-29, ближе к началу этого файла). Механизм починен ПОЛНОСТЬЮ (контроль
  u23/23, u606/619, u50/52 — 96-100%; с правкой u0/316, u2/70 — 0-3%), а ИСХОД — простой
  +10.5 с/прогон на 2 парах (t всего +0.19, статистически ничто), и ГЛАВНОЕ: мёртвое время в
  ОБЕИХ ветках всё равно стоит в блуждании, 56%. То есть несуществующая цель — настоящий,
  подтверждённый дефект, но НЕ (единственная) причина простоя. Рекомендация «включить флаг и
  переизмерить» была моей собственной, не основанной на чтении уже проведённого замера — снимаю
  её.
  НАСТОЯЩИЙ СЛЕДУЮЩИЙ ШАГ уже назван в том же участке файла двумя более поздними записями
  того же дня: «ВНУТРИ БЛУЖДАНИЯ: НАВИГАЦИЯ "ВЕДЁТ ПУТЬ", А ТЕЛО СТОИТ» — на 83-94% тиков
  блуждания `Nav.isPathing()` отвечает истиной (маршрут якобы ведётся), а тело не сдвигается
  вообще; и «МЁРТВОЕ ВРЕМЯ ПРИПИСАНО: ДВЕ ТРЕТИ ЕГО — БЛУЖДАНИЕ» — подтверждено на 18 прогонах
  и двух сборках, что блуждание съедает 67-70% всего мёртвого времени прохождения, независимо
  от почина проводника (пасфайндера), сделанного в другом месте той же сессии. То есть вопрос
  не «куда ведёт маршрут блуждания» (это уже почищено и не помогло), а «почему тело не движется,
  пока навигация утверждает, что ведёт путь» — слой НИЖЕ выбора цели. Это ещё не решено на
  момент этой записи; следующий заход — трассировать, что именно возвращает `Nav.isPathing()`
  в эти тысячи тиков и доводит ли привод тело вообще.
...
- [~] 13. TUNGSTEN как DROP-IN замена baritone (проверить что реально работает):
  ...
  - [~] 13.4 ПОРТ BARITONE-ЭВРИСТИК в tungsten (напоминание юзера): у baritone куча
    ...
    ⛔ ПЕРЕКРЁСТНАЯ ССЫЛКА 2026-09-01: по факту НАБРАЛОСЬ на «в основном сделано», просто
    россыпью по отдельным пунктам реестра, не одним флагом. Эвристики A* — `FastPlanner`
    несёт честный g + октильную эвристику с апстримовым коэффициентом (`searchHeuristicScale`,
    дефолт 3.563, см. C5.21), `path/calculators/ActionCosts.java` — перенесённые cost'ы. Ходы
    (то, чем эти эвристики пользуются) — `MovementAscend`/`MovementDescend`/`MovementTraverse`/
    `MovementDiagonal`/`MovementPillar`/`MovementFall`/`MovementSwim`, каждый со своей пометкой
    «Port of baritone/pathing/movement/movements/...», проверялись этой же сессией (C5.16/17/
    C5.20 и раньше). Стак-детект — `UnstuckChain` перестроен многократно (URG-2, эта сессия).
    Wander — портирован (`TimeoutWanderTask`, `wanderUsesTungsten=true`), но с ИЗВЕСТНЫМИ,
    измеренными дефектами (C10.1, а также вся investigate-цепочка 2026-08-29 «ВНУТРИ БЛУЖДАНИЯ:
    НАВИГАЦИЯ ВЕДЁТ ПУТЬ, А ТЕЛО СТОИТ» ближе к началу этого файла) — портирование состоялось,
    качество — отдельный открытый вопрос. Dimension-логика — НЕ проверялась отдельно в этом
    заходе (нашлось только по одному упоминанию Nether/dimension в `PathFinder.java`/
    `MovementFall.java`, глубина не оценена). Помечено `[~]`: большая часть портирована и
    измерена, dimension-часть не проверена, а качество wander — открытая, активно ведущаяся
    задача, не «готово».
- [~] 14. ПОЛНАЯ break/place-совместимость tungsten с ограничениями baritone/altoclef:
  ...
  - [ ] 14.5 (осталось) авто-детект приватов В САМОМ tungsten при фейле слома (сейчас
    ...
    ПРОВЕРЕНО 2026-09-01, ОСТАЁТСЯ ОТКРЫТЫМ: grep по `PathExecutor.java` на авто-детект/
    авто-пометку привата — пусто. Ничего не изменилось с записи.
...
- [~] 17. КОМБАТ-ДВИЖОК: multi-target / avoid-target (интеграция altoclef):
  ...
  - [ ] 17.2 (осталось) altoclef-мозг: приоритизация целей (ХП/дистанция/угроза),
    ...
    ПРОВЕРЕНО 2026-09-01, ОСТАЁТСЯ ОТКРЫТЫМ, И СОВПАДАЕТ С УЖЕ ИЗВЕСТНЫМ ФАКТОМ: класса
    приоритизации целей/угроз не нашлось нигде в `combat/`. Это тот же пробел, что FIGHT-1
    называет «единого оркестратора на стороне altoclef нет» (см. PRIORITY BLOCK выше) — два
    указателя на один и тот же незакрытый вопрос под разными именами.
...
⛔ ГИГИЕНА 2026-09-01: пункты 4 и 5 — самая старая формулировка этой задачи в файле
(2026-07-20), и с тех пор основная часть выполнена. Не переставляю чекбоксы пачкой, но
закрытие ядра стоит записать явно, чтобы не читать раздел как «ничего не сделано».

- [ ] 4. Полный слой ввода мыши в py4j (инкапсулированно, БЕЗ дублей):
      ПРОВЕРЕНО ПО КОДУ 2026-09-01: `Py4jEntryPoint.mouseClick` (:1762), `screenClickAt` (:1792) и
      `interactCrosshairEntity` (:1849) — все три существуют и реализованы (4.2/4.3 в тексте
      стояли `[ ]`, но код есть; чекбоксы подпунктов не трогаю, поведенчески не переизмеряла).
      4.4 («interactionManager==null») отдельно не проверяла.
...
- [x] 5. MCP-сервер к моду (юзер хочет подрубать Клода напрямую по MCP):
      ЗАКРЫТО, ПРОВЕРЕНО ПО КОДУ 2026-09-01: `Settings.mcpEnabled = true`, `mcpPort = 25350`
      (`Settings.java:620-627`), `AltoClef.java:211` поднимает встроенный MCP-сервер. Совпадает
      с уже отдельно закрытым пунктом 12 в разделе «МЕГА-ЦЕЛЬ 2» этого файла («MCP-сервер В МОДЕ
      по LAN — реализовано 2026-07-21», `mcp_test PASS 4/4`) — это тот же факт под двумя именами,
      просто здесь, в самой первой формулировке задачи, чекбокс никогда не подняли.
...
⛔ ГИГИЕНА 2026-09-01: пункты 1-3 ниже (и всё их глубокое дерево подпунктов) — САМАЯ СТАРАЯ
формулировка в этом файле (2026-07-20, до того как shredder вообще получил имя — см. подпункт
1.1, «поиск подходящего имени... Solved: "shredder"»). С тех пор реальность обогнала этот план
целиком: (1) «новый пасфайндер = baritone + tungsten» стал `shredder`, а `shredder` ПОЛНОСТЬЮ
ВЫРЕЗАН из проекта миграцией G-0 («баритон и шредер вынесены полностью», 2026-08-24, см. раздел
G-0 выше) — сейчас единственный движок tungsten; (2) «PVP: полный аудит и переделка комбата»
(пункт 2) — это то же задание, что раздел **C6** этого файла покрывает несравнимо подробнее и
свежее (вплоть до 2026-08-31); (3) «научить ломать и ставить блоки» (пункт 3) — то же самое, что
раздел **C5** покрывает подробнее (C5.5-C5.14). НЕ РАЗБИРАЮ дерево подпунктов 1-3 построчно —
ценность отрицательная: почти всё внутри отвечает на вопрос про движок, которого больше нет.
Если что-то из глубины этого дерева понадобится (например, конкретная идея ускорения или
хода) — искать её здесь как ИСТОРИЮ идеи, а не как открытый пункт.
...
⛔⛔ НАЙДЕНО ПОСТФАКТУМ 2026-09-01, ПРЯМО В КОДЕ, БЕЗ ЗАПИСИ ЗДЕСЬ: работа над G-2 НЕ
остановилась 6 августа — она продолжилась 11 августа и добилась того, чего 16 заходов G-2.1 не
взяли, просто это никогда не попало в этот файл. Восстановлено по `git log` и по комментариям
`tungsten/combat/CombatController.java` (никакого пересказа по памяти — только цитаты кода):

**G-2.1 (mob_trio), продолжение, коммиты `8a622b2b`/`6631927b`/`d470e562` (2026-08-11):**
девятый по счёту случай «весь класс мёртв» в этом проекте (собственные слова коммита:
«the entire mob combat policy is javadoc — eleven constants, none of them read»). Одиннадцать
констант боевого движка против МОБОВ (`MOB_STRIKE_DISTANCE=2.9`, `MOB_BACK_OFF_DISTANCE=2.65`,
`MOB_ARM_REACH=2.25` и другие, `CombatController.java:63-111`) были объявлены и снабжены полной
спецификацией в javadoc, но НИ ОДНА не читалась — движок вёл бой с мобом ровно так же, как с
игроком (дистанции 2.4/1.6, рассчитанные на противника с досягаемостью 3.0, как у нас самих, а не
2.25, как у зомби). Причина всех 16 провалившихся гипотез G-2.1 в том, что ни одна не трогала
ЭТУ развилку — они настраивали константы игрока, а не подключали константы моба.
ФИКС: `strikeAt = vsPlayer ? STRIKE_DISTANCE : MOB_STRIKE_DISTANCE` (`:730-731`) — безусловная
ветка по типу цели, не флаг, не A/B-настройка. ЗАМЕР (12 прогонов на плечо, потиковый счёт
урона): было (константы игрока) `3 9 6 20 17 9 9 9 6 3 17 15` среднее 10.25 — стало (константы
моба) `3 9 3 3 6 12 6 3 17 6 0 3` среднее 5.92, медиана 9.0 → 4.5, урон почти вдвое меньше,
**и один из двенадцати прогонов дал 0 — первый в истории этого курса PASS `mob_trio`**
(цитата коммита: «the 0 is mob_trio's FIRST EVER PASS»). НЕ «критерий взят» — медиана всё ещё
4.5, не 0 — но впервые само явление «ноль урона» физически показано достижимым, а не только
постулировано. Смежная гипотеза (`d470e562`, вето на сближение при третьем мобе рядом,
`MOB_MIN_CENTRE_GAP=2.30`) измерена и ОТКАЧЕНА — среднее не улучшилось (8.83 против 5.92),
причина та же, что у всех отклонённых гипотез серии: подавление направления — это не
перепозиционирование, оно удлиняет контакт. `crowded`-гейт (`:1080,1098`, `countThreats(...)>=1`,
уже без случайной смены стороны обхода вблизи толпы) остался как единственная закреплённая
находка этой линии, что совпадает с ЗАХОД 10 выше.

**G-2.2 (mob_skeleton), продолжение, коммит `486fc10a` (2026-08-11):** нашёлся и почищен
дефект самого ГЕЙТА курса — он проверял `dw hits > 0` («стрела ПОПАЛА»), то есть идеальный
уклон (`min_hp=20.0`, ни одного попадания) засчитывался КРАСНЫМ той самой проверкой, что должна
была отличать «уклонился» от «скелет не может стрелять» (стоит смирно/безоружен). После починки
(проверка «скелет вооружён и не обездвижен», по серверным данным) курс дал **первый в истории
PASS** (`min_hp: 12 13 16 16 20 16 16 8`, n=8, 1 PASS) — по-прежнему красный по частоте (1/8,
честно так и названо, флаки — это красный, а не «почти работает»), но потолок подтверждён
достижимым. Остаток назван тем же коммитом и уже измерим: `total=63 click=0 cd=29 reach=36
angle=5 los=0 passed=4` — REACH отказывает в 35-56 из 49-83 попыток удара за бой, и это не
дистанционная константа (обе проверенные полосы — 2.9/2.65 и 2.4/1.6 — внутри собственной
досягаемости 3.0), а что-то другое держит бота снаружи.
⛔ ЭТА ЖЕ ЛИНИЯ УЖЕ ПОДРОБНЕЕ И ТОЧНЕЕ ЗАДОКУМЕНТИРОВАНА ДАЛЬШЕ ПОД **G-1.82** (2026-08-13,
287 прогонов, 6 предрегистраций): курс `mob_skeleton` ведёт себя ровно по своей арифметике
(≈20% шанс пройти при честной игре с обеих сторон), и решение — за юзером (оставить как есть /
чинить скорость убийства в ядре / пересмотреть порог курса). Не переоткрываю здесь, только
свожу указатель: то, что G-2.2 назвала «остаток», G-1.82 довела до полной модели и честного
вопроса юзеру.

НИЧЕГО ИЗ ЭТОГО НЕ ОТМЕНЯЕТ РЕШЕНИЕ ЮЗЕРА ОТ 2026-08-06 — не беру G-2 в работу как приоритет,
просто закрываю разрыв между тем, что в коде, и тем, что в этом файле, чтобы следующий заход не
проверял уже проверенное и не изобретал уже найденный фикс заново.
