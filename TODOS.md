...
- [ ] **G-0.1 SmartMoves НЕ УМЕЕТ ПЛАВАТЬ — и поэтому не может стать дефолтом (замер 2026-08-06).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД НАЧИНАТЬ С ДОКАЗАТЕЛЬСТВА: поставить счётчик прямо в
      `BlockNode.getChildren` (ветка `smartMoves`) и в `SmartMoves.generate`, прогнать
      `nav_water --pin smartMoves=true` и посмотреть, растут ли они. Пока они не выросли — любые
      правки в SmartMoves пишутся вслепую.
