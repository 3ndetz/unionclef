...
- [ ] **C5.13 `bridgeForward` ПАДАЕТ С КРАЯ, поставив НОЛЬ блоков** (найдено 2026-07-31 при
  ...
  Дополнительно замечено: бота уводит и по Z (старт z=0.5 → z=1.5), то есть тело идёт не строго
  по оси моста — подозрение на то, что `forwardKey` двигает по направлению КАМЕРЫ, а камера
  уведена прицелом на грань. Проверять это, а не гадать.
