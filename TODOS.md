...
- [~] 13. TUNGSTEN как DROP-IN замена baritone (проверить что реально работает):
  ...
  - [ ] 13.3b @gamer полный проход на tungsten-primary — nightly-масштаб + капризный
    ...
    (@get log PASS). ПРОВЕРКА 2026-07-22 (юзер спросил «gamer работает?»): на seed
    12345 спавн НА ГОРЕ (y=148). @gamer стартует, срубает лог, под @get log 5 ЕДЕТ
    (спуск 148→143), но ЗАСТРЕВАЕТ на горном рельефе. Отмёл: не MobDefense (на
    peaceful так же), не утечка punk (status inactive; PUNK-чат старый из combat-
    тестов), НЕ регрессия combat-работы (VoidGuard гейтится на punk/flee). Корень —
    #20: tungsten block-space неполон на сложном рельефе (ascend/склон). Блокер @gamer
    на реальной земле. Чистая диагностика точки залипания — после чистки чат-спама
    (сделано) + verbose. Дальше — порт недостающих Movements (#20/#21), большая задача
