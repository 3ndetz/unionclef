...
- [ ] **G-1.32 ЦЕПОЧКА ЗАДАЧ КРУТИТСЯ НА «ДОСКАХ» — СНЯТО С ЖИВОГО ПРОГОНА (2026-08-06).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: (1) добавить в стенд проверку «насколько бот ушёл ВНИЗ от старта» — это
      отделит «не нашёл дерево» от «уехал под землю» одним числом; (2) найти, что его туда ведёт
      (кандидаты: цель добычи под землёй, спуск в пещеру по пути, `collectPickaxeFirst`).
