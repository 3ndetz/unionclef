...
- [~] **C5.17 `MovementDescend` ПЕРЕНЕСЁН И ПОДКЛЮЧЁН — вместе с подъёмом даёт первый устойчивый
  ...
  ЗАМЕРЫ СЕРИЯМИ (по правилу, введённому после того, как одиночные прогоны меня обманули):
      база, только traverse:      20, 18, 19  → 19.0
      + подъём:                   19, 17, 16  → 17.3
      + подъём и спуск:           15, 16      → ~15.5
  Тренд устойчиво вниз по трём разным конфигурациям. nav 12/12, ноль отказов гейтов (предыдущее
