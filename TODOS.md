...
- [ ] **G-1.44 ⭐⭐⭐ ОБЩИЙ КОРЕНЬ ТРЁХ ЗАВИСАНИЙ: `Nav.isPathing()` ГЛУШИТ ВСЕ ПРОГРЕСС-ЧЕКЕРЫ (2026-08-07).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: править `AbstractDoToClosestObjectTask` — считать завершённые блуждания и
      после порога отдавать неудачу вместо нового блуждания. Гейт: `chop_canopy` (2/4 сейчас),
      `craft_at_distant_table` (3/5), и `wanderTicks` на красных должен рухнуть с ~3050.
