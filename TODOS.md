...
- [ ] **G-1.44 ⭐⭐⭐ ОБЩИЙ КОРЕНЬ ТРЁХ ЗАВИСАНИЙ: `Nav.isPathing()` ГЛУШИТ ВСЕ ПРОГРЕСС-ЧЕКЕРЫ (2026-08-07).**
      ...
      ⭐ А `canBreak` заканчивается на `&& canReach(pos)` — то есть проверкой БЛИЗОСТИ, осмысленной
      только для блока рядом с игроком. Но её применяют как ФИЛЬТР ПОИСКА по всем блокам мира.
      Отсюда самоподдерживающаяся петля:
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД (правка, наконец обоснованная замером): развести два вопроса —
      «стоит ли это ломать вообще» (твёрдость, защита, `plausibleToBreak`) и «дотянусь ли я ОТСЮДА»
      (`canReach`). Фильтру поиска нужен ПЕРВЫЙ; второй уместен только у самого действия.
