...
- [ ] **G-1.36 ДОБЫЧА СЛОМАНА НА САМОМ ПРОСТОМ ИЗ ВОЗМОЖНЫХ СЛУЧАЕВ (2026-08-07).**
      ...
      **НИ ОДНОЙ строки `PICKUPDEC` за прогон**. Значит `PickupDroppedItemTask.onTick` до неё не
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД, НА УРОВЕНЬ ВЫШЕ: кто должен ЗАПУСКАТЬ подбор. В
      `MineAndCollectTask.MineOrCollectTask.getClosestTo` дроп становится кандидатом только если
      `mod.getEntityTracker().itemDropped(_targets)` истинно, и лишь тогда `getGoalTask` вернёт
      `_pickupTask`. Проверять ПЕРВЫМ: видит ли трекер сущностей выпавший булыжник (rcon говорит,
      что предметы в мире ЕСТЬ — `Count: 3`). Если не видит — искать в трекере/фильтре целей.
