...
- [ ] **G-1.48 ТРИАЖ ВСЕХ 29 МЕТОК ПОРТА — «26 заглушек» превращаются в ОДИН пункт по делу (2026-08-07).**
      ...
      (а) УДАЛИТЬ мёртвый код (`ConstructNetherPortalSpeedrunTask` целиком, старый recipe-API —
      четыре файла, которые обошёл порт трекера), (б) боевые/бронные — когда юзер поднимет
      приоритет, (в) больше НИЧЕГО. Это уже не «класс дефектов», а уборка.
