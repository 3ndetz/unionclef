...
- [ ] **C5.15 ПОГОНЯ: СТЕНД МЕРЯЛ НЕ ТО — ТЕПЕРЬ МЕРЯЕТ, И ВИДНО 15 ЗАМИРАНИЙ**
  ...
  * бот при этом СТОИТ. Ходок работает и не двигает тело.
  ...
  ФИКС (следующий заход, делать ОСТОРОЖНО — одна поспешная правка ядра сегодня уже дала
  12/12 → 9/12): считать тики, где ходок «работает», а позиция бота НЕ меняется, и по порогу
  останавливать ходока — это само откроет уже существующий путь перепланирования. Проверять
  полным свипом nav, а не только погоней.
