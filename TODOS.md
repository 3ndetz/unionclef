...
- [ ] **G-1.47 БАЗОВАЯ ЛИНИЯ ДЛЯ #29 ЗАФИКСИРОВАНА, И ОДНА ЦЕЛЬ ПРИЗНАНА НЕПРОВЕРЯЕМОЙ (2026-08-07).**
      ...
      КОНСОЛИДАЦИЯ — оба планировщика ЖИВЫЕ ( водит бота, 
      зовётся из миксина и двух мест ). Такой задаче нужна полная nav-регрессия, и
      ...
      ⚠️ ЗАМЕЧАНИЕ ПРО ОДНУ ЦЕЛЬ G-0, КОТОРУЮ ЧЕСТНЕЕ НЕ ТРОГАТЬ: последний пользователь  —
       (композит из 8 блочных целей + ). Это
      контент ИЗМЕРЕНИЯ КРАЯ, которого нет НИ В ОДНОЙ сюите. Перенести можно, ПРОВЕРИТЬ нельзя —
