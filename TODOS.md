...
- [ ] **G-1.42 ФЛЕЙК `craft_at_distant_table`: бот УХОДИТ ОТ ЦЕЛИ И ЗАМИРАЕТ (2026-08-07).**
      ...
      **`tbl=6018/510@29`**. Дистанция теперь 29 (наш стол в 28), а не 44 — подметание сработало,
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: разобраться, ПОЧЕМУ сканер знает про блок в 29 блоках лишь в 8% тиков —
      это уже дефект ЯДРА (радиус/обновление `BlockScanner`), а не курса. Гейт готов: `tbl=` теперь
      прямо показывает и частоту находок, и дистанцию.
