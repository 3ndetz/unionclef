...
- [ ] **C5.15 ПОГОНЯ: СТЕНД МЕРЯЛ НЕ ТО — ТЕПЕРЬ МЕРЯЕТ, И ВИДНО 15 ЗАМИРАНИЙ**
  ...
  * `tick=78/65` у исполнителя — маршруты доезжают и проигрываются.
  ...
  СЛЕДУЮЩАЯ ЕДИНИЦА: в погоне физика не должна ВЫТЕСНЯТЬ ходока — только дополнять (как в
  `FastNavigator`, где физике отдают ногу, а ходок продолжает вести). Правка рискованная: гейт
  общий с навигацией, мерить и nav 12/12, и `chase_flat` (зелёный).
