...
- [ ] **C5.18 ПОТОЛОК СТАВИТ ПРАВИЛО НЕПРЕРЫВНОГО ПРЕФИКСА, А НЕ НАБОР КЛАССОВ ХОДОВ**
  ...
  ФИКС (следующая единица): очередь должна брать МАРШРУТ ЦЕЛИКОМ, выбирая класс на каждое ребро,
  и уступать ходоку только на рёбрах, для которых класса нет, — а не отказываться от всего хвоста
  после первого такого ребра. Мерить ТОЛЬКО чередующимся A/B (`;settings queueClimbs`), как
  показала сегодняшняя история с дрейфом хоста.
