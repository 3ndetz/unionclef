...
- [ ] **G-1.44 ⭐⭐⭐ ОБЩИЙ КОРЕНЬ ТРЁХ ЗАВИСАНИЙ: `Nav.isPathing()` ГЛУШИТ ВСЕ ПРОГРЕСС-ЧЕКЕРЫ (2026-08-07).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД — ЕЩЁ ОДИН СЧЁТЧИК, А НЕ ПРАВКА: разложить `canBreak` на три счётчика
      (hardness / shouldAvoidBreaking / plausibleToBreak) и прогнать `chop_canopy`. Один прогон
      скажет, какой именно член даёт 18384 отказа. Рабочая версия: бот уходит блуждать на 40
      блоков, чанки с кандидатами перестают быть известны/загружены, `getBlockState` отдаёт
      воздух — и `plausibleToBreak` честно говорит «ломать нечего». Но это ВЕРСИЯ; за сегодня
      четыре версии уже оказались неверными, и проверять её надо прибором.
