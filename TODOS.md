...
- [ ] **G-1.35 ВТОРАЯ СТУПЕНЬ: СТОРОЖ СЕТКИ СНОВА ЗАЦИКЛИЛСЯ — ТЕПЕРЬ НА КРОВАТИ (2026-08-07).**
      ...
      **предмет в курсоре не входит в `toMove.getMatches()`** — бот держит бревно, которое рецепт
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД: почему выбранный рецепт не принимает `oak_log`. Смотреть, какой именно
      `RecipeTarget` пришёл в `CraftGenericManuallyTask` (какой тип досок), и почему он не совпал с
      тем, что лежит в рюкзаке: скорее всего выбран рецепт под другой тип дерева (matching-materials
      выбирает БОЛЬШИНСТВО, а на старте большинства нет). Курс воспроизводит за 90 секунд.
