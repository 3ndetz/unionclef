...
Счётчик `standOffDeclined` = 57-307 тиков за прогон на edge_duel, то есть правка реально срабатывает.
⛔ НО ОНА НЕ ПРОВЕРЕНА: стенд с 16:20 сидит на 10 fps и не поднимается ни пересозданием клиентов, ни
`gradlew --stop`, ни рестартом сервера — на хосте 51-61% CPU чужой нагрузки. После правки удалось
взять всего по 2 здоровых прогона на плечо. Версия НЕ поднята, релиза НЕТ.
