...
- [ ] **C5.13 `bridgeForward` ПАДАЕТ С КРАЯ, поставив НОЛЬ блоков** (найдено 2026-07-31 при
  ...
  ФИКС (следующий заход, восьмой случай «механизм уже есть»): у моста должна быть геометрия
  BACKPLACE, как у `MovementTraverse` — идти СПИНОЙ вперёд и ставить ПОЗАДИ себя. Она уже
  перенесена и работает в `PathExecutor.tickPlacing` (ветка `MOVE_BACK`, присед, «forward in
  world, facing back»). `BridgeTask` умеет только идти лицом вперёд и смотреть вперёд — поэтому
  на кромке ей физически не на что навестись. nav 12/12.
