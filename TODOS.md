...
- [ ] **C5.11 ВЕРТИКАЛЬНЫЙ УЧАСТОК НУЖНО СТРОИТЬ ПРЫЖКОМ, А НЕ ХОДЬБОЙ** (найдено 2026-07-31,
  ...
  * цель `GoalPlace` = `target.up()` отдаётся навигатору, только если там реально можно стоять;
  ...
  ФИКС: отдавать вертикальный участок `PillarTask` (у неё уже есть исполнение подъёма), либо
  ставить строительные леса рядом и подниматься по ним — это вторая половина `BuilderProcess`,
  которая ставит scaffolding. До этого массовая стройка кладёт всё, что достижимо с земли, и
  честно отдаёт вертикаль агенту.
