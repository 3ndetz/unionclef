...
- [ ] **G-1.32 ЦЕПОЧКА ЗАДАЧ КРУТИТСЯ НА «ДОСКАХ» — СНЯТО С ЖИВОГО ПРОГОНА (2026-08-06).**
      ...
      топ лога — `FastPlanner: 1 nodes, 1 wp, complete, 0 ms`, 218 раз. То есть ФИЗИЧЕСКИЙ
      планировщик тоже возвращает план из ОДНОЙ вершины. Метод уже отработан дважды: заставить
      планировщик САМОМУ сказать, почему он не расширился (как это теперь делает BFS строкой
      «BFS stuck at … — сосед:причина»), и чинить названную причину, а не догадку.
