...
- [ ] **G-1.35 ВТОРАЯ СТУПЕНЬ: СТОРОЖ СЕТКИ СНОВА ЗАЦИКЛИЛСЯ — ТЕПЕРЬ НА КРОВАТИ (2026-08-07).**
      ...
      ⛔ СЛЕДУЮЩИЙ ЗАХОД, УЗКО: читать `MoveItemToSlotTask.onTick` вместе с `SlotHandler.clickSlot`
      на этом курсе — какой именно клик уходит (какой слот, какая кнопка) и что происходит со
      стеком ПОСЛЕ него (курсор? слот назначения?). Полезно поставить одну строку лога прямо в
      `clickWindowSlot`: слот, кнопка, стек до и после. Курс воспроизводит за 90 секунд.
