...
public class BeatMinecraft2Task extends Task {
    ...
    protected void onStart() {
        ...
        mod.getBehaviour().avoidBlockBreaking(blockPos -> {
            if (_bedSpawnLocation != null) {
                return blockPos.equals(WorldHelper.getBedHead(_bedSpawnLocation)) || blockPos.equals(WorldHelper.getBedFoot(_bedSpawnLocation));
            }
            return false;
        });
    }
    ...
    protected Task onTick() {
        ...
        if (!_endPortalOpened && WorldHelper.getCurrentDimension() != Dimension.END && _config.rePickupCraftingTable && !mod.getItemStorage().hasItem(Items.CRAFTING_TABLE) && !thisOrChildSatisfies(isCraftingTableTask)
                && (mod.getBlockScanner().anyFound(blockPos -> WorldHelper.canBreak(blockPos), Blocks.CRAFTING_TABLE)
        ...
        if (WorldHelper.getCurrentDimension() == Dimension.END) {
            ...
            if (mod.getItemStorage().hasItem(ItemHelper.BED) || (shouldForce(mod, _killDragonBedStratsTask))) {
        ...
        if (_config.sleepThroughNight && !_endPortalOpened && WorldHelper.getCurrentDimension() == Dimension.OVERWORLD) {
            ...
            if (!mod.getItemStorage().hasItem(ItemHelper.BED)) {
                if (mod.getBlockScanner().anyFound(blockPos -> WorldHelper.canBreak(blockPos), ItemHelper.itemsToBlocks(ItemHelper.BED))
    ...
    private boolean needsBuildingMaterials(AltoClef mod) {
        return StorageHelper.getBuildingMaterialCount() < _config.minBuildMaterialCount || shouldForce(mod, _buildMaterialsTask);
    }
    ...
    private List<Item> lootableItems(AltoClef mod) {
        ...
        if (!StorageHelper.isArmorEquipped(Items.GOLDEN_BOOTS) && !mod.getItemStorage().hasItemInventoryOnly(Items.GOLDEN_BOOTS)) {
            lootable.add(Items.GOLDEN_BOOTS);
        }
        if ((mod.getItemStorage().getItemCountInventoryOnly(Items.GOLD_INGOT) < 4 && !StorageHelper.isArmorEquipped(Items.GOLDEN_BOOTS) && !mod.getItemStorage().hasItemInventoryOnly(Items.GOLDEN_BOOTS)) || _config.barterPearlsInsteadOfEndermanHunt) {
    ...
    private boolean endPortalFound(AltoClef mod, BlockPos endPortalCenter) {
        ...
        // Check if all frame blocks are valid END_PORTAL_FRAME blocks
        return getFrameBlocks(endPortalCenter).stream().allMatch(frame -> {
            if (!mod.getChunkTracker().isChunkLoaded(frame)) return false;
            return mod.getWorld().getBlockState(frame).getBlock() == Blocks.END_PORTAL_FRAME;
        });
    }

    private boolean endPortalOpened(AltoClef mod, BlockPos endPortalCenter) {
        if (!_endPortalOpened || endPortalCenter == null) return false;
    ...
    private boolean spawnSetNearPortal(AltoClef mod, BlockPos endPortalCenter) {
        if (_bedSpawnLocation == null) return false;
    ...
    private Optional<BlockPos> locateClosestUnopenedRuinedPortalChest(AltoClef mod) {
        ...
        return mod.getBlockScanner().getNearestBlock(blockPos -> !_notRuinedPortalChests.contains(blockPos) && WorldHelper.isUnopenedChest(blockPos) && mod.getPlayer().getBlockPos().isWithinDistance(blockPos, 150) && canBeLootablePortalChest(mod, blockPos), Blocks.CHEST);
    }
    ...
    private Task getEyesOfEnderTask(AltoClef mod, int targetEyes) {
        ...
        switch (WorldHelper.getCurrentDimension()) {
            case OVERWORLD -> {
                ...
                if (shouldForce(mod, _gearTask) && !StorageHelper.isArmorEquippedAll(COLLECT_EYE_ARMOR)) {
                ...
                // Then get diamond
                if (!eyeGearSatisfied) {
                    _gearTask = TaskCatalogue.getSquashedItemTask(Stream.concat(Arrays.stream(COLLECT_EYE_ARMOR).filter(item -> !mod.getItemStorage().hasItem(item) && !StorageHelper.isArmorEquipped(item)).map(item -> new ItemTarget(item, 1)), Arrays.stream(COLLECT_EYE_GEAR)).toArray(ItemTarget[]::new));
                    return _gearTask;
                }
    ...
    private Task getEnderPearlTask(AltoClef mod, int count) {
        if (_config.barterPearlsInsteadOfEndermanHunt) {
            ...
            if (!StorageHelper.isArmorEquipped(Items.GOLDEN_BOOTS)) {
