...
/**
 * Roams around to terminate all players in an area.
 */
public class TerminatorTask extends Task {
    ...
    protected Task onTick() {
        ...
        if (!isReadyToPunk(mod)) {
            ...
            if (mod.getEntityTracker().getClosestEntity(mod.getPlayer().getPos(), entityAccept -> {
                if (!shouldPunk(mod, (PlayerEntity) entityAccept)) return false;
                if (entityAccept.isInRange(mod.getPlayer(), 15)) return true;
                ...
                try {
                    ...
                    if (player.isPresent() && player.get() instanceof PlayerEntity playerEntity && shouldPunk(mod, playerEntity)) {
                        _runAwayTask = new RunAwayFromPlayersTask(player.get(), RUN_AWAY_DISTANCE);
                    }
                } catch (ConcurrentModificationException e) {
                    Debug.logWarning("ConcurrentModificationException in TerminatorTask");
                    e.printStackTrace();
                }
            ...
            int buildingBlocks = mod.getItemStorage().getItemCount(Items.COBBLESTONE) + mod.getItemStorage().getItemCount(Items.DIRT);
            if (buildingBlocks < MIN_BUILDING_BLOCKS) {
        ...
        if (!mod.getItemStorage().hasItem(Items.DIAMOND_PICKAXE) && mod.getItemStorage().getItemCount(Items.DIAMOND) < 3) {
            if (!mod.getItemStorage().hasItem(Items.IRON_PICKAXE) || (_prepareDiamondMiningEquipmentTask.isActive() && !_prepareDiamondMiningEquipmentTask.isFinished())) {
        ...
        if (!StorageHelper.isArmorEquippedAll(ItemHelper.DIAMOND_ARMORS)) {
        ...
        if (_funnyMessageTimer.elapsed()) {
            if (LookHelper.seesPlayer(player, mod.getPlayer(), 80)) {
                ...
                if (_currentVisibleTarget == null || !_currentVisibleTarget.equals(name)) {
                    ...
                    mod.getMessageSender().enqueueWhisper(name, "Prepare to get punked, kid", MessagePriority.ASAP);
                }
            }
        }
    }
    ...
    public static class RunAwayFromPlayersTask extends RunAwayFromEntitiesTask {
        ...
        public RunAwayFromPlayersTask(Entity toRunAwayFrom, double distanceToRun) {
            super(() -> java.util.Collections.singletonList(toRunAwayFrom), distanceToRun, true, 0.1);
        }
